...
impl std::fmt::Debug for Database {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("Database")
            ...
            .field("transaction_count", &self.transaction_count.load(std::sync::atomic::Ordering::Relaxed))
            .field("query_count", &self.query_count.load(std::sync::atomic::Ordering::Relaxed))
            .finish()
    }
}
...
impl Database {
    ...
    pub fn record_transaction(&self) {
        self.transaction_count.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
    }
...
impl DbmsHandler {
    ...
    pub fn get(&self, name: &DatabaseName) -> Option<Arc<Database>> {
        self.databases.read().expect("lock poisoned").get(name).cloned()
    }
    ...
    pub fn list(&self) -> Vec<DatabaseName> {
        self.databases.read().expect("lock poisoned").keys().cloned().collect()
    }

    /// Check if a database exists.
    pub fn exists(&self, name: &DatabaseName) -> bool {
        self.databases.read().expect("lock poisoned").contains_key(name)
    }

    /// Assign a tenant to a database.
    pub fn assign_tenant(&self, db_name: &DatabaseName, tenant: TenantId) -> Result<(), DbmsError> {
        let db = self.get(db_name).ok_or_else(|| DbmsError::DatabaseNotFound(db_name.clone()))?;
    ...
    pub fn tenant(&self, db_name: &DatabaseName) -> Option<TenantId> {
        self.get(db_name)?.tenant.read().expect("lock poisoned").clone()
    }
}
...
impl BackupManager {
    ...
    pub fn list_backups(&self, db_name: &str) -> Vec<BackupInfo> {
        self.backups.read().expect("lock poisoned")
            .iter()
    ...
    pub fn remove_old_backups(&self, db_name: &str, keep_count: usize) {
        ...
        let mut db_backups: Vec<_> = backups.iter()
            .enumerate()
            .filter(|(_, b)| b.db_name == db_name)
            .map(|(i, b)| (i, b.timestamp))
            .collect();
        db_backups.sort_by_key(|(_, ts)| *ts);
        let to_remove: std::collections::HashSet<_> = db_backups.iter()
...
impl Database {
    ...
    pub fn stats(&self) -> DatabaseStats {
        ...
        let indices = self.storage.active_label_indices.read().unwrap().len()
            + self.storage.active_label_property_indices.read().unwrap().len();
        ...
        DatabaseStats {
            ...
            transaction_count: self.transaction_count.load(std::sync::atomic::Ordering::Relaxed),
    ...
    pub fn check_quota(&self, quota: &DatabaseQuota) -> Result<(), DbmsError> {
        ...
        if let Some(max_v) = quota.max_vertices {
            if stats.vertex_count > max_v {
                return Err(DbmsError::QuotaExceeded(format!(
                    "vertex count {} exceeds quota {}", stats.vertex_count, max_v
                )));
            }
        }
        if let Some(max_e) = quota.max_edges {
            if stats.edge_count > max_e {
                return Err(DbmsError::QuotaExceeded(format!(
                    "edge count {} exceeds quota {}", stats.edge_count, max_e
                )));
            }
        }
        if let Some(max_mem) = quota.max_memory_mb {
            ...
            if mem_mb > max_mem {
                return Err(DbmsError::QuotaExceeded(format!(
                    "memory {}MB exceeds quota {}MB", mem_mb, max_mem
                )));
            }
        }
        Ok(())
...
impl DbmsHandler {
    ...
    pub fn set_state(&self, name: &DatabaseName, state: DatabaseState) -> Result<(), DbmsError> {
        let db = self.get(name).ok_or_else(|| DbmsError::DatabaseNotFound(name.clone()))?;
    ...
    pub fn get_state(&self, name: &DatabaseName) -> Option<DatabaseState> {
        self.get(name).map(|db| *db.state.read().expect("lock poisoned"))
    }
    ...
    pub fn rename(&self, old_name: &DatabaseName, new_name: DatabaseName) -> Result<(), DbmsError> {
        ...
        let db = dbs.remove(old_name).ok_or_else(|| DbmsError::DatabaseNotFound(old_name.clone()))?;
...
impl QueryTracker {
    ...
    pub fn record_query(&self, latency_ms: u64, success: bool) {
        ...
        if !success {
            self.failed_queries.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        }
    }
    ...
    pub fn failed_queries(&self) -> u64 {
        self.failed_queries.load(std::sync::atomic::Ordering::Relaxed)
    }
    ...
    pub fn avg_latency_ms(&self) -> u64 {
        ...
        if total == 0 {
            ...
            self.total_latency_ms.load(std::sync::atomic::Ordering::Relaxed) / total
        }
    }
}
...
impl DatabaseRuntime {
    ...
    pub fn add_user(&self, username: String, role: DatabaseRole) {
        ...
        self.users.write().expect("lock poisoned").insert(username, user);
    }

    pub fn remove_user(&self, username: &str) -> bool {
        self.users.write().expect("lock poisoned").remove(username).is_some()
    }

    pub fn get_user(&self, username: &str) -> Option<DbUser> {
        self.users.read().expect("lock poisoned").get(username).cloned()
    }
...
impl DbmsRuntime {
    ...
    pub fn create_db(&self, name: DatabaseName) -> Result<Arc<DatabaseRuntime>, DbmsError> {
        ...
        self.runtimes.write().expect("lock poisoned").insert(name, rt.clone());
        Ok(rt)
    }
    ...
    pub fn get_runtime(&self, name: &DatabaseName) -> Option<Arc<DatabaseRuntime>> {
        self.runtimes.read().expect("lock poisoned").get(name).cloned()
    }
    ...
    pub fn all_runtimes(&self) -> Vec<(DatabaseName, Arc<DatabaseRuntime>)> {
        self.runtimes.read().expect("lock poisoned")
            .iter()
    ...
    pub fn check_health(&self, name: &DatabaseName) -> Result<DatabaseHealth, DbmsError> {
        let rt = self.get_runtime(name).ok_or_else(|| DbmsError::DatabaseNotFound(name.clone()))?;
...
impl ConnectionPool {
    ...
    pub fn try_acquire(&self) -> bool {
        let current = self.active.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
...
mod tests {
    ...
    fn test_tenant_assignment() {
        let handler = DbmsHandler::new();
        handler.assign_tenant(&"default".into(), "tenant-1".into()).unwrap();
    ...
    fn test_database_state() {
        let handler = DbmsHandler::new();
        assert_eq!(handler.get_state(&"default".into()), Some(DatabaseState::Online));
        handler.set_state(&"default".into(), DatabaseState::Offline).unwrap();
        assert_eq!(handler.get_state(&"default".into()), Some(DatabaseState::Offline));
    }
    ...
    fn test_cannot_rename_default() {
        let handler = DbmsHandler::new();
        assert!(matches!(handler.rename(&"default".into(), "other".into()), Err(DbmsError::CannotRenameDefault)));
    }
