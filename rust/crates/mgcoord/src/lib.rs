...
use raft::{CoordinatorNode, CoordinatorNodeId, TypeConfig, ClusterCommand};
...
impl ClusterState {
    ...
    pub fn get(&self, instance_id: &str) -> Option<Instance> {
        self.instances.read().expect("lock poisoned").get(instance_id).cloned()
    }

    pub fn list(&self) -> Vec<Instance> {
        self.instances.read().expect("lock poisoned").values().cloned().collect()
    }

    pub fn update_health(&self, instance_id: &str, health: InstanceHealth) {
        if let Some(inst) = self.instances.write().expect("lock poisoned").get_mut(instance_id) {
            inst.health = health;
    ...
    pub fn set_route(&self, database: &str, main_addr: SocketAddr) {
        self.routing.write().expect("lock poisoned").insert(database.to_string(), main_addr);
    }

    pub fn get_route(&self, database: &str) -> Option<SocketAddr> {
        self.routing.read().expect("lock poisoned").get(database).copied()
    }

    pub fn remove_route(&self, database: &str) {
        self.routing.write().expect("lock poisoned").remove(database);
    }

    pub fn routes(&self) -> Vec<(String, SocketAddr)> {
        self.routing.read().expect("lock poisoned").iter().map(|(k, v)| (k.clone(), *v)).collect()
    }
    ...
    pub fn check_health(&self, timeout: Duration) -> Vec<String> {
        ...
        for (id, inst) in instances.iter_mut() {
            if inst.health == InstanceHealth::Up && now.duration_since(inst.last_heartbeat) > timeout {
...
impl ReplicationLagTracker {
    pub fn new() -> Self {
        Self { lags: Arc::new(RwLock::new(HashMap::new())) }
    }

    pub fn record_lag(&self, instance_id: &str, lag: Duration) {
        self.lags.write().expect("lock poisoned").insert(instance_id.to_string(), lag);
    }

    pub fn get_lag(&self, instance_id: &str) -> Option<Duration> {
        self.lags.read().expect("lock poisoned").get(instance_id).copied()
    }
    ...
    pub fn has_lagging_replicas(&self, max_lag: Duration) -> Vec<String> {
        self.lags.read().expect("lock poisoned")
            .iter()
...
impl ShardAllocator {
    ...
    pub fn assign(&self, shard: ShardId, instance_id: &str) {
        self.assignments.write().expect("lock poisoned").insert(shard, instance_id.to_string());
    }

    /// Get the instance responsible for a shard.
    pub fn get(&self, shard: ShardId) -> Option<String> {
        self.assignments.read().expect("lock poisoned").get(&shard).cloned()
    }
    ...
    pub fn shards_for(&self, instance_id: &str) -> Vec<ShardId> {
        self.assignments.read().expect("lock poisoned")
            .iter()
...
impl LoadBalancer {
    ...
    pub fn pick_replica(&self, replicas: &[Instance], lag_tracker: Option<&ReplicationLagTracker>) -> Option<String> {
        if replicas.is_empty() { return None; }
        // Filter out lagging replicas if tracker provided
        let candidates: Vec<&Instance> = match lag_tracker {
            Some(lt) => replicas.iter()
                .filter(|r| lt.get_lag(&r.id).map_or(true, |lag| lag.as_secs() < 30))
                .collect(),
            None => replicas.iter().collect(),
        };
        let candidates = if candidates.is_empty() { replicas.iter().collect() } else { candidates };
...
impl Default for LoadBalancer {
    fn default() -> Self { Self::new(LoadBalanceStrategy::RoundRobin) }
}
    ...
    pub fn has_cycle(&self) -> bool {
            ...
            if let Some(targets) = edges.get(node) {
                for t in targets {
                    if !visited.contains(t) {
                        if dfs(t, edges, visited, rec_stack) { return true; }
        ...
        for node in self.edges.keys() {
            if !visited.contains(node) {
                if dfs(node, &self.edges, &mut visited, &mut rec_stack) {
                    return true;
                }
            }
        }
...
impl DataMigration {
    ...
    pub fn create_task(&self, shard: ShardId, from: &str, to: &str) -> String {
        ...
        self.tasks.write().expect("lock poisoned").insert(id.clone(), task);
        id
    }
    ...
    pub fn active_tasks(&self) -> Vec<MigrationTask> {
        self.tasks.read().expect("lock poisoned")
            .values()
...
impl MultiDcRouting {
    ...
    pub fn register_dc(&self, dc: DataCenter) {
        self.dc_map.write().expect("lock poisoned").insert(dc.id.clone(), dc);
    }

    pub fn assign_instance_to_dc(&self, instance_id: &str, dc_id: &str) {
        self.instance_dc.write().expect("lock poisoned").insert(instance_id.to_string(), dc_id.to_string());
    }

    pub fn instance_dc(&self, instance_id: &str) -> Option<String> {
        self.instance_dc.read().expect("lock poisoned").get(instance_id).cloned()
    }

    pub fn instances_in_dc(&self, dc_id: &str) -> Vec<String> {
        self.instance_dc.read().expect("lock poisoned")
            .iter()
    ...
    pub fn local_replicas(&self, replicas: &[Instance], client_dc: &str) -> Vec<Instance> {
        ...
        let local: Vec<Instance> = replicas.iter()
            .filter(|r| inst_dc.get(&r.id).map_or(false, |dc| dc == client_dc))
            .cloned()
            .collect();
        if local.is_empty() {
            replicas.to_vec()
    ...
    pub fn all_dcs(&self) -> Vec<DataCenter> {
        self.dc_map.read().expect("lock poisoned").values().cloned().collect()
    }
}
...
impl MetricsCollector {
    ...
    pub fn record(&self, instance_id: &str, metrics: InstanceMetrics) {
        self.metrics.write().expect("lock poisoned").insert(instance_id.to_string(), metrics);
    }

    pub fn get(&self, instance_id: &str) -> Option<InstanceMetrics> {
        self.metrics.read().expect("lock poisoned").get(instance_id).cloned()
    }
    ...
    pub fn total_active_connections(&self) -> usize {
        self.metrics.read().expect("lock poisoned").values().map(|v| v.active_connections).sum()
    }
...
pub enum ClusterError {
    ...
    RaftInit(openraft::error::RaftError<CoordinatorNodeId, openraft::error::InitializeError<CoordinatorNodeId, CoordinatorNode>>),
    ClientWrite(openraft::error::RaftError<CoordinatorNodeId, openraft::error::ClientWriteError<CoordinatorNodeId, CoordinatorNode>>),
    NotLeader,
    Network(String),
}
...
impl ClusterManager {
    ...
    pub async fn init_single_node(&self) -> Result<(), ClusterError> {
        ...
        self.raft.initialize(nodes).await.map_err(ClusterError::RaftInit)
    }
    ...
    pub async fn add_node(
        ...
        self.raft.add_learner(id, node, false).await.map_err(ClusterError::ClientWrite)
    }
    ...
    pub async fn remove_node(
        ...
        let changes = openraft::ChangeMembers::RemoveNodes(
            std::iter::once(id).collect(),
        );
        self.raft.change_membership(changes, false).await.map_err(ClusterError::ClientWrite)
    }

    /// Get the current leader node ID, if known.
    pub fn get_leader(&self) -> Option<CoordinatorNodeId> {
        let metrics = self.raft.metrics();
        let leader = metrics.borrow().current_leader.clone();
        leader
    }
    ...
    pub fn is_leader(&self) -> bool {
        let metrics = self.raft.metrics();
        let current_leader = metrics.borrow().current_leader.clone();
    ...
    pub async fn propose(
        ...
        self.raft.client_write(cmd).await.map_err(ClusterError::ClientWrite)
    }

    /// Shut down the Raft node gracefully.
    pub async fn shutdown(self) -> Result<(), ClusterError> {
        self.raft.shutdown().await.map_err(|e| ClusterError::Network(format!("shutdown join error: {}", e)))
    }
}
...
mod tests {
    ...
    fn test_bootstrap_cluster() {
        ...
        let config = ClusterConfig::new(
            vec![Instance::new("cfg-1".into(), test_addr(7689), InstanceRole::Replica)],
            "default".into(),
        );
    ...
    fn test_load_balancer_round_robin() {
        ...
        let choices: std::collections::HashSet<String> = [a.unwrap(), b.unwrap()].into_iter().collect();
    ...
    fn test_migration_lifecycle() {
        ...
        dm.start_task(&id);
        assert_eq!(dm.get_task(&id).unwrap().status, MigrationStatus::InProgress);
    ...
    fn test_metrics_collector() {
        ...
        collector.record("i1", InstanceMetrics {
            cpu_percent: 80.0,
            memory_used_mb: 1024,
        ...
        collector.record("i2", InstanceMetrics {
            cpu_percent: 40.0,
            ..Default::default()
        });
