...
fn int_of(result: &QueryResult, col: &str) -> i64 {
    let v = result.rows[0].get(col).unwrap_or_else(|| panic!("missing column {}", col));
    match v {
...
fn int_of_row(result: &QueryResult, row: usize, col: &str) -> i64 {
    let v = result.rows[row].get(col).unwrap_or_else(|| panic!("missing column {}", col));
    match v {
        PropertyValue::Int(n) => *n,
        _ => panic!("expected Int in column {}, got {:?}", col, v),
    }
...
fn workload_set_membership() {
    ...
    let count_res = ctx
        .run("MATCH (n:SetItem) RETURN count(*) AS cnt")
        .unwrap();
    let final_count = int_of(&count_res, "cnt") as usize;

    assert_eq!(
        final_count, all_confirmed.len(),
...
fn workload_register_lww() {
    ...
    for t in 0..threads {
        ...
        handles.push(thread::spawn(move || {
            ...
            for i in 0..ops_per_thread {
                ...
                let q = format!(
                    "MATCH (n:Register) SET n.val = {} RETURN true AS ok",
                    val
                );
                match c.run(&q) {
                    Ok(res) if !res.rows.is_empty() && bool_of_row(&res, 0, "ok") => {
    ...
    if !all_writes.is_empty() {
        assert!(
            ...
            final_val, all_writes
        );
    } else {
        assert_eq!(final_val, 0, "with no successful writes, value should remain 0");
    }
}
...
fn workload_concurrent_create_count() {
    ...
    let res = ctx
        .run("MATCH (n:Item) RETURN count(*) AS cnt")
        .unwrap();
...
fn workload_write_skew_guards() {
    ...
    assert!(
        ...
        active_0, active_1, successes.load(std::sync::atomic::Ordering::Relaxed)
    );
}
...
fn workload_read_your_writes() {
    ...
    for tid in 0..threads {
        ...
        handles.push(thread::spawn(move || {
            ...
            for i in 0..ops_per_thread {
                ...
                match c.run(&format!(
                    "MATCH (n:Ryw {{uid: {}}}) RETURN n.val AS v",
                    uid
                )) {
    ...
    assert_eq!(
        ...
        fcount, threads * ops_per_thread
    );

    // Verify all expected nodes exist
    let count = int_of(
        &ctx.run("MATCH (n:Ryw) RETURN count(*) AS c").unwrap(),
        "c",
    );
    assert_eq!(
        count as usize, threads * ops_per_thread,
        "expected {} Ryw nodes, found {}",
        threads * ops_per_thread, count
    );
}
...
fn workload_monotonic_reads() {
    ...
    let final_val = int_of(
        &ctx.run("MATCH (n:Mono) RETURN n.val AS v").unwrap(),
        "v",
    );
    assert!(
        final_val >= 1,
        "counter should be at least 1 after {} write attempts, got {}",
        writer_ops, final_val
    );
}
...
fn workload_dirty_write_prevention() {
    ...
    let res = ctx
        .run("MATCH (n:Slot {id: 0}) RETURN n.val AS v")
        .unwrap();
...
fn workload_detach_delete_consistency() {
    ...
    for tid in 0..threads {
        ...
        handles.push(thread::spawn(move || {
            ...
            for _ in 0..ops_per_thread {
                ...
                let q = format!(
                    "MATCH (n:Node {{id: {}}}) DETACH DELETE n",
                    target
                );
                let _ = c.run(&q);
            }
    ...
    let remaining_vertices = ctx
        .run("MATCH (n:Node) RETURN count(*) AS cnt")
        .unwrap();
...
fn workload_multi_replica_deterministic() {
    ...
    cluster.run_all(
        "MATCH (a:Node {id: 1}), (b:Node {id: 2}) CREATE (a)-[:LINK {weight: 50}]->(b)",
    );
    ...
    let states: Vec<(i64, i64, i64)> = (0..n_replicas)
        .map(|ri| {
            let vcount = int_of(&cluster.run(ri, "MATCH (n) RETURN count(*) AS c").unwrap(), "c");
            let ecount = int_of(&cluster.run(ri, "MATCH ()-[e]->() RETURN count(*) AS c").unwrap(), "c");
            let ctr = int_of(&cluster.run(ri, "MATCH (c:Counter) RETURN c.val AS v").unwrap(), "v");
            (vcount, ecount, ctr)
        })
        .collect();
...
fn workload_primary_replica_batches() {
    ...
    for batch in 0..n_batches {
        ...
        let p_total = int_of(
            &cluster.run(primary, "MATCH (a:Account) RETURN sum(a.balance) AS s").unwrap(),
            "s",
        );
        for &ri in &replicas {
            let r_total = int_of(
                &cluster.run(ri, "MATCH (a:Account) RETURN sum(a.balance) AS s").unwrap(),
                "s",
            );
    ...
    let check = |ri: usize| -> Vec<i64> {
        let res = cluster.run(ri, "MATCH (a:Account) RETURN a.balance AS b ORDER BY a.id").unwrap();
        (0..res.rows.len()).map(|r| int_of_row(&res, r, "b")).collect()
    };
...
fn workload_multi_replica_with_nemesis() {
        ...
        Arc::new(std::sync::Mutex::new(Vec::new()));
    {
        ...
        for _ in 0..n_mutations {
            let from = lcg(&mut seed) % 5;
            let mut to = lcg(&mut seed) % 5;
            while to == from { to = lcg(&mut seed) % 5; }
            let amount = 1 + (lcg(&mut seed) % 30);
            let q = format!(
    ...
    let p_total = int_of(
        &cluster.run(primary_idx, "MATCH (a:Account) RETURN sum(a.balance) AS s").unwrap(),
        "s",
    );
    ...
    let p_balances: Vec<i64> = {
        let res = cluster.run(primary_idx,
            "MATCH (a:Account) RETURN a.balance AS b ORDER BY a.id").unwrap();
        (0..res.rows.len()).map(|r| int_of_row(&res, r, "b")).collect()
    };
    let r_balances: Vec<i64> = {
        let res = cluster.run(replica_idx,
            "MATCH (a:Account) RETURN a.balance AS b ORDER BY a.id").unwrap();
        (0..res.rows.len()).map(|r| int_of_row(&res, r, "b")).collect()
    };
    ...
    assert_eq!(replayed_cnt.load(std::sync::atomic::Ordering::Relaxed), n_mutations as u64);
}
