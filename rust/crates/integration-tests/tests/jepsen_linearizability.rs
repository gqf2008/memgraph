...
fn test_no_fabricated_values() {
    ...
    for _ in 0..30 {
        ...
        if run_query_cli(main_data.path(), &query)
    ...
    let out = run_query_cli(
        main_data.path(),
        "MATCH (n:Register {id: 0}) RETURN n.val AS v",
    )
...
fn test_monotonic_reads() {
    ...
    for window in reads.windows(2) {
        assert!(window[1] >= window[0], "reads non-monotonic: {:?}", reads);
    }
...
fn test_persistence_across_restart() {
    ...
    let out = run_query_cli(
        main_data.path(),
        "MATCH (n:Persist {id: 0}) RETURN n.val AS v",
    )
    .expect("read failed");
    let stdout = String::from_utf8_lossy(&out.stdout);
