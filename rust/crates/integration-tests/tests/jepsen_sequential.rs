...
fn test_sequential_with_nemesis_kill_restart() {
    ...
    assert_eq!(
        val1, 10,
        "main should see batch1 final value 10, got {}",
        val1
    );
...
fn test_sequential_replica_partition_survival() {
    ...
    // Start replica
    let mut replica_proc =
        spawn_replica(replica_data.path(), "127.0.0.1:10202").expect("spawn replica failed");
    wait_for_startup();
    ...
    assert!(
        replica_proc.is_alive(),
        "replica should be alive after connecting"
    );
    ...
    assert!(
        partitioned.is_alive(),
        "partitioned replica should stay alive"
    );
    ...
    assert!(
        partitioned.is_alive(),
        "partitioned replica should survive main death"
    );
...
fn test_sequential_concurrent_writers() {
    ...
    // Read final value
    let main_out = run_query_cli(
        main_data.path(),
        "MATCH (n:Counter {id: 0}) RETURN n.val AS v",
    )
...
fn test_sequential_main_replica_startup() {
    ...
    // Start replica
    let mut replica_proc =
        spawn_replica(replica_data.path(), "127.0.0.1:10207").expect("spawn replica failed");
    wait_for_startup();
