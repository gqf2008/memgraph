...
fn test_multi_node_main_replica_startup() {
    ...
    let mut main_proc = Command::new(&bin)
        .args([
            "--data-directory",
            main_data.path().to_str().unwrap(),
            "--replication-role",
            "main",
            "--replication-port",
            "10001",
            "--bolt-port",
            "0", // disable bolt to avoid port conflicts
        ])
        .stdout(Stdio::null())
        .stderr(Stdio::null())
        .spawn()
    ...
    let mut replica_proc = Command::new(&bin)
        .args([
            "--data-directory",
            replica_data.path().to_str().unwrap(),
            "--replication-role",
            "replica",
            "--replica-of",
            "127.0.0.1:10001",
...
fn test_multi_node_multiple_replicas() {
    ...
    let mut replica1 = Command::new(&bin)
        .args([
            "--data-directory",
            replica1_data.path().to_str().unwrap(),
            "--replication-role",
            "replica",
            "--replica-of",
            "127.0.0.1:10002",
    ...
    let mut replica2 = Command::new(&bin)
        .args([
            "--data-directory",
            replica2_data.path().to_str().unwrap(),
            "--replication-role",
            "replica",
            "--replica-of",
            "127.0.0.1:10002",
...
fn test_multi_node_failover() {
    ...
    if let Ok(mut check) = check_proc {
        ...
        assert!(
            still_alive,
            "replica data directory should allow restart after main death"
        );
    }
    ...
    assert!(
        !replica_status.success(),
        "replica should have been killed, not crashed"
    );
}
...
fn test_multi_node_write_then_read() {
    ...
    assert!(
        replica_alive,
        "replica should still be alive after receiving deltas"
    );
...
fn test_multi_node_coordinator_startup() {
    ...
    assert!(
        !status.success(),
        "coordinator should have been killed, not crashed"
    );
}
...
fn test_multi_node_replica_promotion() {
    ...
    let mut old_replica = Command::new(&bin)
        .args([
            "--data-directory",
            old_replica_data.path().to_str().unwrap(),
            "--replication-role",
            "replica",
            "--replica-of",
            "127.0.0.1:10013",
    ...
    let mut new_replica = Command::new(&bin)
        .args([
            "--data-directory",
            new_replica_data.path().to_str().unwrap(),
            "--replication-role",
            "replica",
            "--replica-of",
            "127.0.0.1:10013",
    ...
    assert!(
        new_replica_alive,
        "new replica should connect to promoted main"
    );
