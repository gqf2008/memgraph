...
impl<'a> DbAccessor<'a> {
    ...
    pub fn vertex_set_property(
        ...
        self.storage
            .vertex_set_property(&self.tx, gid, key, value)
    }
    ...
    pub fn has_vertex(&self, gid: Gid) -> bool { self.storage.has_vertex(gid) }
    pub fn has_edge(&self, gid: Gid) -> bool { self.storage.has_edge(gid) }
    pub fn edge_from(&self, gid: Gid) -> Option<Gid> { self.storage.edge_from(gid) }
    pub fn edge_to(&self, gid: Gid) -> Option<Gid> { self.storage.edge_to(gid) }
    pub fn vertex_count(&self) -> usize { self.storage.vertex_count() }
    pub fn edge_count(&self) -> usize { self.storage.edge_count() }
    pub fn label_count(&self, label: LabelId) -> usize { self.storage.label_count(label) }
    ...
    pub fn edge_set_property(&self, gid: Gid, key: PropertyId, value: PropertyValue) -> Result<(), StorageError> {
    ...
    pub fn edges_by_type_property(&self, edge_type: EdgeTypeId, prop: PropertyId) -> Vec<(Gid, PropertyValue)> {
        self.storage.edges_by_type_property(edge_type, prop)
    }

    pub fn edges_by_type_property_value(&self, edge_type: EdgeTypeId, prop: PropertyId, value: &PropertyValue) -> Vec<Gid> {
        self.storage.edges_by_type_property_value(edge_type, prop, value)
    }
    ...
    pub fn chunked_vertices_by_label(&self, label: LabelId, num_chunks: usize) -> Vec<Vec<(Gid, Vec<LabelId>, PropertyStore)>> {
    ...
    pub fn chunked_edges_by_type(&self, edge_type: EdgeTypeId, num_chunks: usize) -> Vec<Vec<(Gid, Gid, Gid, EdgeTypeId, PropertyStore)>> {
    ...
    pub fn vertices_by_label_property(&self, label: LabelId, property: PropertyId, value: &PropertyValue) -> Vec<Gid> {
        self.storage.vertices_by_label_property(label, property, value)
    }
    ...
    pub fn point_within_bbox_2d(
        &self,
        label: LabelId, prop: PropertyId,
        lower_left: mgcore::point::Point2D,
        upper_right: mgcore::point::Point2D,
    ) -> Vec<Gid> {
        self.storage.point_index.within_bbox_2d(label, prop, lower_left, upper_right)
    }

    pub fn point_nearest_2d(
        &self,
        label: LabelId, prop: PropertyId,
        query: mgcore::point::Point2D, k: usize,
    ) -> Vec<(Gid, f64)> {
