...
pub fn reciprocity(storage: &Storage) -> f64 {
    ...
    let edge_set: HashSet<(Gid, Gid)> = edges.iter()
...
pub fn average_clustering_coefficient(storage: &Storage) -> f64 {
    ...
    if count == 0 { 0.0 } else { total_cc / count as f64 }
}
...
mod tests {
    ...
    fn test_degree_histogram() {
        ...
        let bucket_1 = hist.iter().find(|(k, _)| *k == 1).map(|(_, v)| *v).unwrap_or(0);
        let bucket_4 = hist.iter().find(|(k, _)| *k == 4).map(|(_, v)| *v).unwrap_or(0);
