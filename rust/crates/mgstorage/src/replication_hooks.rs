    ...
    pub fn wal_to_events(record: &WalRecord) -> Vec<ReplicationEvent> {
        match record {
            WalRecord::VertexCreate { gid, .. } => {
                vec![ReplicationEvent::VertexCreate { gid: *gid }]
            }
            WalRecord::VertexDelete { gid } => vec![ReplicationEvent::VertexDelete { gid: *gid }],
            WalRecord::VertexAddLabel { gid, label } => vec![ReplicationEvent::VertexAddLabel {
                gid: *gid,
                label: *label,
            }],
            WalRecord::VertexRemoveLabel { gid, label } => {
                vec![ReplicationEvent::VertexRemoveLabel {
                    gid: *gid,
                    label: *label,
                }]
            }
            WalRecord::VertexSetProperty { gid, key, value } => {
                vec![ReplicationEvent::VertexSetProperty {
                    gid: *gid,
                    key: *key,
                    value: value.clone(),
                }]
            ...
            WalRecord::EdgeCreate {
                ...
                edge_type,
                ..
            } => vec![ReplicationEvent::EdgeCreate {
                gid: *gid,
                from: *from_vertex,
                to: *to_vertex,
                edge_type: *edge_type,
            ...
            WalRecord::EdgeSetProperty { gid, key, value } => {
                vec![ReplicationEvent::EdgeSetProperty {
                    gid: *gid,
                    key: *key,
                    value: value.clone(),
                }]
...
mod tests {
    ...
    fn test_buffered_hook() {
        ...
        hook.on_event(ReplicationEvent::VertexCreate {
            gid: Gid::from(1u64),
        });
        hook.on_event(ReplicationEvent::VertexCreate {
            gid: Gid::from(2u64),
    ...
    fn test_multicast_hook() {
        ...
        multi.on_event(ReplicationEvent::VertexCreate {
            gid: Gid::from(1u64),
        });
        assert_eq!(a.len(), 1);
        assert_eq!(b.len(), 1);
    ...
    fn test_filtered_hook() {
        ...
        filtered.on_event(ReplicationEvent::VertexCreate {
            gid: Gid::from(1u64),
        });
        filtered.on_event(ReplicationEvent::EdgeDelete {
            gid: Gid::from(100u64),
        });
    ...
    fn test_wal_to_events() {
        let wal = WalRecord::VertexCreate {
            gid: Gid::from(1u64),
            timestamp: 1,
        };
        ...
        assert!(
            matches!(events[0], ReplicationEvent::VertexCreate { gid } if gid == Gid::from(1u64))
        );
    }
}
