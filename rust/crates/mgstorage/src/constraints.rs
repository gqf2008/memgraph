...
pub enum ConstraintError {
    UniqueViolation { label: LabelId, properties: Vec<PropertyId>, existing_gid: Gid },
    ExistenceViolation { label: LabelId, property: PropertyId },
    TypeViolation { label: LabelId, property: PropertyId, expected: ConstraintType, got: String },
}
...
impl ConstraintType {
    ...
    pub fn matches_value(&self, val: &PropertyValue) -> bool {
        if matches!(val, PropertyValue::Null) { return true; }
...
fn format_unique_key(
    ...
    let parts: Vec<String> = constraint.properties.iter()
        .map(|p| {
...
impl Constraints {
    ...
    pub fn add_unique_constraint(&self, label: LabelId, properties: Vec<PropertyId>) {
        ...
        if !guard.iter().any(|c| c.label == label && c.properties == properties) {
    ...
    pub fn add_existence_constraint(&self, label: LabelId, property: PropertyId) {
        ...
        if !guard.iter().any(|c| c.label == label && c.property == property) {
    ...
    pub fn add_type_constraint(&self, label: LabelId, property: PropertyId, expected: ConstraintType) {
        let mut guard = self.type_constraints.write().unwrap();
        if !guard.iter().any(|c| c.label == label && c.property == property) {
            guard.push(TypeConstraintEntry { label, property, expected });
        }
    }
    ...
    pub fn check_type(
        ...
        for constraint in guard.iter() {
            if constraint.label != label || constraint.property != property { continue; }
    ...
    pub fn has_unique_constraint(&self, label: LabelId, properties: &[PropertyId]) -> bool {
        self.unique.read().unwrap().iter().any(|c| c.label == label && c.properties == properties)
    }

    pub fn has_existence_constraint(&self, label: LabelId, property: PropertyId) -> bool {
        self.existence.read().unwrap().iter().any(|c| c.label == label && c.property == property)
    }

    pub fn has_type_constraint(&self, label: LabelId, property: PropertyId) -> bool {
        self.type_constraints.read().unwrap().iter().any(|c| c.label == label && c.property == property)
    }

    pub fn drop_unique_constraint(&self, label: LabelId, properties: &[PropertyId]) {
        self.unique.write().unwrap().retain(|c| !(c.label == label && c.properties == properties));
    }

    pub fn drop_existence_constraint(&self, label: LabelId, property: PropertyId) {
        self.existence.write().unwrap().retain(|c| !(c.label == label && c.property == property));
    }

    pub fn drop_type_constraint(&self, label: LabelId, property: PropertyId) {
        self.type_constraints.write().unwrap().retain(|c| !(c.label == label && c.property == property));
    }
    ...
    pub fn list(&self) -> Vec<ConstraintInfo> {
        ...
        for c in self.unique.read().unwrap().iter() {
            out.push(ConstraintInfo {
                ...
                label: c.label,
                property: c.properties.first().copied().unwrap_or(PropertyId::from(0u32)),
            });
        }
...
impl TtlConfig {
    pub fn new() -> Self {
        Self { ttls: RwLock::new(HashMap::new()) }
    }
...
mod tests {
    ...
    fn test_unique_constraint() {
        ...
        assert!(constraints.check_unique(label, &vals, Gid::from(1u64)).is_ok());
        constraints.record_unique_values(label, &vals, Gid::from(1u64));

        // Same value, different gid → violation
        assert!(constraints.check_unique(label, &vals, Gid::from(2u64)).is_err());

        // Different value → ok
        vals.set(prop, PropertyValue::Int(99));
        assert!(constraints.check_unique(label, &vals, Gid::from(2u64)).is_ok());
    }
    ...
    fn test_type_constraint() {
        ...
        assert!(constraints.check_type(label, prop, &PropertyValue::Int(42)).is_ok());
        assert!(constraints.check_type(label, prop, &PropertyValue::String("hi".into())).is_err());
        assert!(constraints.check_type(label, prop, &PropertyValue::Null).is_ok());
    }
