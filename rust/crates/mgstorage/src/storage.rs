...
use mgcore::delta::{
    apply_deltas_for_read, DeltaAction, DeltaKind, IsolationLevel, View,
...
impl TextIndexEntry {
    pub fn new(index: std::sync::Arc<crate::text_index::TextIndex>, properties: Vec<(PropertyId, String)>) -> Self {
...
pub struct Storage {
    ...
    trigger_executor: std::sync::Mutex<Option<std::sync::Arc<dyn crate::triggers::TriggerExecutor>>>,
...
impl Storage {
    ...
    pub fn list_active_queries(&self) -> Vec<ActiveQueryInfo> {
        self.active_queries.lock().unwrap().values().cloned().collect()
    }
    ...
    pub fn set_trigger_executor(&self, executor: std::sync::Arc<dyn crate::triggers::TriggerExecutor>) {
        *self.trigger_executor.lock().unwrap() = Some(executor);
    }

    /// Fire triggers matching the given event and optional label.
    fn fire_triggers(&self, event: crate::triggers::TriggerEvent, label: Option<LabelId>, gid: Gid) {
    ...
    pub fn commit_transaction(&self, tx: &Transaction) -> bool {
        ...
        let mut _shard_guards: Vec<std::sync::MutexGuard<'_, ()>> = Vec::with_capacity(shard_indices.len());
    ...
    /// Check for write-write conflicts. Returns true if no conflicts detected.
    /// A conflict exists when another transaction committed a delta for a Gid
    /// that this transaction also modified, and that commit happened after
    /// this transaction's start timestamp.
    fn check_write_write_conflicts(&self, tx: &Transaction) -> bool {
        let write_set = tx.take_write_set();
        self.check_write_write_conflicts_with_write_set(tx, &write_set)
    }
    ...
    fn check_write_write_conflicts_with_write_set(
        ...
        use mgcore::delta::TRANSACTION_INITIAL_ID;
    ...
    pub fn gc_with_horizon(&self, oldest_active_tx: u64) -> usize {
        ...
        for (_gid, vertex) in vertices.iter() {
            ...
            if new_head != head { vertex.set_delta(new_head); }
        }
        drop(vertices);
        ...
        for (_gid, edge) in edges.iter() {
            ...
            if head.is_null() { continue; }
            let new_head = unsafe { Self::trim_chain(head, oldest_active_tx) };
            if new_head != head { edge.set_delta(new_head); }
        }
        drop(edges);
        ...
        deltas.retain(|d| {
            ...
            ts >= oldest_active_tx || ts >= TRANSACTION_INITIAL_ID
                || matches!(d.kind, DeltaKind::DeleteObject | DeltaKind::DeleteDeserializedObject { .. })
        });
    ...
    unsafe fn trim_chain(head: *mut CoreDelta, watermark: u64) -> *mut CoreDelta {
        ...
        loop {
            let d = match current.as_ref() { Some(d) => d, None => break };
            let ts = d.commit_info.timestamp();
            ...
            if next.is_null() { return std::ptr::null_mut(); } // entire chain gone
            current = next;
        }
    ...
    pub fn create_vertex(
        ...
        if self.is_on_disk() {
            if let Ok(disk) = self.vertex_disk.lock() {
                if let Some(ref d) = *disk {
                    d.put(gid.as_uint(), vertex.as_ref())
                        .map_err(|e| StorageError::ConstraintViolation(format!("disk error: {}", e)))?;
                }
            }
        }
    ...
    pub fn get_vertex(&self, gid: Gid, tx: &Transaction) -> Option<VertexSnapshot> {
        ...
        if delta.is_null() && !has_ttl && !vertex.deleted() {
            ...
            self.vertex_snapshot_cache.write().unwrap().put(gid, snapshot.clone());
        ...
        apply_deltas_for_read(
            ...
            |d| {
                match &d.kind {
                    ...
                    DeltaKind::Label { action: DeltaAction::AddLabel, value } => {
                    ...
                    DeltaKind::Label { action: DeltaAction::RemoveLabel, value } => {
                        labels.push(*value);
                    }
                    // SET_PROPERTY: undo set → restore old value
                    DeltaKind::SetProperty { key, old_value } => {
                        match old_value {
        ...
        if has_ttl {
            let now_ms = std::time::SystemTime::now()
                ...
                .unwrap_or_default()
                .as_millis() as u64;
            if self.ttl_config.is_expired(&labels, vertex.creation_timestamp, now_ms) {
                return None;
            }
        }
    ...
    pub fn vertex_set_property(
        ...
        let ci = tx.commit_info.clone();
        let cmd_id = tx.next_command_id();
        let delta = Box::new(CoreDelta::new_set_property(key, old_value.clone(), ci, cmd_id));
        let delta_ptr = Box::into_raw(delta);
        self.deltas.write().unwrap().push(unsafe { Box::from_raw(delta_ptr) });

        // Link delta into chain (newest first)
        unsafe {
            (*delta_ptr).next.store(vertex.delta(), std::sync::atomic::Ordering::Release);
        }
    ...
    pub fn vertex_add_label(
        ...
        self.deltas.write().unwrap().push(unsafe {
            Box::from_raw(delta_ptr)
        });
        ...
        for (prop_id, value) in vertex.properties.iter() {
            if active_pi.contains(&(label, prop_id)) {
                match &value {
                    PropertyValue::Point2D(p) => self.point_index.insert_2d(label, prop_id, gid, *p),
                    PropertyValue::Point3D(p) => self.point_index.insert_3d(label, prop_id, gid, *p),
                    _ => {}
                }
            }
        }
    ...
    pub fn create_label_property_index(&self, label: LabelId, property: PropertyId) -> bool {
        let changed = self.active_label_property_indices.write().unwrap().insert((label, property));
        if changed {
            self.bump_schema_generation();
        }
        changed
    ...
    pub fn drop_label_property_index(&self, label: LabelId, property: PropertyId) -> bool {
        let changed = self.active_label_property_indices.write().unwrap().remove(&(label, property));
        if changed {
            self.bump_schema_generation();
        }
        changed
    ...
    pub fn has_label_property_index(&self, label: LabelId, property: PropertyId) -> bool {
        self.active_label_property_indices.read().unwrap().contains(&(label, property))
    }

    pub fn create_point_index(&self, label: LabelId, property: PropertyId) -> bool {
        let changed = self.active_point_indices.write().unwrap().insert((label, property));
        if changed {
            self.bump_schema_generation();
        }
        changed
    ...
    pub fn drop_point_index(&self, label: LabelId, property: PropertyId) -> bool {
        let changed = self.active_point_indices.write().unwrap().remove(&(label, property));
        if changed {
            self.bump_schema_generation();
        }
        changed
    ...
    pub fn has_point_index(&self, label: LabelId, property: PropertyId) -> bool {
        self.active_point_indices.read().unwrap().contains(&(label, property))
    }

    pub fn create_edge_type_index(&self, edge_type: EdgeTypeId) -> bool {
        let changed = self.active_edge_type_indices.write().unwrap().insert(edge_type);
        if changed {
            self.bump_schema_generation();
        }
        changed
    ...
    pub fn drop_edge_type_index(&self, edge_type: EdgeTypeId) -> bool {
        let changed = self.active_edge_type_indices.write().unwrap().remove(&edge_type);
        if changed {
            self.bump_schema_generation();
        }
        changed
    ...
    pub fn has_edge_type_index(&self, edge_type: EdgeTypeId) -> bool {
        self.active_edge_type_indices.read().unwrap().contains(&edge_type)
    }

    pub fn create_edge_type_property_index(&self, edge_type: EdgeTypeId, property: PropertyId) -> bool {
        let changed = self.active_edge_type_property_indices.write().unwrap().insert((edge_type, property));
        if changed {
            self.bump_schema_generation();
        }
        changed
    ...
    pub fn drop_edge_type_property_index(&self, edge_type: EdgeTypeId, property: PropertyId) -> bool {
        let changed = self.active_edge_type_property_indices.write().unwrap().remove(&(edge_type, property));
        if changed {
            self.bump_schema_generation();
        }
        changed
    ...
    pub fn has_edge_type_property_index(&self, edge_type: EdgeTypeId, property: PropertyId) -> bool {
        self.active_edge_type_property_indices.read().unwrap().contains(&(edge_type, property))
    }
    ...
    pub fn vertex_remove_label(
        ...
        self.constraints.remove_unique_values(label, &vertex.properties);
    ...
    pub fn edge_set_property(
        ...
        unsafe { (*delta_ptr).next.store(edge.delta(), std::sync::atomic::Ordering::Release); }
    ...
    pub fn delete_edge(&self, tx: &Transaction, gid: Gid) -> Result<(), StorageError> {
        ...
        let edge = edges
            .get(&gid)
            .ok_or(StorageError::EdgeNotFound(gid))?;
    ...
    pub fn all_vertices(&self) -> Vec<(Gid, Vec<LabelId>, PropertyStore)> {
        ...
        vertices
            ...
            .filter(|(_, v)| !has_ttl || !self.ttl_config.is_expired(&v.labels, v.creation_timestamp, now_ms))
        ...
        edges
            ...
            .filter_map(|(gid, e)| {
                self.edge_index.get(gid).map(|entry| {
                    (*gid, entry.from_vertex, entry.to_vertex, entry.edge_type, e.properties.clone())
                })
            })
            .collect()
    }
    ...
    pub fn all_edges_gids(&self) -> Vec<(Gid, Gid, Gid, EdgeTypeId)> {
        ...
        edges
            ...
            .filter_map(|(gid, _)| {
                self.edge_index.get(gid).map(|entry| {
                    (*gid, entry.from_vertex, entry.to_vertex, entry.edge_type)
                })
            })
            .collect()
    }
    ...
    pub fn all_vertices_in_tx(&self, tx: &Transaction) -> Vec<VertexSnapshot> {
        let gids: Vec<Gid> = {
            let vertices = self.vertices.read().unwrap();
            vertices.iter().map(|(gid, _)| *gid).collect()
        };
        gids.into_iter()
    ...
    pub fn all_edges_in_tx(&self, tx: &Transaction) -> Vec<EdgeSnapshot> {
        let gids: Vec<Gid> = {
            let edges = self.edges.read().unwrap();
            edges.iter().map(|(gid, _)| *gid).collect()
        };
        gids.into_iter()
    ...
    pub fn vertex_in_edges(&self, gid: Gid, edge_type: Option<EdgeTypeId>) -> Vec<(Gid, Gid, EdgeTypeId)> {
        let vertices = self.vertices.read().unwrap();
        let Some(v) = vertices.get(&gid) else { return vec![] };
        v.in_edges.iter()
            .filter(|triple| edge_type.map_or(true, |et| triple.edge_type == et))
            .map(|triple| {
    ...
    pub fn vertex_out_edges(&self, gid: Gid, edge_type: Option<EdgeTypeId>) -> Vec<(Gid, Gid, EdgeTypeId)> {
        let vertices = self.vertices.read().unwrap();
        let Some(v) = vertices.get(&gid) else { return vec![] };
        v.out_edges.iter()
            .filter(|triple| edge_type.map_or(true, |et| triple.edge_type == et))
    ...
    pub fn ttl_cleanup(&self) -> usize {
        ...
        for (gid, v) in vertices.iter() {
            ...
            if self.ttl_config.is_expired(&v.labels, v.creation_timestamp, now_ms) {
                expired.push(*gid);
            }
        }
        drop(vertices);
            ...
            if let Some(v) = vertices.get_mut(gid) {
                ...
                for label in &v.labels {
                    for (prop_id, _) in v.properties.iter() {
                        if active_lp.contains(&(*label, prop_id)) {
                            self.label_property_index.remove(
                                mgcore::types::LabelPropKey::new(*label, prop_id), *gid);
                        }
                    }
                }
                drop(active_lp);
    ...
    pub fn has_expired_ttl(&self) -> bool {
        ...
        vertices.iter().any(|(_, v)| {
            !v.deleted() && self.ttl_config.is_expired(&v.labels, v.creation_timestamp, now_ms)
        })
    }
    ...
    pub fn check_constraints_for_vertex(&self, gid: Gid) -> Result<(), StorageError> {
        let vertices = self.vertices.read().unwrap();
        let v = vertices.get(&gid).ok_or(StorageError::VertexNotFound(gid))?;
        for label in &v.labels {
            self.constraints.check_existence(*label, &v.properties)
                .map_err(|e| StorageError::ConstraintViolation(format!("{:?}", e)))?;
            self.constraints.check_unique(*label, &v.properties, *(&gid))
                .map_err(|e| StorageError::ConstraintViolation(format!("{:?}", e)))?;
            for (key, value) in v.properties.iter() {
                self.constraints.check_type(*label, key, value)
                    .map_err(|e| StorageError::ConstraintViolation(format!("{:?}", e)))?;
            }
        }
        Ok(())
    ...
    pub fn has_vertex(&self, gid: Gid) -> bool {
        self.vertices.read().unwrap().get(&gid).map_or(false, |v| !v.deleted())
    }

    pub fn has_edge(&self, gid: Gid) -> bool {
        self.edges.read().unwrap().get(&gid).map_or(false, |e| !e.deleted())
    }
    ...
    pub fn delete_vertex_and_edges(
        ...
        let incident_edges: Vec<Gid> = {
            let vertices = self.vertices.read().unwrap();
            if let Some(v) = vertices.get(&gid) {
                v.in_edges.iter().chain(v.out_edges.iter())
                    .map(|t| t.edge.gid())
                    .collect()
            } else {
    ...
    pub fn edges_by_property_value(
        &self,
        prop: PropertyId,
        value: &PropertyValue,
    ) -> Vec<Gid> {
    ...
    pub fn chunked_vertices(&self, num_chunks: usize) -> Vec<Vec<(Gid, Vec<LabelId>, PropertyStore)>> {
        let all = self.all_vertices();
        let chunk_size = (all.len() + num_chunks - 1) / num_chunks;
    ...
    pub fn chunked_edges(&self, num_chunks: usize) -> Vec<Vec<(Gid, Gid, Gid, EdgeTypeId, PropertyStore)>> {
    ...
    pub fn chunked_edges_by_type(
        ...
        let items: Vec<_> = gids.iter()
            .filter_map(|gid| {
                ...
                Some((*gid, entry.from_vertex, entry.to_vertex, entry.edge_type, e.properties.clone()))
            })
            .collect();
        let chunk_size = (items.len() + num_chunks - 1) / num_chunks;
        let mut chunks = Vec::with_capacity(num_chunks);
...
mod tests {
    ...
    fn test_label_property_index_maintained() {
        ...
        storage.vertex_set_property(&tx, gid, prop, PropertyValue::Int(99)).unwrap();
    ...
    fn test_ttl_different_labels() {
        ...
        storage.vertex_add_label(&tx, gid2, label_permanent).unwrap();
    ...
    fn test_delta_gc() {
        ...
        let gid = Gid::from(1u64);
        storage.create_vertex(&tx, gid).unwrap();
        storage.vertex_add_label(&tx, gid, LabelId::from(10u32)).unwrap();
        storage.commit_transaction(&tx);
    ...
    fn test_existence_constraint_enforced_on_write() {
        ...
        storage.create_vertex(&tx, gid).unwrap();
        storage.vertex_set_property(&tx, gid, key, PropertyValue::Int(42)).unwrap();
        storage.vertex_add_label(&tx, gid, label).unwrap();
        ...
        let err = storage
            ...
            .unwrap_err();
        assert!(matches!(err, StorageError::ConstraintViolation(_)), "expected constraint violation, got {:?}", err);
    }
    ...
    fn test_type_constraint_enforced_on_write() {
        ...
        storage.constraints.add_type_constraint(label, key, crate::constraints::ConstraintType::Int);
    ...
    fn test_unique_constraint_enforced_on_write() {
        ...
        let gid1 = Gid::from(1u64);
        storage.create_vertex(&tx, gid1).unwrap();
        storage.vertex_set_property(&tx, gid1, key, PropertyValue::Int(42)).unwrap();
        storage.vertex_add_label(&tx, gid1, label).unwrap();

        let gid2 = Gid::from(2u64);
        storage.create_vertex(&tx, gid2).unwrap();
        storage.vertex_set_property(&tx, gid2, key, PropertyValue::Int(99)).unwrap();
        ...
        storage.vertex_set_property(&tx, gid2, key, PropertyValue::Int(77)).unwrap();
    }
    ...
    fn test_unique_constraint_old_value_removed_on_update() {
        ...
        storage.vertex_set_property(&tx, gid1, key, PropertyValue::Int(99)).unwrap();

        // Another vertex should now be able to use 42
        let gid2 = Gid::from(2u64);
        storage.create_vertex(&tx, gid2).unwrap();
        storage.vertex_set_property(&tx, gid2, key, PropertyValue::Int(42)).unwrap();
        storage.vertex_add_label(&tx, gid2, label).unwrap();
    ...
    fn test_detach_delete_vertex_and_edges() {
        ...
        storage.create_vertex(&tx, from_gid).unwrap();
        storage.create_vertex(&tx, to_gid).unwrap();
        storage.create_edge(&tx, edge_gid, from_gid, to_gid, edge_type).unwrap();
        storage.commit_transaction(&tx);
    ...
    fn test_edge_property_index() {
        ...
        storage.create_vertex(&tx, to_gid).unwrap();
        storage.create_edge(&tx, edge_gid, from_gid, to_gid, edge_type).unwrap();
        storage.edge_set_property(&tx, edge_gid, weight_prop, PropertyValue::Double(3.14)).unwrap();
    ...
    fn test_edge_property_index_cleaned_on_delete() {
        ...
        storage.create_vertex(&tx, from_gid).unwrap();
        storage.create_vertex(&tx, to_gid).unwrap();
        storage.create_edge(&tx, edge_gid, from_gid, to_gid, EdgeTypeId::from(5u32)).unwrap();
        storage.edge_set_property(&tx, edge_gid, prop, PropertyValue::Int(42)).unwrap();

        assert!(storage.edges_by_property_value(prop, &PropertyValue::Int(42)).len() == 1);

        storage.delete_edge(&tx, edge_gid).unwrap();
        assert!(storage.edges_by_property_value(prop, &PropertyValue::Int(42)).is_empty());
    }
    ...
    fn test_schema_info_tracking() {
        ...
        storage.create_vertex(&tx, Gid::from(1u64)).unwrap();
        storage.vertex_add_label(&tx, Gid::from(1u64), label).unwrap();
        storage.vertex_set_property(&tx, Gid::from(1u64), prop, PropertyValue::Int(42)).unwrap();

        let from_gid = Gid::from(1u64);
        let to_gid = Gid::from(2u64);
        storage.create_vertex(&tx, to_gid).unwrap();
        storage.create_edge(&tx, Gid::from(100u64), from_gid, to_gid, etype).unwrap();
        storage.edge_set_property(&tx, Gid::from(100u64), prop, PropertyValue::String("hi".into())).unwrap();
    ...
    fn test_vertex_incident_edge_count() {
        ...
        // a → b, a → c, b → a
        storage.create_edge(&tx, Gid::from(100u64), a, b, EdgeTypeId::from(0u32)).unwrap();
        storage.create_edge(&tx, Gid::from(101u64), a, c, EdgeTypeId::from(0u32)).unwrap();
        storage.create_edge(&tx, Gid::from(102u64), b, a, EdgeTypeId::from(0u32)).unwrap();
    ...
    fn test_edges_by_type_property_value() {
        ...
        storage.create_edge(&tx, Gid::from(100u64), from, to, etype).unwrap();
        storage.edge_set_property(&tx, Gid::from(100u64), prop, PropertyValue::Int(42)).unwrap();
    ...
    fn test_on_disk_vertex_crud() {
        ...
        let gid = Gid::from(1u64);
        storage.create_vertex(&tx, gid).unwrap();
        storage.vertex_set_property(&tx, gid, PropertyId::from(0u32), PropertyValue::Int(42)).unwrap();
        storage.commit_transaction(&tx);

        let tx2 = storage.begin_transaction(IsolationLevel::SnapshotIsolation);
        let snap = storage.get_vertex(gid, &tx2).unwrap();
        assert_eq!(*snap.properties.get(PropertyId::from(0u32)), PropertyValue::Int(42));
    ...
    fn test_on_disk_edge_crud() {
        ...
        storage.create_vertex(&tx, from).unwrap();
        storage.create_vertex(&tx, to).unwrap();
        storage.create_edge(&tx, eid, from, to, EdgeTypeId::from(5u32)).unwrap();
        storage.commit_transaction(&tx);
    ...
    fn test_on_disk_label_and_property() {
        ...
        storage.create_vertex(&tx, gid).unwrap();
        storage.vertex_add_label(&tx, gid, LabelId::from(10u32)).unwrap();
        storage.vertex_set_property(&tx, gid, PropertyId::from(0u32), PropertyValue::Int(99)).unwrap();
        storage.commit_transaction(&tx);

        let tx2 = storage.begin_transaction(IsolationLevel::SnapshotIsolation);
        let snap = storage.get_vertex(gid, &tx2).unwrap();
        assert!(snap.labels.contains(&LabelId::from(10u32)));
        assert_eq!(snap.properties.get(PropertyId::from(0u32)).clone(), PropertyValue::Int(99));

        let tx3 = storage.begin_transaction(IsolationLevel::SnapshotIsolation);
        storage.vertex_remove_label(&tx3, gid, LabelId::from(10u32)).unwrap();
    ...
    fn test_on_disk_edge_property_crud() {
        ...
        storage.create_vertex(&tx, to).unwrap();
        storage.create_edge(&tx, eid, from, to, EdgeTypeId::from(5u32)).unwrap();
        storage.edge_set_property(&tx, eid, PropertyId::from(0u32), PropertyValue::String("friend".into())).unwrap();
        storage.commit_transaction(&tx);

        let tx2 = storage.begin_transaction(IsolationLevel::SnapshotIsolation);
        let edge = storage.get_edge(eid, &tx2).unwrap();
        assert_eq!(edge.properties.get(PropertyId::from(0u32)).clone(), PropertyValue::String("friend".into()));
    ...
    fn test_gc_with_horizon() {
        ...
        let before = storage.deltas.read().unwrap().len();
        assert!(before >= 2, "should have at least 2 deltas (create + add_label)");
        ...
        assert!(collected_max > 0, "expected some deltas collected with max horizon");
    ...
    fn test_text_index_auto_maintenance() {
        ...
        let entry = TextIndexEntry::new(
            ...
            vec![(prop_name, "name".into())],
        );
        storage.text_indices.write().unwrap().insert(label, entry);

        // Set a text property — should auto-index
        storage.vertex_set_property(&tx, gid, prop_name, PropertyValue::String("Alice in Wonderland".into())).unwrap();
        ...
        storage.vertex_set_property(&tx, gid, prop_name, PropertyValue::String("Bob the Builder".into())).unwrap();
        let text_indices = storage.text_indices.read().unwrap();
        let entry = text_indices.get(&label).unwrap();
        let results = entry.index.search("Wonderland", 10).unwrap();
    ...
    fn test_text_index_mimic_integration() {
        ...
        storage.create_vertex(&tx, gid).unwrap();
        storage.vertex_add_label(&tx, gid, label).unwrap();
        storage.vertex_set_property(&tx, gid, prop_name, PropertyValue::String("Hello World".into())).unwrap();
        ...
        storage.vertex_set_property(&tx, gid, prop_name, PropertyValue::String("Goodbye World".into())).unwrap();

        // Search for old value
        let text_indices = storage.text_indices.read().unwrap();
        let entry = text_indices.get(&label).unwrap();
        let results = entry.index.search("Hello", 10).unwrap();
        assert_eq!(results.len(), 0, "Should NOT find Hello after update, got {} results", results.len());
    ...
    fn test_point_index_auto_maintenance() {
        ...
        storage.vertex_set_property(&tx, gid, prop_2d, PropertyValue::Point2D(p2d)).unwrap();

        let in_box = storage.point_index.within_bbox_2d(label, prop_2d,
        ...
        storage.vertex_set_property(&tx, gid, prop_3d, PropertyValue::Point3D(p3d)).unwrap();

        let nearest = storage.point_index.nearest_2d(label, prop_2d,
            Point2D::new(Crs::Cartesian2D, 10.0, 20.0), 1);
        ...
        storage.vertex_set_property(&tx, gid, prop_2d, PropertyValue::Point2D(p2d_new)).unwrap();

        let in_box_old = storage.point_index.within_bbox_2d(label, prop_2d,
            Point2D::new(Crs::Cartesian2D, 5.0, 15.0),
            Point2D::new(Crs::Cartesian2D, 15.0, 25.0));
        assert_eq!(in_box_old.len(), 0);

        let in_box_new = storage.point_index.within_bbox_2d(label, prop_2d,
        ...
        storage.vertex_remove_label(&tx, gid, label).unwrap();
        let in_box_after = storage.point_index.within_bbox_2d(label, prop_2d,
        ...
        let in_box_re = storage.point_index.within_bbox_2d(label, prop_2d,
            Point2D::new(Crs::Cartesian2D, 5.0, 15.0),
            Point2D::new(Crs::Cartesian2D, 15.0, 25.0));
        ...
        storage.delete_vertex(&tx, gid).unwrap();
        let in_box_del = storage.point_index.within_bbox_2d(label, prop_2d,
        ...
        let threads: Vec<_> = (0..4)
            .map(|t| {
                ...
                std::thread::spawn(move || {
                    for i in 0..25 {
                        let tx = s.begin_transaction(IsolationLevel::SnapshotIsolation);
                        s.vertex_set_property(&tx, gid, prop, PropertyValue::Int(t * 25 + i as i64)).unwrap();
                        s.commit_transaction(&tx);
                    }
                })
            })
    ...
    fn test_concurrent_label_property_index_updates() {
        ...
        for i in 0..50 {
            ...
            storage.create_vertex(&tx, gid).unwrap();
            storage.vertex_add_label(&tx, gid, label).unwrap();
            storage.vertex_set_property(&tx, gid, prop, PropertyValue::Int(i as i64)).unwrap();
            storage.commit_transaction(&tx);
        }
            ...
            .map(|t| {
                ...
                std::thread::spawn(move || {
                    for i in 0..50 {
                        let gid = Gid::from(i as u64 + 1);
                        let tx = s.begin_transaction(IsolationLevel::SnapshotIsolation);
                        s.vertex_set_property(&tx, gid, prop, PropertyValue::Int((t * 50 + i) as i64)).unwrap();
                        s.commit_transaction(&tx);
                    }
                })
            })
    ...
    fn test_constrained_concurrent_unique_violation() {
        ...
        let t1 = std::thread::spawn(move || {
            ...
            s1.vertex_set_property(&tx, Gid::from(1u64), prop, PropertyValue::Int(42)).unwrap();
    ...
    fn test_write_write_conflict_detection() {
        ...
        // tx3 starts, modifies the vertex, and commits first
        {
            let tx3 = storage.begin_transaction(IsolationLevel::SnapshotIsolation);
            let prop = PropertyId::from(0u32);
            storage.vertex_set_property(&tx3, gid, prop, PropertyValue::Int(100)).unwrap();
            assert!(storage.commit_transaction(&tx3));
        }

        // tx2 now modifies the same vertex
        let prop = PropertyId::from(0u32);
        storage.vertex_set_property(&tx2, gid, prop, PropertyValue::Int(200)).unwrap();
    ...
    fn test_no_conflict_on_different_gids() {
        ...
        let tx2 = storage.begin_transaction(IsolationLevel::SnapshotIsolation);
        storage.vertex_set_property(&tx2, Gid::from(1u64), PropertyId::from(0u32), PropertyValue::Int(10)).unwrap();

        // tx3 modifies vertex 2 and commits
        {
            let tx3 = storage.begin_transaction(IsolationLevel::SnapshotIsolation);
            storage.vertex_set_property(&tx3, Gid::from(2u64), PropertyId::from(0u32), PropertyValue::Int(20)).unwrap();
            assert!(storage.commit_transaction(&tx3));
        }
    ...
    fn test_concurrent_create_unique_gids() {
        ...
        assert_eq!(total, threads * per_thread,
            "commits failed: expected {}, got {} committed", threads * per_thread, total);
        assert_eq!(count, threads * per_thread,
            "vertex count mismatch: expected {}, got {} in storage", threads * per_thread, count);
    }
        ...
        for t in 0..threads {
            ...
            handles.push(std::thread::spawn(move || {
                ...
                for i in 0..per_thread {
                    ...
                    let tx = s.begin_transaction(IsolationLevel::SnapshotIsolation);
                    s.create_vertex(&tx, gid).unwrap();
                    s.vertex_set_property(&tx, gid, PropertyId::from(0u32), PropertyValue::Int((t * per_thread + i) as i64)).unwrap();
    ...
    fn test_sharded_commit_disjoint_gids() {
        ...
        let total: usize = handles.into_iter().map(|h| h.join().unwrap()).sum();
        assert_eq!(total, threads * per_thread,
            "expected all {} commits to succeed, got {}", threads * per_thread, total);

        let tx = storage.begin_transaction(IsolationLevel::SnapshotIsolation);
        let count = storage.all_vertices().len();
        drop(tx);
        assert_eq!(count, threads * per_thread,
            "expected {} vertices, got {}", threads * per_thread, count);
    }
    ...
    fn test_sharded_commit_same_gid_serializes() {
        ...
        for t in 0..threads {
            ...
            handles.push(std::thread::spawn(move || {
                ...
                for i in 0..per_thread {
                    ...
                    s.vertex_set_property(&tx, gid, prop, PropertyValue::Int(current + 1)).unwrap();
                    if s.commit_transaction(&tx) {
                        ok += 1;
                    }
                }
        ...
        assert!(
            ...
            "expected some commits to succeed, got {}", total_ok
        );
        ...
        assert_eq!(final_val, total_ok as i64,
            "final value ({}) should equal successful commits ({})", final_val, total_ok);
    }
}
