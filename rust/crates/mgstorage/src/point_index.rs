...
impl PointIndex {
    ...
    pub fn insert_2d(&self, label: LabelId, prop: PropertyId, gid: Gid, point: Point2D) {
        ...
        entries.push(PointEntry { gid, point_2d: Some(point), point_3d: None });
    }
    ...
    pub fn insert_3d(&self, label: LabelId, prop: PropertyId, gid: Gid, point: Point3D) {
        ...
        let entries = map.entry(key).or_default();
        entries.retain(|e| e.gid != gid);
        entries.push(PointEntry { gid, point_2d: None, point_3d: Some(point) });
    }
    ...
    pub fn within_bbox_2d(&self, label: LabelId, prop: PropertyId,
        lower_left: Point2D, upper_right: Point2D) -> Vec<Gid> {
        let key = (label, prop);
        let map = self.entries.read().unwrap();
        map.get(&key).map(|entries| {
            entries.iter()
                .filter(|e| {
                    if let Some(p) = &e.point_2d {
                        p.x >= lower_left.x && p.x <= upper_right.x
    ...
    pub fn nearest_2d(&self, label: LabelId, prop: PropertyId,
        query: Point2D, k: usize) -> Vec<(Gid, f64)> {
        let key = (label, prop);
        let map = self.entries.read().unwrap();
        let results: Vec<(Gid, f64)> = map.get(&key).map(|entries| {
            let mut dists: Vec<(Gid, f64)> = entries.iter()
                .filter_map(|e| e.point_2d.as_ref().map(|p| {
                    let dx = p.x - query.x;
                    let dy = p.y - query.y;
                    (e.gid, (dx * dx + dy * dy) as f64)
                }))
                .collect();
...
mod tests {
    ...
    fn test_insert_and_search_2d() {
        ...
        let in_box = idx.within_bbox_2d(label, prop,
    ...
    fn test_remove() {
        ...
        let label = LabelId::from(1u32);
        let prop = PropertyId::from(0u32);
        idx.insert_2d(label, prop, Gid::from(1u64), Point2D::new(Crs::WGS84, 10.0, 20.0));
