...
impl SchemaInfo {
    ...
    pub fn record_vertex_property(&self, label: LabelId, prop: PropertyId) {
        self.vertex_props.write().unwrap().entry(label).or_default().insert(prop);
    }

    pub fn record_edge_property(&self, etype: EdgeTypeId, prop: PropertyId) {
        self.edge_props.write().unwrap().entry(etype).or_default().insert(prop);
    }
    ...
    pub fn label_properties(&self, label: LabelId) -> HashSet<PropertyId> {
        self.vertex_props.read().unwrap().get(&label).cloned().unwrap_or_default()
    }

    pub fn edge_type_properties(&self, etype: EdgeTypeId) -> HashSet<PropertyId> {
        self.edge_props.read().unwrap().get(&etype).cloned().unwrap_or_default()
    }
