...
pub fn import_vertices_csv(
    ...
    let headers = header_line.as_deref().map(parse_csv_line).unwrap_or_default();

    let mut count = 0usize;
    for line in lines {
        let line = line.map_err(|e| StorageError::ConstraintViolation(format!("csv record: {}", e)))?;
...
pub fn import_edges_csv(
    ...
    let from_col = headers.iter().position(|h| h.eq_ignore_ascii_case("from"))
        .unwrap_or(0);
    let to_col = headers.iter().position(|h| h.eq_ignore_ascii_case("to"))
        .unwrap_or(1);
...
impl StreamingBulkLoader {
    ...
    pub fn queue_edge(
        &mut self, gid: Gid, from: Gid, to: Gid, edge_type: EdgeTypeId) {
...
mod tests {
    ...
    fn test_parse_property_value() {
        ...
        assert_eq!(parse_property_value("hello"), PropertyValue::String("hello".into()));
    ...
    fn test_streaming_bulk_loader() {
        ...
        let mut props = HashMap::new();
        props.insert(PropertyId::from(0u32), PropertyValue::String("Alice".into()));
        ...
        loader.queue_edge(Gid::from(100u64), Gid::from(1u64), Gid::from(2u64), EdgeTypeId::from(0u32));
