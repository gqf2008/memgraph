...
/// The backing Vec is wrapped in an Arc so that cloning a PropertyStore
/// is cheap (refcount bump) instead of allocating a new Vec. Mutating
/// methods use copy-on-write via Arc::make_mut.
...
impl PropertyStore {
    ...
    pub fn set(&mut self, key: PropertyId, value: PropertyValue) {
        ...
        if idx >= storage.len() {
            storage.resize(idx + 1, PropertyValue::Null);
        }
        storage[idx] = value;
    }
    ...
    pub fn remove(&mut self, key: PropertyId) {
        let idx = key.as_uint() as usize;
        if idx < self.storage.len() {
            Arc::make_mut(&mut self.storage)[idx] = PropertyValue::Null;
        }
    }
    ...
    pub fn compact(&mut self) {
        let storage = Arc::make_mut(&mut self.storage);
        let last_set = storage.iter().rposition(|v| !v.is_null());
        match last_set {
            Some(idx) => storage.truncate(idx + 1),
            None => storage.clear(),
        }
    ...
    /// Returns the number of strong references to the underlying storage.
    /// Useful for debugging copy-on-write behavior.
...
mod tests {
    ...
    fn test_property_store_growth() {
        ...
        assert_eq!(
            store.get(PropertyId::from_uint(100)),
            &PropertyValue::Int(-1)
        );
    ...
    fn test_property_store_estimate_size() {
        ...
        assert!(
            size >= std::mem::size_of::<PropertyStore>()
                + store.storage.capacity() * std::mem::size_of::<PropertyValue>()
        );
    }
}
