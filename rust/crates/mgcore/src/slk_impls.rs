        ...
        match tag {
            ...
            6 => {
                ...
                for _ in 0..size {
                    entries.push((
                        String::slk_load(reader)?,
                        PropertyValue::slk_load(reader)?,
                    ));
                }
                Ok(PropertyValue::Map(entries))
            }
            7 => Ok(PropertyValue::Vertex(crate::property_value::VertexRef::new(
                Gid::slk_load(reader)?,
                Vec::new(),
                crate::property_store::PropertyStore::default(),
            ))),
            8 => Ok(PropertyValue::Edge(crate::property_value::EdgeRefValue::new(
                Gid::slk_load(reader)?,
            ...
            9 => {
                ...
                Ok(PropertyValue::Path(crate::property_value::PathValue { vertices, edges }))
            }
            10 => Ok(PropertyValue::Date(Date::slk_load(reader)?)),
            11 => Ok(PropertyValue::LocalTime(LocalTime::slk_load(reader)?)),
            12 => Ok(PropertyValue::LocalDateTime(LocalDateTime::slk_load(reader)?)),
            13 => Ok(PropertyValue::Duration(Duration::slk_load(reader)?)),
            14 => Ok(PropertyValue::ZonedDateTime(ZonedDateTime::slk_load(reader)?)),
...
impl SlkSave for crate::property_store::PropertyStore {
    fn slk_save(&self, builder: &mut Builder) {
        let items: Vec<(PropertyId, PropertyValue)> = self
            .iter()
            .map(|(k, v)| (k, v.clone()))
            .collect();
...
impl SlkLoad for crate::vertex::Vertex {
    fn slk_load(reader: &mut Reader) -> Result<Self, SlkDecodeError> {
        ...
        let vertex = crate::vertex::Vertex::new(gid, std::ptr::null_mut(), creation_timestamp);
        // labels and properties are public, so we can set them directly
        // But Vertex::new creates a new vertex; we need to modify the fields.
        // Since fields are pub, we use unsafe to reconstruct.
        unsafe {
            let v = std::ptr::addr_of!(vertex).read();
            let mut v = std::mem::ManuallyDrop::new(v);
            let raw = std::ptr::addr_of_mut!(*v);
            (*raw).labels = labels;
            (*raw).properties = properties;
            if deleted {
                (*raw).set_deleted(true);
            }
            Ok(raw.read())
        }
    }
...
impl SlkLoad for crate::edge::Edge {
    fn slk_load(reader: &mut Reader) -> Result<Self, SlkDecodeError> {
        ...
        let properties = crate::property_store::PropertyStore::slk_load(reader)?;
        let deleted = bool::slk_load(reader)?;
        let edge = crate::edge::Edge::new(gid, std::ptr::null_mut());
        unsafe {
            let e = std::ptr::addr_of!(edge).read();
            let mut e = std::mem::ManuallyDrop::new(e);
            let raw = std::ptr::addr_of_mut!(*e);
            (*raw).properties = properties;
            if deleted {
                (*raw).set_deleted(true);
            }
