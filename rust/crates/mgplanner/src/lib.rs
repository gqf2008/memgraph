...
#[derive(Clone, Debug)]
pub enum LogicalOp {
    AllScan { alias: Option<String> },
    LabelScan { alias: Option<String>, label: LabelId },
    LabelPropertyScan { alias: Option<String>, label: LabelId, property: PropertyId, value: PropertyValue },
    EdgeTypeScan { alias: Option<String>, edge_type: EdgeTypeId },
    EdgeTypePropertyScan { alias: Option<String>, edge_type: EdgeTypeId, property: PropertyId, value: PropertyValue },
    EdgeExpand { direction: Direction, edge_type: Option<EdgeTypeId> },
    Filter { condition: Expression },
    Project { expressions: Vec<Expression> },
    Join { left: Box<LogicalPlan>, right: Box<LogicalPlan>, join_type: JoinType },
    Sort { key: Vec<OrderByItem> },
    Limit { count: usize },
    Skip { count: usize },
    Unwind { expr: Expression, alias: String },
    Produce { items: Vec<ReturnItem> },
    CreateVertex { labels: Vec<LabelId>, properties: Vec<(PropertyId, Expression)> },
    SetProperty { key: PropertyId, value: Expression },
...
impl LogicalPlan {
    fn fmt_indented(&self, f: &mut fmt::Formatter<'_>, indent: usize) -> fmt::Result {
        ...
        match &self.op {
            ...
            LogicalOp::EdgeExpand { direction, edge_type } => writeln!(f, "{}EdgeExpand(dir={:?}, type={:?}) (cost={:.2}, rows={:.0})", prefix, direction, edge_type, self.cost.total(), self.cardinality),
...
impl PlanCost {
    pub fn total(&self) -> f64 { self.cpu + self.io }
}
...
impl CostModel {
    ...
    pub fn hash_aggregate_cost(&self, input_card: f64) -> f64 {
        input_card * (self.hash_aggregate_build_cost_per_row + self.hash_aggregate_probe_cost_per_row)
    }
...
impl PlanStats {
    pub fn from_storage(storage: &Storage) -> Self {
        ...
        for (label, prop) in active_lp.iter() {
            let count = storage.label_property_index.vertex_count_by_label_property(*label, *prop);
        ...
        Self { vertex_count, edge_count, label_counts, label_property_counts, edge_type_counts, edge_type_property_counts }
    }
}
...
impl CatalogStats {
    ...
    pub fn from_plan_stats(stats: &PlanStats) -> Self {
        ...
        for (label, count) in &stats.label_counts {
            tables.insert(*label, TableStats { row_count: *count, ..Default::default() });
        }
        let mut edge_types = HashMap::new();
        for (et, count) in &stats.edge_type_counts {
            edge_types.insert(*et, EdgeTypeStats { edge_count: *count, ..Default::default() });
        }
        Self { tables, edge_types, total_vertices: stats.vertex_count }
    }
    ...
    pub fn label_cardinality(&self, label: LabelId) -> f64 {
        self.tables.get(&label).map(|t| t.row_count as f64).unwrap_or(1.0)
    }

    /// Estimate the number of rows for an edge type scan.
    pub fn edge_type_cardinality(&self, edge_type: EdgeTypeId) -> f64 {
        self.edge_types.get(&edge_type).map(|s| s.edge_count as f64).unwrap_or(1.0)
    }
...
pub fn estimate_selectivity(filter_expr: &Expression, catalog_stats: &CatalogStats) -> f64 {
    match filter_expr {
        ...
        Expression::Lt(lhs, rhs) | Expression::Gt(lhs, rhs) | Expression::Lte(lhs, rhs) | Expression::Gte(lhs, rhs) => {
        ...
        Expression::StartsWith(_, _) | Expression::EndsWith(_, _) | Expression::Contains(_, _) | Expression::RegexMatch(_, _) => 0.05,
...
pub fn estimate_cardinality(op: &LogicalOp, catalog_stats: &CatalogStats) -> f64 {
    match op {
        ...
        LogicalOp::EdgeTypeScan { edge_type, .. } => catalog_stats.edge_type_cardinality(*edge_type),
        ...
        LogicalOp::Distinct => 1.0, // placeholder, caller scales
        LogicalOp::Limit { count } => *count as f64,
        LogicalOp::Skip { .. } => 1.0, // placeholder
...
#[derive(Clone, Debug)]
pub enum PhysicalOp {
    SeqScan { alias: Option<String>, label: Option<LabelId> },
    IndexScan {
        alias: Option<String>,
        label: LabelId,
        property: PropertyId,
...
pub fn physical_plan_from_logical(logical: &LogicalPlan, cm: &CostModel, catalog_stats: &CatalogStats) -> PhysicalPlan {
    physical_plan_from_logical_recursive(logical, cm, catalog_stats)
}

fn physical_plan_from_logical_recursive(logical: &LogicalPlan, cm: &CostModel, catalog_stats: &CatalogStats) -> PhysicalPlan {
    match &logical.op {
        LogicalOp::AllScan { alias } => PhysicalPlan {
            op: PhysicalOp::SeqScan { alias: alias.clone(), label: None },
            cost: PlanCost { cpu: cm.scan_all_cost, io: 0.0 },
            cardinality: catalog_stats.total_vertices.max(1) as f64,
        },
        LogicalOp::LabelScan { alias, label } => PhysicalPlan {
            op: PhysicalOp::SeqScan { alias: alias.clone(), label: Some(*label) },
            cost: PlanCost { cpu: cm.scan_label_cost, io: 0.0 },
            cardinality: catalog_stats.label_cardinality(*label),
        },
        LogicalOp::LabelPropertyScan { alias, label, property, value } => {
            ...
            PhysicalPlan {
                ...
                cost: PlanCost { cpu: cm.index_seek_cost_per_row * card, io: 0.0 },
                cardinality: card.max(1.0),
            }
        }
        LogicalOp::EdgeTypeScan { alias, edge_type } => PhysicalPlan {
            op: PhysicalOp::EdgeTypeScan { alias: alias.clone(), edge_type: *edge_type },
            cost: PlanCost { cpu: cm.scan_label_cost, io: 0.0 },
            cardinality: catalog_stats.edge_type_cardinality(*edge_type),
        },
        LogicalOp::EdgeTypePropertyScan { alias, edge_type, property, value } => {
        ...
        LogicalOp::Filter { condition } => {
            ...
            PhysicalPlan {
                ...
                cost: PlanCost { cpu: card * cm.filter_cost_per_row, io: 0.0 },
            ...
            match &right.op {
                ...
                LogicalOp::Sort { key } => {
                    ...
                    return PhysicalPlan {
                        ...
                        cost: PlanCost { cpu: sort_cost, io: 0.0 },
                        cardinality: left_card,
                    };
                }
                ...
                LogicalOp::TopN { key, count } => {
                    let topn_cost = left_card * cm.sort_cost_factor * (*count as f64).log2().max(1.0);
                    return PhysicalPlan {
                        op: PhysicalOp::TopN {
                            key: key.clone(),
                            count: *count,
                        ...
                        cost: PlanCost { cpu: topn_cost, io: 0.0 },
                        cardinality: (*count as f64).min(left_card),
                    };
                }
                LogicalOp::Filter { condition } => {
        ...
        LogicalOp::Limit { count } => {
            ...
            if let PhysicalOp::Sort { key, child: sort_child } = child_plan.op {
        ...
        LogicalOp::Aggregate { group_by, aggregates } => {
            ...
            PhysicalPlan {
                op,
                cost: PlanCost { cpu: agg_cost, io: 0.0 },
        ...
        LogicalOp::Distinct => {
            ...
            PhysicalPlan {
                op: PhysicalOp::HashAggregate {
                    group_by: vec![], // distinct = group-by all columns (simplified)
                    aggregates: vec![],
                    child: Box::new(child),
                },
                cost: PlanCost { cpu: child_card * cm.distinct_cost_per_row, io: 0.0 },
        ...
        LogicalOp::Union { left, right, all } => {
            ...
            PhysicalPlan {
                ...
                cost: PlanCost { cpu: cm.union_overhead, io: 0.0 },
                cardinality: card,
            }
        }
        LogicalOp::Produce { items } => {
        ...
        LogicalOp::EmptyResult => PhysicalPlan {
            op: PhysicalOp::SeqScan { alias: None, label: None },
...
fn choose_join_physical(left: PhysicalPlan, right: PhysicalPlan, join_type: &JoinType, cm: &CostModel) -> PhysicalPlan {
    ...
    let (op, cost) = match join_type {
        JoinType::HashJoin { left_key, right_key } => {
...
pub fn optimize_query(logical_plan: LogicalPlan, cm: &CostModel, catalog_stats: &CatalogStats) -> PhysicalPlan {
    ...
    match plan.op {
        LogicalOp::Join { left, right, join_type } => {
            ...
            if can_swap && right.cardinality < left.cardinality {
                LogicalPlan {
                    op: LogicalOp::Join { left: Box::new(right), right: Box::new(left), join_type },
                    cost: PlanCost { cpu: card + cm.nested_loop_join_cost, io: 0.0 },
                    cardinality: card,
                }
            } else {
                LogicalPlan {
                    op: LogicalOp::Join { left: Box::new(left), right: Box::new(right), join_type },
                    cost: PlanCost { cpu: card + cm.nested_loop_join_cost, io: 0.0 },
                    cardinality: card,
                }
            }
        ...
        LogicalOp::Union { left, right, all } => {
            ...
            LogicalPlan {
                op: LogicalOp::Union { left: Box::new(left), right: Box::new(right), all },
                cost: PlanCost { cpu: plan.cost.cpu + cm.union_overhead, io: 0.0 },
...
fn extract_property_equality(expr: &Expression) -> Option<(&str, PropertyId, PropertyValue)> {
    match expr {
        Expression::Eq(lhs, rhs) => {
            if let Some(r) = try_property_literal(&**lhs, &**rhs) { return Some(r); }
            try_property_literal(&**rhs, &**lhs)
        }
        Expression::And(lhs, rhs) => {
            if let Some(result) = extract_property_equality(lhs) { return Some(result); }
...
fn try_property_literal<'a>(prop_expr: &'a Expression, lit_expr: &Expression) -> Option<(&'a str, PropertyId, PropertyValue)> {
...
fn extract_specific_property_equality(
    ...
    match expr {
        ...
        Expression::And(lhs, rhs) => {
            if let Some(result) = extract_specific_property_equality(lhs, target_alias, target_property) {
                return Some(result);
            }
...
pub fn plan_query(storage: &Storage, query: &Query) -> LogicalPlan {
    ...
    // Try cache first
    {
        ...
        if cache_guard.is_none() {
            *cache_guard = Some(lru::LruCache::new(std::num::NonZeroUsize::new(1024).unwrap()));
        }
...
fn ast_to_logical(query: &Query, stats: &PlanStats) -> LogicalPlan {
    ...
    for clause in &query.clauses {
        let clause_plan = match clause {
            Clause::Match { pattern, where_clause } => plan_match(pattern, where_clause, stats, &query.index_hints),
            Clause::Create { pattern } => {
                let mut children = Vec::new();
                for element in &pattern.elements {
                    let props: Vec<_> = element.node.properties.iter().map(|(k, v)| (*k, v.clone())).collect();
                    children.push(leaf(LogicalOp::CreateVertex {
                        labels: element.node.labels.clone(), properties: props,
                    }, stats.vertex_count as f64));
                }
                chain_children(children)
            }
            Clause::Return { items, distinct: _, all: _ } => leaf(LogicalOp::Produce { items: items.clone() }, 1.0),
            Clause::With { items, where_clause } => {
                let card = current.as_ref().map(|p| p.cardinality).unwrap_or(1.0);
                let mut child = leaf(
                    LogicalOp::Project { expressions: items.iter().map(|i| i.expression.clone()).collect() },
                    card,
                );
                if let Some(expr) = where_clause {
                    let filter_card = child.cardinality * 0.3;
                    child = chain(child, leaf(LogicalOp::Filter { condition: expr.clone() }, filter_card));
                }
                child
            }
            Clause::Set { items } => {
                ...
                for item in items {
                    if let SetItem::Property { key, value, .. } = item {
                        children.push(leaf(LogicalOp::SetProperty { key: *key, value: value.clone() }, 1.0));
                    }
                }
                chain_children(children)
            }
            Clause::Delete { expressions, detach } => {
                leaf(LogicalOp::Delete { expressions: expressions.clone(), detach: *detach }, 1.0)
            }
            ...
            Clause::Unwind { expression, alias } => {
                leaf(LogicalOp::Unwind { expr: expression.clone(), alias: alias.clone() }, 10.0)
            }
            _ => continue,
        };
    ...
    current.unwrap_or_else(|| LogicalPlan { op: LogicalOp::AllScan { alias: None }, cost: PlanCost::default(), cardinality: 0.0 })
}

fn plan_match(pattern: &MatchPattern, where_clause: &Option<Expression>, stats: &PlanStats, index_hints: &[IndexHint]) -> LogicalPlan {
    ...
    for element in &pattern.elements {
        ...
        let mut scan = if let Some(label) = node.labels.first() {
            let mut plan = leaf(LogicalOp::LabelScan { alias: node.alias.clone(), label: *label }, node_card);
            ...
            if let Some(ref wc) = where_clause {
                if let Some((alias, prop, value)) = extract_property_equality(wc) {
                    if node.alias.as_deref() == Some(alias) {
                        let lp_count = stats.label_property_counts.get(&(*label, prop)).copied().unwrap_or(0);
                        if lp_count > 0 {
                            plan = leaf(
                                LogicalOp::LabelPropertyScan {
                                    alias: node.alias.clone(), label: *label, property: prop, value,
                                },
                                lp_count as f64,
                            );
                        }
                ...
                if let Some(hint_prop) = hint.property {
                    ...
                    if let Some(ref wc) = where_clause {
                        if let Some(alias) = node.alias.as_deref() {
                            if let Some(value) = extract_specific_property_equality(wc, alias, hint_prop) {
                                let lp_count = stats.label_property_counts.get(&(*label, hint_prop)).copied().unwrap_or(0);
                                plan = leaf(
                                    LogicalOp::LabelPropertyScan {
                                        alias: node.alias.clone(), label: *label, property: hint_prop, value,
                                    },
            ...
            leaf(LogicalOp::AllScan { alias: node.alias.clone() }, node_card)
        };

        for (edge, _right_node) in &element.edges {
            let edge_plan = if edge.edge_types.len() == 1 {
                let etype = edge.edge_types[0];
                // Check for edge type + property equality in WHERE clause
                if let Some(ref wc) = where_clause {
                    if let Some((alias, prop, value)) = extract_edge_property_equality(wc) {
                        if edge.alias.as_deref() == Some(alias) {
                            let etp_count = stats.edge_type_property_counts.get(&(etype, prop)).copied().unwrap_or(0);
                            if etp_count > 0 {
                                leaf(
                                    LogicalOp::EdgeTypePropertyScan {
                                        alias: edge.alias.clone(), edge_type: etype, property: prop, value,
                                    },
                                    etp_count as f64,
                                )
                            } else {
                                leaf(
                                    LogicalOp::EdgeTypeScan { alias: edge.alias.clone(), edge_type: etype },
                                    stats.edge_type_counts.get(&etype).copied().unwrap_or(edge_count_fallback) as f64,
                                )
                            }
                        } else {
                            leaf(
                ...
                leaf(
                    LogicalOp::EdgeExpand { direction: edge.direction, edge_type: edge.edge_types.first().copied() },
                    node_card * 5.0,
                )
            };
    ...
    let mut match_plan = match element_plans.len() {
        0 => leaf(LogicalOp::AllScan { alias: None }, stats.vertex_count as f64),
        ...
        _ => {
            ...
            iter.next().map(|first| {
                iter.fold(first, |left, right| {
                    ...
                    LogicalPlan {
                        op: LogicalOp::Join {
                            left: Box::new(left), right: Box::new(right), join_type: JoinType::NestedLoop,
                        },
                        cost: PlanCost { cpu: 10.0 + join_card, io: 0.0 },
    ...
    if let Some(ref wc) = where_clause {
        ...
        match_plan = chain(match_plan, leaf(LogicalOp::Filter { condition: wc.clone() }, filter_card));
    }
...
fn leaf(op: LogicalOp, cardinality: f64) -> LogicalPlan {
    ...
    LogicalPlan { op, cost: PlanCost { cpu: cost, io: 0.0 }, cardinality: card }
}

fn chain(left: LogicalPlan, right: LogicalPlan) -> LogicalPlan {
    let cardinality = left.cardinality.max(right.cardinality);
    LogicalPlan {
        op: LogicalOp::Join { left: Box::new(left), right: Box::new(right), join_type: JoinType::NestedLoop },
        cost: PlanCost { cpu: 1.0, io: 0.0 },
        cardinality,
    }
}
...
fn chain_children(children: Vec<LogicalPlan>) -> LogicalPlan {
    ...
    match iter.next() {
        ...
        None => LogicalPlan { op: LogicalOp::AllScan { alias: None }, cost: PlanCost::default(), cardinality: 0.0 },
    }
}
...
fn estimate_cost(op: &LogicalOp, input_card: f64, cm: &CostModel) -> (f64, f64) {
    match op {
        ...
        LogicalOp::LabelPropertyScan { .. } => (cm.scan_label_property_cost, input_card * cm.equality_selectivity),
        LogicalOp::EdgeTypeScan { .. } => (cm.scan_label_cost, input_card * cm.label_selectivity),
        LogicalOp::EdgeTypePropertyScan { .. } => (cm.scan_label_property_cost, input_card * cm.equality_selectivity),
        LogicalOp::Filter { condition } => {
            let sel = estimate_selectivity_legacy(condition, cm);
            (input_card * cm.filter_cost_per_row, input_card * sel)
        }
        LogicalOp::Sort { .. } => (input_card * cm.sort_cost_factor * input_card.log2().max(1.0), input_card),
        ...
        LogicalOp::Aggregate { group_by, .. } => {
            let out_card = if group_by.is_empty() { 1.0 } else { input_card * 0.1 };
...
fn estimate_selectivity_legacy(expr: &Expression, cm: &CostModel) -> f64 {
    match expr {
        ...
        Expression::Lt(_, _) | Expression::Gt(_, _) | Expression::Lte(_, _) | Expression::Gte(_, _) => cm.range_selectivity,
        Expression::And(lhs, rhs) => {
        ...
        Expression::StartsWith(_, _) | Expression::EndsWith(_, _) | Expression::Contains(_, _) | Expression::RegexMatch(_, _) => cm.like_selectivity,
        ...
        LogicalOp::Join { left, right, join_type } => {
            ...
            if let LogicalOp::Limit { count } = &left.op {
                ...
                if let LogicalOp::Filter { condition } = &right.op {
                    ...
                    let new_right = pushdown_limit(
                        LogicalPlan {
                            op: LogicalOp::Filter { condition: condition.clone() },
                            cost: right.cost.clone(),
                            cardinality: right.cardinality,
                        },
                        _cm,
                ...
                if let LogicalOp::Project { expressions } = &right.op {
                    ...
                    let new_right = pushdown_limit(
                        LogicalPlan {
                            op: LogicalOp::Project { expressions: expressions.clone() },
                            cost: right.cost.clone(),
                            cardinality: right.cardinality,
                        },
                        _cm,
...
fn collect_identifiers(expr: &Expression, out: &mut Vec<String>) {
    match expr {
        ...
        Expression::Lte(lhs, rhs) | Expression::Gte(lhs, rhs) |
        Expression::Add(lhs, rhs) | Expression::Sub(lhs, rhs) |
        Expression::Mul(lhs, rhs) | Expression::Div(lhs, rhs) |
        Expression::Mod(lhs, rhs) | Expression::In(lhs, rhs) |
        Expression::StartsWith(lhs, rhs) | Expression::EndsWith(lhs, rhs) |
        Expression::Contains(lhs, rhs) | Expression::RegexMatch(lhs, rhs) => {
            collect_identifiers(lhs, out);
            collect_identifiers(rhs, out);
        }
        Expression::Not(inner) | Expression::Neg(inner) | Expression::IsNull(inner) | Expression::IsNotNull(inner) => {
...
fn remove_redundant_filters(plan: LogicalPlan) -> LogicalPlan {
    match plan.op {
        LogicalOp::Join { left, right, join_type } => {
            ...
            if let LogicalOp::Filter { condition: Expression::Bool(true) } = &left.op {
                let right = remove_redundant_filters(*right);
                return LogicalPlan {
                    op: LogicalOp::Join { left: left.clone(), right: Box::new(right), join_type },
                    cost: plan.cost.clone(),
                    cardinality: plan.cardinality,
                };
            }
...
fn merge_filters(plan: LogicalPlan) -> LogicalPlan {
    match plan.op {
        LogicalOp::Join { left, right, join_type } => {
            ...
            if let LogicalOp::Filter { condition: right_cond } = &right.op {
                let left = merge_filters(*left);
                if let LogicalOp::Join { left: inner_left, right: inner_right, join_type: inner_join_type } = &left.op {
                    if let LogicalOp::Filter { condition: left_cond } = &inner_right.op {
                        // Collapse: Join(Join(base, Filter(A)), Filter(B)) => Join(base, Filter(And(A, B)))
                        let merged_filter = LogicalPlan {
                            op: LogicalOp::Filter {
                                condition: Expression::And(Box::new(left_cond.clone()), Box::new(right_cond.clone())),
                            },
                            cost: PlanCost { cpu: left.cost.cpu + right.cost.cpu, io: 0.0 },
                ...
                if let LogicalOp::Filter { condition: left_cond } = &left.op {
                    let merged_filter = LogicalPlan {
                        op: LogicalOp::Filter {
                            condition: Expression::And(Box::new(left_cond.clone()), Box::new(right_cond.clone())),
                        },
    ...
    match expr {
        ...
        Expression::Div(lhs, rhs) => {
            ...
            match (&l, &r) {
                ...
                (Expression::Double(a), Expression::Double(b)) if *b != 0.0 => Expression::Double(a / b),
                (Expression::Int(a), Expression::Double(b)) if *b != 0.0 => Expression::Double(*a as f64 / b),
                (Expression::Double(a), Expression::Int(b)) if *b != 0 => Expression::Double(a / *b as f64),
        ...
        Expression::IsNull(inner) => {
            ...
            match &i {
                ...
                Expression::Bool(_) | Expression::Int(_) | Expression::Double(_) | Expression::String(_) => Expression::Bool(false),
        ...
        Expression::IsNotNull(inner) => {
            ...
            match &i {
                ...
                Expression::Bool(_) | Expression::Int(_) | Expression::Double(_) | Expression::String(_) => Expression::Bool(true),
        ...
        Expression::Map(entries) => Expression::Map(
            entries.iter().map(|(k, v)| (k.clone(), fold_expression(v))).collect(),
        ),
        // For other expressions, recurse into children where applicable.
        Expression::Property { object, key } => {
            let o = fold_expression(object);
            Expression::Property { object: Box::new(o), key: *key }
        }
        Expression::Label { object, label } => {
            let o = fold_expression(object);
            Expression::Label { object: Box::new(o), label: *label }
        }
        Expression::Function { name, arguments, distinct } => {
            let args = arguments.iter().map(fold_expression).collect();
            Expression::Function { name: name.clone(), arguments: args, distinct: *distinct }
        }
        Expression::Case { expression, whens, else_branch } => Expression::Case {
            expression: expression.as_ref().map(|e| Box::new(fold_expression(e))),
            whens: whens.iter().map(|(w, t)| (fold_expression(w), fold_expression(t))).collect(),
...
fn remove_true_filters(plan: LogicalPlan) -> LogicalPlan {
    match plan.op {
        LogicalOp::Join { left, right, join_type } => {
            ...
            if let LogicalOp::Filter { condition: Expression::Bool(true) } = &right.op {
                return left;
            }
...
fn index_intersection(plan: LogicalPlan, stats: &PlanStats) -> LogicalPlan {
    match plan.op {
        LogicalOp::Join { left, right, join_type } => {
            ...
            let l_info = if let LogicalOp::LabelPropertyScan { alias, label, property, .. } = &left.op {
                Some((alias.clone(), *label, *property))
            } else { None };
            let r_info = if let LogicalOp::LabelPropertyScan { alias, label, property, .. } = &right.op {
                Some((alias.clone(), *label, *property))
            } else { None };
            if let (Some((l_alias, l_label, l_prop)), Some((r_alias, r_label, r_prop))) = (l_info, r_info) {
                if l_alias == r_alias && l_label == r_label {
                    let l_count = stats.label_property_counts.get(&(l_label, l_prop)).copied().unwrap_or(1) as f64;
                    let r_count = stats.label_property_counts.get(&(r_label, r_prop)).copied().unwrap_or(1) as f64;
                    let intersect_card = (l_count * r_count / stats.vertex_count.max(1) as f64).max(1.0);
                    return LogicalPlan {
                        op: LogicalOp::Join {
                            left: Box::new(left),
                            right: Box::new(right),
                            join_type: JoinType::HashJoin { left_key: l_alias.unwrap_or_default(), right_key: r_alias.unwrap_or_default() },
                        },
                        cost: PlanCost { cpu: intersect_card + 50.0, io: 0.0 },
...
fn explain_plan_recursive(plan: &LogicalPlan, buf: &mut String, depth: usize, is_last: bool) {
    ...
    buf.push_str(&format!(
        "{}{}  [cost={:.1}, card={:.1}]\n",
        indent, op_str, plan.cost.total(), plan.cardinality
    ));
...
fn format_op(op: &LogicalOp) -> String {
    match op {
        ...
        LogicalOp::LabelScan { alias, label } => format!("LabelScan(alias={:?}, label={})", alias, label.as_uint()),
        LogicalOp::LabelPropertyScan { alias, label, property, value } => {
            format!("LabelPropertyScan(alias={:?}, label={}, prop={}, value={:?})", alias, label.as_uint(), property.as_uint(), value)
        }
        LogicalOp::EdgeTypeScan { alias, edge_type } => {
            format!("EdgeTypeScan(alias={:?}, type={})", alias, edge_type.as_uint())
        }
        LogicalOp::EdgeTypePropertyScan { alias, edge_type, property, value } => {
            format!("EdgeTypePropertyScan(alias={:?}, type={}, prop={}, val={:?})", alias, edge_type.as_uint(), property.as_uint(), value)
        }
        LogicalOp::EdgeExpand { direction, edge_type } => {
            format!("EdgeExpand(dir={:?}, type={:?})", direction, edge_type.map(|e| e.as_uint()))
        }
        ...
        LogicalOp::CreateVertex { labels, properties } => format!("CreateVertex(labels={}, props={})", labels.len(), properties.len()),
        LogicalOp::SetProperty { key, .. } => format!("SetProperty({})", key.as_uint()),
        LogicalOp::Delete { expressions, detach } => format!("Delete({} exprs, detach={})", expressions.len(), detach),
        LogicalOp::Aggregate { group_by, aggregates } => format!("Aggregate(group_by={}, aggs={})", group_by.len(), aggregates.len()),
...
fn plan_children(plan: &LogicalPlan) -> Vec<&LogicalPlan> {
    match &plan.op {
        LogicalOp::Join { left, right, .. } => vec![left, right],
        LogicalOp::Union { left, right, .. } => vec![left, right],
        LogicalOp::Filter { .. } | LogicalOp::Project { .. } | LogicalOp::Sort { .. }
        | LogicalOp::Limit { .. } | LogicalOp::Skip { .. } | LogicalOp::Aggregate { .. }
        | LogicalOp::Distinct | LogicalOp::EdgeExpand { .. } | LogicalOp::Unwind { .. }
        | LogicalOp::TopN { .. }
        | LogicalOp::Produce { .. } | LogicalOp::SetProperty { .. } | LogicalOp::Delete { .. }
        | LogicalOp::CreateVertex { .. }
        | LogicalOp::EdgeTypeScan { .. } | LogicalOp::EdgeTypePropertyScan { .. }
...
impl PlanProfile {
    ...
    pub fn cardinality_error(&self) -> Option<f64> {
        self.actual_rows.map(|a| (a as f64) / self.estimated_rows.max(1.0))
    }
}
...
mod tests {
    ...
    fn test_plan_label_scan() {
        ...
        storage.vertex_add_label(&tx, Gid::from(1u64), LabelId::from(10u32)).unwrap();
    ...
    fn test_estimate_filter_cost() {
        ...
        let (cost, card) = estimate_cost(
            &LogicalOp::Filter { condition: Expression::Bool(true) },
            1000.0, &cm,
        );
        assert!(cost > 0.0);
        ...
        let (_cost2, card2) = estimate_cost(
            &LogicalOp::Filter { condition: Expression::Bool(false) },
            1000.0, &cm,
        );
        assert_eq!(card2, 0.0);
        ...
        let (_cost3, card3) = estimate_cost(
            &LogicalOp::Filter { condition: Expression::Eq(Box::new(Expression::Identifier("n".into())), Box::new(Expression::Int(1))) },
            1000.0, &cm,
        );
        assert!(card3 < 1000.0);
    }
    ...
    fn test_aggregate_operator() {
        ...
        let plan = LogicalPlan {
            op: LogicalOp::Aggregate {
                group_by: vec![Expression::Identifier("a".into())],
    ...
    fn test_union_operator() {
        ...
        let (cost_all, card_all) = estimate_cost(
            &LogicalOp::Union {
                left: Box::new(LogicalPlan { op: LogicalOp::AllScan { alias: None }, cost: PlanCost::default(), cardinality: 100.0 }),
                right: Box::new(LogicalPlan { op: LogicalOp::AllScan { alias: None }, cost: PlanCost::default(), cardinality: 100.0 }),
                all: true,
            },
            200.0, &cm,
        );
        ...
        let plan = LogicalPlan {
            op: LogicalOp::Join {
                left: Box::new(LogicalPlan {
                    op: LogicalOp::Filter { condition: Expression::Bool(true) },
                    cost: PlanCost::default(),
                    cardinality: 10.0,
                }),
    ...
    fn test_selectivity_estimation() {
        let cm = CostModel::default();
        assert_eq!(estimate_selectivity_legacy(&Expression::Bool(true), &cm), 1.0);
        assert_eq!(estimate_selectivity_legacy(&Expression::Bool(false), &cm), 0.0);
        assert_eq!(estimate_selectivity_legacy(&Expression::Eq(Box::new(Expression::Int(1)), Box::new(Expression::Int(2))), &cm), cm.equality_selectivity);
        assert_eq!(estimate_selectivity_legacy(&Expression::Lt(Box::new(Expression::Int(1)), Box::new(Expression::Int(2))), &cm), cm.range_selectivity);

        // And selectivity = product
        let and_expr = Expression::And(
            Box::new(Expression::Eq(Box::new(Expression::Int(1)), Box::new(Expression::Int(2)))),
            Box::new(Expression::Eq(Box::new(Expression::Int(3)), Box::new(Expression::Int(4)))),
        );
        assert_eq!(estimate_selectivity_legacy(&and_expr, &cm), cm.equality_selectivity * cm.equality_selectivity);
    ...
    fn test_explain_plan() {
        let plan = LogicalPlan {
            op: LogicalOp::Join {
                left: Box::new(LogicalPlan {
                    op: LogicalOp::LabelScan { alias: Some("n".into()), label: LabelId::from(1u32) },
                    cost: PlanCost { cpu: 100.0, io: 0.0 },
                    cardinality: 10.0,
                }),
                right: Box::new(LogicalPlan {
                    op: LogicalOp::Filter {
            ...
            cost: PlanCost { cpu: 105.0, io: 0.0 },
            cardinality: 10.0,
        };
    ...
    fn test_filter_references_preserved() {
        let condition = Expression::Eq(
            Box::new(Expression::Property { object: Box::new(Expression::Identifier("n".into())), key: PropertyId::from(1u32) }),
    ...
    fn test_physical_plan_filter() {
        ...
        let logical = LogicalPlan {
            op: LogicalOp::Filter {
                condition: Expression::Eq(Box::new(Expression::Identifier("n".into())), Box::new(Expression::Int(1))),
            },
            cost: PlanCost::default(),
            cardinality: 100.0,
        };
    ...
    fn test_physical_plan_aggregate_prefers_streaming_when_sorted() {
        ...
        let sorted_child = PhysicalPlan {
            op: PhysicalOp::Sort {
                key: vec![OrderByItem { expression: Expression::Identifier("a".into()), ascending: true }],
    ...
    fn test_hash_aggregate_cost_formula() {
        ...
        let expected = 500.0 * (cm.hash_aggregate_build_cost_per_row + cm.hash_aggregate_probe_cost_per_row);
        assert_eq!(cost, expected);
    }
    ...
    fn test_estimate_cardinality_with_catalog() {
        ...
        let scan = LogicalOp::LabelScan { alias: Some("n".into()), label };
    ...
    fn test_egraph_rewrite_rules_compile() {
        ...
        assert!(rules.iter().any(|r| r.name().to_string().contains("push-filter")));
        assert!(rules.iter().any(|r| r.name().to_string().contains("join-commute")));
        assert!(rules.iter().any(|r| r.name().to_string().contains("limit-pushdown")));
        assert!(rules.iter().any(|r| r.name().to_string().contains("distinct-idempotent")));
    }
    ...
    fn test_plan_edge_type_scan() {
        ...
        storage.create_edge(&tx, Gid::from(100u64), alice, bob, knows).unwrap();
        storage.commit_transaction(&tx);
        ...
        assert!(plan_str.contains("EdgeTypeScan") || plan_str.contains("EdgeExpand"), "plan: {}", plan_str);
    }
    ...
    fn test_plan_edge_type_property_scan() {
        ...
        storage.create_edge(&tx, Gid::from(100u64), alice, bob, knows).unwrap();
        storage.edge_set_property(&tx, Gid::from(100u64), since, mgcore::property_value::PropertyValue::Int(2010)).unwrap();
        storage.commit_transaction(&tx);
        ...
        let plan = plan_query(&storage, &query);
        let plan_str = format!("{:?}", plan.op);
        assert!(plan_str.contains("EdgeTypePropertyScan") || plan_str.contains("EdgeTypeScan") || plan_str.contains("EdgeExpand"), "plan: {}", plan_str);
    }
    ...
    fn test_edge_type_scan_cardinality() {
        ...
        catalog.edge_types.insert(etype, EdgeTypeStats { edge_count: 250, distinct_source_vertices: 100, distinct_target_vertices: 100 });
        let scan = LogicalOp::EdgeTypeScan { alias: Some("r".into()), edge_type: etype };
        let card = estimate_cardinality(&scan, &catalog);
    ...
    fn test_physical_plan_edge_type_scan() {
        ...
        catalog.total_vertices = 1000;
        let logical = LogicalPlan {
            op: LogicalOp::EdgeTypeScan { alias: Some("r".into()), edge_type: etype },
            cost: PlanCost::default(),
            cardinality: 50.0,
        };
        let physical = physical_plan_from_logical(&logical, &cm, &catalog);
    ...
    fn test_physical_plan_edge_type_property_scan() {
        ...
        let physical = physical_plan_from_logical(&logical, &cm, &catalog);
        assert!(matches!(physical.op, PhysicalOp::EdgeTypePropertyScan { .. }));
    }
    ...
    fn test_constant_fold_nested_arithmetic() {
        let expr = Expression::Mul(
            Box::new(Expression::Add(Box::new(Expression::Int(1)), Box::new(Expression::Int(2)))),
    ...
    fn test_constant_fold_logical() {
        let expr = Expression::And(Box::new(Expression::Bool(true)), Box::new(Expression::Bool(false)));
        let folded = fold_expression(&expr);
        assert_eq!(folded, Expression::Bool(false));

        let expr2 = Expression::Or(Box::new(Expression::Bool(false)), Box::new(Expression::Bool(true)));
    ...
    fn test_constant_fold_comparison() {
        ...
        let expr3 = Expression::Gte(Box::new(Expression::String("abc".into())), Box::new(Expression::String("abc".into())));
    ...
    fn test_constant_fold_mixed_with_variables() {
        ...
        assert_eq!(
            folded,
            Expression::Add(Box::new(Expression::Int(3)), Box::new(Expression::Identifier("n".into())))
        );
    }
        ...
        let plan = LogicalPlan {
            op: LogicalOp::Filter {
                condition: Expression::And(
                    ...
                    Box::new(Expression::Eq(Box::new(Expression::Int(1)), Box::new(Expression::Int(1)))),
                ),
            },
            cost: PlanCost::default(),
            cardinality: 100.0,
    ...
    fn test_simplify_trivial_filters_false_to_empty() {
        let plan = LogicalPlan {
            op: LogicalOp::Join {
                ...
                right: Box::new(LogicalPlan {
                    op: LogicalOp::Filter { condition: Expression::Bool(false) },
                    cost: PlanCost::default(),
                    cardinality: 100.0,
                }),
                join_type: JoinType::NestedLoop,
    ...
    fn test_simplify_trivial_filters_null_to_empty() {
        let plan = LogicalPlan {
            op: LogicalOp::Filter { condition: Expression::Null },
            cost: PlanCost::default(),
            cardinality: 100.0,
        };
        let simplified = simplify_trivial_filters(plan);
    ...
    fn test_pushdown_limit_through_project() {
        ...
        let plan = LogicalPlan {
            op: LogicalOp::Join {
                ...
                right: Box::new(LogicalPlan {
                    op: LogicalOp::Project { expressions: vec![Expression::Identifier("n".into())] },
                    cost: PlanCost::default(),
                    cardinality: 100.0,
                }),
                join_type: JoinType::NestedLoop,
    ...
    fn test_pushdown_skip_through_project() {
        ...
        let plan = LogicalPlan {
            op: LogicalOp::Join {
                ...
                right: Box::new(LogicalPlan {
                    op: LogicalOp::Project { expressions: vec![Expression::Identifier("a".into())] },
                    cost: PlanCost::default(),
                    cardinality: 100.0,
                }),
                join_type: JoinType::NestedLoop,
    ...
    fn test_remove_true_filters_preserves_non_trivial_in_join() {
        let plan = LogicalPlan {
            op: LogicalOp::Join {
                ...
                right: Box::new(LogicalPlan {
                    op: LogicalOp::Filter {
                        condition: Expression::Eq(Box::new(Expression::Identifier("a".into())), Box::new(Expression::Int(1))),
                    },
                    cost: PlanCost::default(),
                    cardinality: 100.0,
                }),
    ...
    fn test_constant_fold_evaluates_literal_expression() {
        let plan = LogicalPlan {
            op: LogicalOp::Filter {
                condition: Expression::Add(Box::new(Expression::Int(2)), Box::new(Expression::Int(3))),
            },
            cost: PlanCost::default(),
            cardinality: 100.0,
        };
    ...
    fn test_constant_fold_evaluates_bool_and() {
        let plan = LogicalPlan {
            op: LogicalOp::Filter {
                condition: Expression::And(Box::new(Expression::Bool(true)), Box::new(Expression::Bool(false))),
            },
            cost: PlanCost::default(),
            cardinality: 100.0,
        };
    ...
    fn test_edge_type_cardinality_with_stats() {
        ...
        stats.edge_types.insert(etype, EdgeTypeStats { edge_count: 500, distinct_source_vertices: 100, distinct_target_vertices: 100 });
