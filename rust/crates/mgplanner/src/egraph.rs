...
use mgparser::ast::{Expression, Direction};
    ...
    fn make(egraph: &EGraph<PlanLang, Self>, enode: &PlanLang) -> Self::Data {
        ...
        match enode {
            ...
            PlanLang::Join([left, right, _]) => {
                ...
                card = left_data.estimated_cardinality.max(right_data.estimated_cardinality);
            ...
            PlanLang::Sort([_, child]) => {
                ...
                cost = child_data.estimated_cost
                    + child_data.estimated_cardinality * 10.0 * child_data.estimated_cardinality.log2().max(1.0);
...
fn plan_to_id(plan: &LogicalPlan, expr: &mut RecExpr<PlanLang>) -> Id {
    let id = match &plan.op {
        ...
        LogicalOp::LabelScan { alias, label } => {
            let alias_id = alias.as_ref().map(|a| expr.add(PlanLang::Symbol(a.as_str().into()))).unwrap_or_else(|| expr.add(PlanLang::Num(0)));
        ...
        LogicalOp::LabelPropertyScan { alias, label, property, value } => {
            ...
            expr.add(PlanLang::LabelPropScan([label_id, prop_id, value_id, alias_id]))
        }
        ...
        LogicalOp::Join { left, right, join_type } => {
            ...
            let type_id = match join_type {
                ...
                crate::JoinType::HashJoin { left_key, right_key } => {
        ...
        LogicalOp::Sort { key } => {
            let child_id = expr.add(PlanLang::AllScan); // placeholder
        ...
        LogicalOp::EdgeExpand { direction, edge_type } => {
            ...
            let et_id = edge_type.map(|et| expr.add(PlanLang::Num(et.as_uint() as i64)))
...
fn node_to_plan(expr: &RecExpr<PlanLang>, id: Id) -> LogicalPlan {
    ...
    match node {
        ...
        PlanLang::Project([_exprs_id, child_id]) => {
            let child = node_to_plan(expr, *child_id);
            LogicalPlan {
                op: LogicalOp::Project { expressions: vec![] },
                cost: child.cost,
                cardinality: child.cardinality,
            }
        }
...
mod tests {
    ...
    fn test_new_egraph_rules_exist() {
        let rules = rules();
        assert!(rules.iter().any(|r| r.name().to_string().contains("limit-pushdown-sort")));
        assert!(rules.iter().any(|r| r.name().to_string().contains("limit-pushdown-filter")));
        assert!(rules.iter().any(|r| r.name().to_string().contains("remove-true-filter")));
        assert!(rules.iter().any(|r| r.name().to_string().contains("union-commute")));
        assert!(rules.iter().any(|r| r.name().to_string().contains("distinct-idempotent")));
        assert!(rules.iter().any(|r| r.name().to_string().contains("join-assoc")));
    }
    ...
    fn test_projection_pushdown_rules_exist() {
        let rules = rules();
        assert!(rules.iter().any(|r| r.name().to_string().contains("project-pushdown")));
    }

    #[test]
    fn test_distinct_limit_swap_rule_exists() {
        let rules = rules();
        assert!(rules.iter().any(|r| r.name().to_string().contains("distinct-limit-swap")));
    }

    #[test]
    fn test_filter_pushdown_distinct_rule_exists() {
        let rules = rules();
        assert!(rules.iter().any(|r| r.name().to_string().contains("filter-pushdown-distinct")));
    }
}
