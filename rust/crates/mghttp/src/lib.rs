...
impl HttpApi {
    ...
    pub fn execute_query(
        ...
        let rows: Vec<serde_json::Value> = result
            .rows
            .into_iter()
            .map(|row: HashMap<String, mgcore::property_value::PropertyValue>| {
    ...
    pub fn list_sessions(&self) -> serde_json::Value {
        let active_count = if self.storage.has_active_transactions() { 1 } else { 0 };
        serde_json::json!({
            "sessions": [
                {
                    "id": "default",
    ...
    pub fn list_indices(&self) -> serde_json::Value {
        let label_indices: Vec<String> = self.storage.active_label_indices
            .read().unwrap()
            .iter()
            .map(|id| format!("{}", id.as_uint()))
            .collect();
        let label_prop_indices: Vec<serde_json::Value> = self.storage.active_label_property_indices
            .read().unwrap()
            .iter()
            .map(|(label, property)| serde_json::json!({
                "label": label.as_uint(),
    ...
    pub fn drop_index(&self, body: &serde_json::Value) -> Result<serde_json::Value, String> {
        if let (Some(label), Some(property)) = (
            ...
            let mut indices = self.storage.active_label_property_indices.write().unwrap();
            indices.remove(&(mgcore::types::LabelId::from(label as u32), mgcore::types::PropertyId::from(property as u32)));
    ...
    pub fn create_constraint(&self, body: &serde_json::Value) -> Result<serde_json::Value, String> {
        ...
        let property = body.get("property")
            .and_then(|v| v.as_u64())
            .ok_or("missing property")? as u32;
        let kind_str = body.get("kind")
            .and_then(|v| v.as_str())
            .ok_or("missing kind")?;
        let label_id = mgcore::types::LabelId::from(label);
        let prop_id = mgcore::types::PropertyId::from(property);
        match kind_str {
            "unique" => self.storage.constraints.add_unique_constraint(label_id, vec![prop_id]),
            "exists" => self.storage.constraints.add_existence_constraint(label_id, prop_id),
            "type" => {
                let expected = body.get("expected_type")
                    .and_then(|v| v.as_str())
                    .unwrap_or("string");
                ...
                self.storage.constraints.add_type_constraint(label_id, prop_id, ctype);
            }
    ...
    pub fn drop_constraint(&self, body: &serde_json::Value) -> Result<serde_json::Value, String> {
        ...
        let prop_id = mgcore::types::PropertyId::from(property);
        match kind_str {
            "unique" => self.storage.constraints.remove_unique_constraint(label_id, &[prop_id]),
            "exists" => self.storage.constraints.remove_existence_constraint(label_id, prop_id),
            "type" => self.storage.constraints.remove_type_constraint(label_id, prop_id),
            _ => return Err(format!("unknown kind: {}", kind_str)),
        };
...
fn json_to_property_value(v: &serde_json::Value) -> Result<mgcore::property_value::PropertyValue, String> {
...
fn property_value_to_json(v: mgcore::property_value::PropertyValue) -> Option<serde_json::Value> {
    ...
    Some(match v {
        ...
        PropertyValue::Double(f) => serde_json::Value::Number(
    ...
    pub fn dispatch(
        ...
        match (method, path) {
            ...
            ("POST", "/config") => {
                ...
                let key = body.get("key").and_then(|v| v.as_str()).ok_or("missing key")?;
            ...
            ("POST", "/query") => {
                let body = body.ok_or("missing body")?;
                let query = body.get("query").and_then(|v| v.as_str()).ok_or("missing query")?;
            ...
            ("POST", "/query/terminate") => {
                let body = body.ok_or("missing body")?;
                let _session_id = body.get("session_id").and_then(|v| v.as_u64())
...
mod tests {
    ...
    fn test_config_update() {
        let api = make_api();
        assert!(api.update_config("query_timeout", serde_json::json!(300)).is_ok());
        assert!(api.update_config("unknown_key", serde_json::json!(1)).is_err());
    }
    ...
    fn test_graph_import_export_roundtrip() {
        let api = make_api();
        // Create some data
        api.execute_query("CREATE (n:Person {name: 'Alice', age: 30})-[:KNOWS]->(m:Person {name: 'Bob'})", None).unwrap();

        // Export
        let export = api.execute_query("MATCH (n)-[r]->(m) RETURN n, r, m", None).unwrap();
