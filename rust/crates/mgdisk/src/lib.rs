...
use mgslk::{slk_encode, Reader, SlkLoad, SlkSave};
...
impl DiskStore {
    ...
    fn replay_wal(&self) -> Result<(), std::io::Error> {
        ...
        while offset + header_size <= data.len() {
            let header: WalEntryHeader = bincode::deserialize(&data[offset..offset + header_size])
                .map_err(|e| std::io::Error::new(std::io::ErrorKind::InvalidData, e))?;
            let entry_end =
                offset + header_size + header.key_len as usize + header.value_len as usize;
            ...
            let key =
                String::from_utf8_lossy(&data[key_start..key_start + header.key_len as usize]);
    ...
    pub fn delete_adjacency(
        ...
        let path = self.root.join("adj").join(direction).join(format!(
            "{:016x}:{:08x}:{:016x}:{:016x}",
            src, label, dst, eid
        ));
        if path.exists() {
    ...
    pub fn get_property(
        ...
        let path = self
            ...
            .join("prop")
            .join(entity_type)
            .join(format!("{:016x}:{:08x}", entity_id, prop_key));
        if !path.exists() {
            return Ok(None);
        }
    ...
    pub fn put_entities_batch<T: SlkSave>(
        &self,
        prefix: &str,
        items: &[(u64, T)],
    ) -> Result<(), std::io::Error> {
...
impl<'a> BatchTx<'a> {
    ...
    pub fn put(&mut self, prefix: &str, id: u64, value: &[u8]) {
        self.ops
            .push((format!("{}:{}", prefix, id), Some(value.to_vec())));
    }
