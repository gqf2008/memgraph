...
impl Clone for ServerMetrics {
    fn clone(&self) -> Self {
        Self {
            ...
            query_duration_ms_total: AtomicU64::new(self.query_duration_ms_total.load(Ordering::Relaxed)),
        }
    }
}
...
impl ServerMetrics {
    ...
    pub fn record_query_duration(&self, ms: u64) {
        self.query_duration_ms_total.fetch_add(ms, Ordering::Relaxed);
    }
...
impl AdminState {
    ...
    pub fn with_config(config: ServerConfig) -> Self {
        let rate_limiter = RateLimiter::new(config.rate_limit_max_tokens, config.rate_limit_refill_secs);
    ...
    pub fn register_connection(&self, peer: String, user: Option<String>, client_name: String, bolt_version: (u8, u8)) -> ConnectionId {
    ...
    pub fn start_query(&self, conn_id: ConnectionId, query: String) -> u64 {
        ...
        self.kill_flags.lock().unwrap().insert(qid, AtomicBool::new(false));
    ...
    pub fn finish_query(&self, qid: u64) {
        if let Some(aq) = self.active_queries.write().unwrap().remove(&qid) {
            ...
            self.query_stats.write().unwrap()
                .entry(fingerprint).or_default()
    ...
    pub fn is_query_killed(&self, qid: u64) -> bool {
        let flags = self.kill_flags.lock().unwrap();
        flags.get(&qid).map(|f: &AtomicBool| f.load(Ordering::Relaxed)).unwrap_or(false)
    }
...
impl RateLimiter {
    ...
    pub fn allow(&self, key: &str) -> bool {
        ...
        let now = Instant::now();
        let entry = tokens.entry(key.to_string()).or_insert((self.max_tokens, now));
...
impl UserQuota {
    ...
    pub fn current_for(&self, user: &str) -> usize {
        self.connections.lock().unwrap().get(user).copied().unwrap_or(0)
    }
}
...
mod tests {
    ...
    fn test_register_connection() {
        ...
        let id = admin.register_connection("127.0.0.1:7687".into(), Some("admin".into()), "neo4j-python/5.0".into(), (4, 4));
    ...
    fn test_query_fingerprint_normalization() {
        assert_eq!(query_fingerprint("MATCH (n) RETURN n"), "match (n) return n");
        assert_eq!(query_fingerprint("  MATCH   (n)\nRETURN  n  "), "match (n) return n");
    }
