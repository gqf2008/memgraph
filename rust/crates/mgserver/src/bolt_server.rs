...
fn bolt_value_to_property(v: &Value) -> PropertyValue {
    match v {
        ...
        Value::List(items) => PropertyValue::List(items.iter().map(bolt_value_to_property).collect()),
        Value::Map(entries) => PropertyValue::Map(entries.iter().map(|(k, v)| (k.clone(), bolt_value_to_property(v))).collect()),
...
fn property_value_to_bolt(pv: &PropertyValue, catalog: &Catalog) -> Value {
    match pv {
        PropertyValue::Vertex(v) => {
            let labels: Vec<String> = v.labels.iter()
                .map(|l| catalog.label_name(*l))
                .collect();
            let properties: HashMap<String, Value> = v.properties.iter()
                .map(|(pid, pval)| (catalog.property_name(pid), property_value_to_bolt(pval, catalog)))
                .collect();
            Value::node(v.gid.as_int(), labels, properties)
        }
        PropertyValue::Edge(e) => {
            let type_name = catalog.edge_type_name(e.edge_type);
            let properties: HashMap<String, Value> = e.properties.iter()
                .map(|(pid, pval)| (catalog.property_name(pid), property_value_to_bolt(pval, catalog)))
                .collect();
            Value::relationship(
                e.gid.as_int(),
        ...
        PropertyValue::List(items) => {
            Value::List(items.iter().map(|v| property_value_to_bolt(v, catalog)).collect())
        }
...
pub async fn run(
    ...
    let listener = match TcpListener::bind(&addr).await {
        Ok(l) => {
            tracing::info!("Bolt server listening on {} (max {} connections)", addr, max_connections);
            l
        }
        Err(e) => {
...
async fn handle_connection(
    ...
    let version = if Handshake::is_manifest_v1_request(&client_versions) {
        ...
        if stream.write_all(&Handshake::MANIFEST_V1.to_be_bytes()).await.is_err() {
            return;
        }
        ...
        while cap_buf[cap_len - 1] & 0x80 != 0 {
            ...
            if stream.read_exact(&mut cap_buf[cap_len..cap_len + 1]).await.is_err() {
                return;
            }
            cap_len += 1;
        }
        let normalized = Handshake::normalize_manifest_version(chosen_version);
        tracing::info!("[bolt] manifest chose version 0x{:08X} -> normalized 0x{:04X}", chosen_version, normalized);
        if Handshake::SUPPORTED.contains(&normalized) {
            normalized
        } else {
            tracing::warn!("[bolt] manifest chose unsupported version 0x{:08X} (normalized 0x{:04X})", chosen_version, normalized);
    ...
    if !Handshake::is_manifest_v1_request(&client_versions) {
        if stream.write_all(&version.to_be_bytes()).await.is_err() {
            return;
        }
    }
    tracing::info!("[bolt] negotiated v{}.{} with {}", version >> 8, version & 0xFF, peer_str);
    ...
    loop {
        ...
        if !is_valid_in_state(&msg, conn.state) {
            send_failure(&mut stream, "Protocol.State", &format!("{} not valid in state {:?}", msg_name(&msg), conn.state)).await;
            conn.state = BoltState::Failed;
            continue;
        }

        match msg {
            Message::Hello { ref user_agent, ref extra } => {
                ...
                let has_embedded_auth = extra.get("scheme")
                ...
                if has_embedded_auth {
                    ...
                    let principal = extra.get("principal")
                        .and_then(|v| if let Value::String(s) = v { Some(s.as_str()) } else { None })
                        .unwrap_or("");
                    let credentials = extra.get("credentials")
                        .and_then(|v| if let Value::String(s) = v { Some(s.as_str()) } else { None })
                        .unwrap_or("");
                    if auth.is_required() && auth.authenticate(principal, credentials).is_none() {
                        send_failure(&mut stream, "Neo.ClientError.Security.Unauthorized", "Authentication failed").await;
                        conn.state = BoltState::Failed;
                        continue;
                    }
                    let user_str = extra.get("principal")
                        .and_then(|v| if let Value::String(s) = v { Some(s.clone()) } else { None });
                    let cid = admin.register_connection(peer_str.clone(), user_str, user_agent.clone(), ((version >> 8) as u8, (version & 0xFF) as u8));
                    conn_id = Some(cid);
                    ...
                    let cid = admin.register_connection(peer_str.clone(), None, user_agent.clone(), ((version >> 8) as u8, (version & 0xFF) as u8));
                    conn_id = Some(cid);
                    conn.state = BoltState::Authenticated;
                }
                ...
                meta.insert("connection_id".into(), Value::String(format!("bolt-{}", peer_str)));
            ...
            Message::Logon { ref extra } => {
                ...
                let principal = extra.get("principal")
                    .and_then(|v| if let Value::String(s) = v { Some(s.clone()) } else { None });
                let cid = admin.register_connection(peer_str.clone(), principal, "unknown".into(), ((version >> 8) as u8, (version & 0xFF) as u8));
                conn_id = Some(cid);
                conn.state = BoltState::Authenticated;
                if send_message(&mut stream, &Message::Success { metadata: HashMap::new() }).await.is_err() {
                    break;
                }
            }
            Message::Logoff => {
            ...
            Message::Run { ref query, ref parameters, .. } => {
                tracing::debug!("[bolt] RUN: {}", query);
                let qid = conn_id.map(|cid| admin.start_query(cid, query.clone()));
                let params: HashMap<String, PropertyValue> = parameters.iter().map(|(k, v)| (k.clone(), bolt_value_to_property(v))).collect();
                let timeout = if admin.config().query_timeout_ms > 0 {
                    Some(std::time::Duration::from_millis(admin.config().query_timeout_ms))
                } else {
                    None
                };
                ...
                match execute_with_catalog_auth_dbms_and_params_timeout(&storage, query, Some(&catalog), &params, None, None, timeout) {
                    Ok(result) => {
                        let mut meta = HashMap::new();
                        meta.insert("fields".into(),
                            Value::List(result.columns.iter().map(|c| Value::String(c.clone())).collect()));
                        ...
                        if send_message(&mut stream, &Message::Success { metadata: meta }).await.is_err() {
                            if let Some(qid) = qid { admin.fail_query(qid); }
                            break;
                        }
                    }
                    Err(e) => {
                ...
                if let Some(ref result) = conn.pending_result {
                    if let Some(ref mut idx) = conn.result_iter {
                        ...
                        while *idx < result.rows.len() && sent < n {
                            ...
                            let fields: Vec<Value> = result.columns.iter().map(|col| {
                                row.get(col).map(|v| property_value_to_bolt(v, &catalog)).unwrap_or(Value::Null)
                            }).collect();
                            if send_message(&mut stream, &Message::Record { fields }).await.is_err() {
                                break;
                            }
                            *idx += 1;
                            sent += 1;
            ...
            Message::Discard { n, .. } => {
                ...
                if let Some(ref mut idx) = conn.result_iter {
                    let result_len = conn.pending_result.as_ref().map(|r| r.rows.len()).unwrap_or(0);
            ...
            Message::Begin { .. } => {
                if conn.in_transaction {
                    send_failure(&mut stream, "Database.Error", "nested transactions not supported").await;
                    conn.state = BoltState::Failed;
                } else {
                    conn.explicit_tx = Some(storage.begin_transaction(mgcore::delta::IsolationLevel::SnapshotIsolation));
            ...
            Message::Success { .. } | Message::Failure { .. } | Message::Ignored | Message::Record { .. } => {
...
async fn write_message(stream: &mut TcpStream, payload: &[u8]) -> std::io::Result<()> {
    ...
    while offset < payload.len() {
        ...
        stream.write_all(&payload[offset..offset + chunk_size]).await?;
        offset += chunk_size;
    }
...
async fn send_failure(stream: &mut TcpStream, code: &str, message: &str) {
    let _ = send_message(stream, &Message::Failure {
        code: code.into(),
...
fn is_valid_in_state(msg: &Message, state: BoltState) -> bool {
    match state {
        ...
        BoltState::Authentication => matches!(msg, Message::Logon { .. } | Message::Reset | Message::Goodbye),
        BoltState::Authenticated => matches!(msg, Message::Run { .. } | Message::Begin { .. } | Message::Reset | Message::Goodbye),
        BoltState::Ready => matches!(msg, Message::Run { .. } | Message::Begin { .. } | Message::Reset | Message::Goodbye | Message::Route { .. } | Message::Telemetry { .. } | Message::Logoff),
        BoltState::Streaming => matches!(msg, Message::Pull { .. } | Message::Discard { .. } | Message::Reset | Message::Goodbye | Message::Route { .. } | Message::Logoff),
        BoltState::TxReady => matches!(msg, Message::Run { .. } | Message::Commit | Message::Rollback | Message::Reset | Message::Goodbye | Message::Route { .. } | Message::Telemetry { .. } | Message::Logoff),
        BoltState::TxStreaming => matches!(msg, Message::Pull { .. } | Message::Discard { .. } | Message::Reset | Message::Goodbye | Message::Route { .. } | Message::Logoff),
...
fn parse_message(payload: &[u8]) -> Option<Message> {
    let val = match mgbolt::decode_value(payload) {
        Ok(v) => v.0,
        Err(e) => { tracing::debug!("[bolt] decode error: {:?}", e); return None; }
    };
    match val {
        Value::Struct(0x01, ref fields) => {
            let mut meta = fields.first()
                .and_then(|v| if let Value::Map(m) = v { Some(m.clone()) } else { None })
                .unwrap_or_default();
            let user_agent = meta.remove("user_agent")
                .and_then(|v| if let Value::String(s) = v { Some(s) } else { None })
                .unwrap_or_else(|| "unknown".into());
            Some(Message::Hello { user_agent, extra: meta })
        }
        Value::Struct(0x10, ref fields) => {
            let query = fields.first()
                .and_then(|v| if let Value::String(s) = v { Some(s.clone()) } else { None })
                .unwrap_or_default();
            let parameters = fields.get(1)
                .and_then(|v| if let Value::Map(m) = v { Some(m.clone()) } else { None })
                .unwrap_or_default();
            let extra = fields.get(2)
                .and_then(|v| if let Value::Map(m) = v { Some(m.clone()) } else { None })
                .unwrap_or_default();
            Some(Message::Run { query, parameters, extra })
        }
        ...
        Value::Struct(0x66, ref fields) => {
            ...
            if let Some(Value::List(bms)) = extra.get("bookmarks") {
                bookmarks = bms.iter().filter_map(|v| if let Value::String(s) = v { Some(s.clone()) } else { None }).collect();
            }
            Some(Message::Route { routing, bookmarks, extra })
        }
        Value::Struct(0x54, ref fields) => {
            let extra = fields.first()
                .and_then(|v| if let Value::Map(m) = v { Some(m.clone()) } else { None })
                .unwrap_or_default();
            let api = extra.get("api").and_then(|v| if let Value::Int(i) = v { Some(*i) } else { None }).unwrap_or(0);
...
fn parse_pull_discard_args(fields: &[Value]) -> (i64, i64) {
    ...
    for (i, field) in fields.iter().enumerate() {
        if let Value::Map(m) = field {
            if i == 0 {
                if let Some(Value::Int(v)) = m.get("n") { n = *v; }
            }
            if let Some(Value::Int(v)) = m.get("qid") { qid = *v; }
        }
    }
    (n, qid)
}
