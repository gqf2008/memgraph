...
impl ServerContext {
    pub fn new(data_dir: PathBuf) -> Self {
        ...
        info!("System: {} CPUs, {} MB total memory, PID {}",
        ...
        Self {
            ...
            cluster_state: std::sync::RwLock::new(Arc::new(mgcoord::ClusterState::new(String::new()))),
        }
    }

    pub fn new_with_config(data_dir: PathBuf, cache_size: usize, cache_ttl: Option<Duration>, cache_policy: EvictionPolicy) -> Self {
        let mut ctx = Self::new(data_dir);
        ctx.query_cache = QueryCache::new(cache_size)
...
pub fn initialize(ctx: &ServerContext, config: &StartupConfig) -> Result<(), String> {
    ...
    match recover(
        ...
        Ok(()) => info!("Recovery complete from {} + {}", snap_path.display(), wal_path.display()),
...
pub fn shutdown(ctx: &ServerContext) {
    ...
    if !conns.is_empty() {
        ...
        for i in 0..30 {
            ...
            if i == 29 {
                warn!("Forcing shutdown with {} connections still open", ctx.admin.connection_count());
            }
        }
    }
...
pub fn persist_state(ctx: &ServerContext) {
    ...
    if let Ok(wal_writer) = WalWriter::create(&wal_path) {
        ctx.storage.set_wal(Box::new(ServerWalWriter { inner: wal_writer }));
...
pub fn reload_config(ctx: &ServerContext, path: &Path) -> Result<(), String> {
    ...
    let cfg: ServerConfigFile = serde_json::from_str(&contents)
        ...
        if let Some(ref path) = args.config {
            if let Ok(contents) = fs::read_to_string(path) {
                if let Ok(cfg) = serde_json::from_str::<ServerConfigFile>(&contents) {
                    if let Some(v) = cfg.max_connections { runtime_config.max_connections = v; }
                    if let Some(v) = cfg.query_timeout_ms { runtime_config.query_timeout_ms = v; }
                    if let Some(v) = cfg.slow_query_threshold_ms { runtime_config.slow_query_threshold_ms = v; }
                    if let Some(v) = cfg.gc_interval_secs { runtime_config.gc_interval_secs = v; }
                    if let Some(v) = cfg.query_cache_size { cache_size = v; }
                    if let Some(v) = cfg.query_cache_ttl_secs { cache_ttl_secs = Some(v); }
                }
            }
        }
...
pub fn health_check_startup(ctx: &ServerContext) -> Result<Vec<String>, String> {
    ...
    if ctx.storage.all_vertices().len() == 0 {
        // Empty storage is fine, just verify it doesn't panic
    }

    // Check catalog
    let (labels, props, edge_types) = ctx.catalog.dump_mappings();
    info!("Catalog: {} labels, {} properties, {} edge types", labels.len(), props.len(), edge_types.len());
