...
use mgstorage::storage::{EdgeSnapshot, Storage, VertexSnapshot};
    ...
    B::Error: std::fmt::Display + Send + Sync,
{
    ...
    let result = match (method, path.as_str()) {
        ...
        (Method::GET, path)
            if path.starts_with("/api/v1/vertices/") && path.ends_with("/edges") =>
        {
        ...
        (Method::GET, path)
            if path.starts_with("/api/v1/labels/") && path.ends_with("/vertices") =>
        {
        ...
        (Method::POST, "/api/v1/vertices/batch-delete") => {
            handle_delete_vertices_batch(&state, req).await
        }
        (Method::POST, "/api/v1/edges/batch-delete") => {
            handle_delete_edges_batch(&state, req).await
        }
...
async fn handle_get_vertex(
    ...
    match state.storage.get_vertex(gid, &tx) {
        Some(snap) => Ok(json_response(&vertex_snapshot_to_json(
            &snap,
            &state.catalog,
        ))),
...
async fn handle_list_edges(
    ...
    let arr: Vec<serde_json::Value> = all
        .into_iter()
        .map(|(gid, from, to, etype, props)| {
            edge_to_json(gid, from, to, etype, &props, &state.catalog)
        })
        .collect();
    Ok(json_response(&serde_json::Value::Array(arr)))
}
    ...
    B::Error: std::fmt::Display + Send + Sync,
{
    ...
    let mut params: std::collections::HashMap<String, PropertyValue> =
    ...
    let timeout = if state.admin.config().query_timeout_ms > 0 {
        Some(std::time::Duration::from_millis(
            state.admin.config().query_timeout_ms,
        ))
    } else {
        None
    };
...
async fn handle_delete_vertex(
    ...
    match state.storage.delete_vertex_and_edges(&tx, gid) {
        Ok((_vertex_count, _edge_count)) => {
            state.storage.commit_transaction(&tx);
            Ok(json_response(
                &serde_json::json!({ "deleted": true, "gid": gid.as_uint() }),
            ))
        }
        Err(e) => Ok(error_response(
            StatusCode::NOT_FOUND,
            &format!("vertex not found: {}", e),
        )),
    }
}
    ...
    B::Error: std::fmt::Display + Send + Sync,
{
    ...
    let from_gid = req_json
        .get("from_vertex")
        .and_then(|v| v.as_u64())
        .map(Gid::from)
        .ok_or("missing from_vertex")?;
    let to_gid = req_json
        .get("to_vertex")
        .and_then(|v| v.as_u64())
        .map(Gid::from)
        .ok_or("missing to_vertex")?;
    let edge_type_name = req_json
        .get("edge_type")
        .and_then(|v| v.as_str())
        .unwrap_or("REL");
...
async fn handle_list_vertices_by_label(
    ...
    let label_name = path
        .strip_prefix("/api/v1/labels/")
        .and_then(|p| p.strip_suffix("/vertices"))
        .unwrap_or("");
...
async fn handle_cluster_status(
    ...
    if let Some(ref cluster_state) = state.cluster_state {
        body.insert(
            ...
            serde_json::json!(cluster_state.instance_count()),
        );
        body.insert(
            "healthy_instance_count".to_string(),
            serde_json::json!(cluster_state.healthy_instance_count()),
        );
...
mod tests {
    ...
    async fn test_query_endpoint() {
        ...
        state
            .storage
            .vertex_add_label(&tx, gid, state.catalog.label("Person"))
            .unwrap();
        state.storage.commit_transaction(&tx);
    ...
    async fn test_bad_query_returns_400() {
        ...
        let query_req = Request::builder()
            ...
            .uri("/api/v1/query")
            .header("content-type", "application/json")
            .body(Full::new(Bytes::from(r#"{"query":"INVALID CYPHER!!!"}"#)))
            .unwrap();
    ...
    async fn test_update_edge() {
        ...
        let put_req = Request::builder()
            ...
            .uri(format!("/api/v1/edges/{}", e1.as_uint()))
            .header("content-type", "application/json")
            .body(Full::new(Bytes::from(r#"{"properties":{"since":2024}}"#)))
            .unwrap();
        let resp = handle_request(put_req, state.clone()).await.unwrap();
        assert_eq!(resp.status(), StatusCode::OK);
        let body = read_body_to_string(resp).await;
