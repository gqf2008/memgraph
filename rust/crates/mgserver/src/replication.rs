...
use mgrepl::{
    AsyncReplicationClient, ReplicationClient, ReplicationConfig, ReplicationMode,
    ReplicationRole, ReplServer, SyncReplicationClient,
};
use mgstorage::{WalRecord, WalAppender};
use mgstorage::storage::Storage;
use mgrpc::{decode_body, DeltaBatch, DeltaStreamRequest, MessageHeader, RpcClient, WalRequest, WalTransfer};
...
impl MainReplicationState {
    pub fn bind(addr: &str, config: ReplicationConfig, storage: Arc<Storage>) -> std::io::Result<Arc<Self>> {
...
fn serve_wal_request(mut rpc: RpcClient, state: &MainReplicationState, req: WalRequest) {
    if req.epoch_id != state.epoch_id {
        ...
        let empty = WalTransfer { file_name: String::new(), data: Vec::new() };
...
impl ReplicationManager {
    ...
    pub fn new_main(bind_addr: &str, config: ReplicationConfig, storage: Arc<Storage>) -> std::io::Result<Self> {
    ...
    pub fn new_replica(main_addr: &str, config: ReplicationConfig, storage: Arc<Storage>) -> std::io::Result<Self> {
    ...
    fn wal_to_delta(record: &WalRecord) -> Option<DeltaRecord> {
        match record {
            ...
            WalRecord::VertexSetProperty { gid, key, value } => Some(DeltaRecord::VertexSetProperty {
                gid: *gid,
                key: *key,
                value: value.clone(),
            }),
...
impl WalAppender for ReplicatingWalWriter {
    fn append(&mut self, record: WalRecord) {
        ...
        match &record {
            WalRecord::TransactionEnd { commit_timestamp, .. } => {
...
mod tests {
    ...
    fn temp_wal_with_data(name: &str, payload: &[u8]) -> PathBuf {
        let path = std::env::temp_dir().join(format!("mgserver_repl_test_{}_{}.mgwal", name, std::process::id()));
    ...
    fn test_replica_can_pull_wal_via_request_wal() {
        ...
        let state = MainReplicationState::bind("127.0.0.1:0", ReplicationConfig::default(), storage)
    ...
    fn test_wal_request_epoch_mismatch_returns_empty() {
        ...
        assert_eq!(files.len(), 1);
        let (file_name, data) = files.into_iter().next().unwrap();
        assert!(file_name.is_empty(), "epoch mismatch should yield empty file_name");
        assert!(data.is_empty());
