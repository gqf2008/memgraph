...
impl PreparedStatementCache {
    ...
    pub fn record_execution(&self, id: u64, duration_ms: u64) {
        ...
        if let Some(stmt) = inner.get_mut(&id) {
            ...
            // EWMA with alpha=0.1
            let alpha = 0.1;
            stmt.avg_duration_ms = alpha * duration_ms as f64 + (1.0 - alpha) * stmt.avg_duration_ms;
        }
    }
...
impl CacheWarmer {
    ...
    pub fn warm(&self, cache: &QueryCache, storage: &mgstorage::storage::Storage, _catalog: &mgcatalog::Catalog) -> usize {
        let mut warmed = 0;
...
mod tests {
    ...
    fn test_query_cache_lru_eviction() {
        ...
        cache.put("q1".into(), mgparser::parse_query("RETURN 1").unwrap(), None);
        cache.put("q2".into(), mgparser::parse_query("RETURN 2").unwrap(), None);
        // Access q1 to make it most-recently-used
        cache.get("q1");
        cache.put("q3".into(), mgparser::parse_query("RETURN 3").unwrap(), None);
    ...
    fn test_prepared_statement() {
        ...
        let stmt = cache.prepare("MATCH (n {name: $name}) RETURN n".into()).unwrap();
        assert_eq!(stmt.id, 1);
    ...
    fn test_cache_warmer() {
        ...
        let cache = QueryCache::new(10);
        let warmed = warmer.warm(&cache, &mgstorage::storage::Storage::new(), &mgcatalog::Catalog::new());
        assert_eq!(warmed, 0);
    }
}
