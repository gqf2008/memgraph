...
mod tests {
    ...
    fn test_create_user() {
        let auth = AuthConfig::basic("admin", "secret");
        auth.create_user("alice".into(), "password".into(), vec!["reader".into()])
            .unwrap();
        assert!(auth.list_users().contains(&"alice".into()));
        assert!(matches!(
            auth.create_user("alice".into(), "x".into(), vec![]),
    ...
    fn test_drop_user() {
        let auth = AuthConfig::basic("admin", "secret");
        auth.create_user("bob".into(), "pass".into(), vec![])
            .unwrap();
    ...
    fn test_lock_unlock() {
        let auth = AuthConfig::basic("admin", "secret");
        auth.create_user("carol".into(), "pass".into(), vec![])
            .unwrap();
    ...
    fn test_is_admin() {
        ...
        auth.create_user("user".into(), "pass".into(), vec!["reader".into()])
            .unwrap();
    ...
    fn test_label_permissions() {
        ...
        auth.create_user("dave".into(), "pass".into(), vec!["reader".into()])
            .unwrap();
