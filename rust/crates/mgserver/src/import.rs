...
mod tests {
    ...
    fn test_import_jsonl() {
        ...
        writeln!(
            f,
            r#"{{"type":"node","label":"Person","properties":{{"name":"Alice","age":30}}}}"#
        )
        .unwrap();
    ...
    fn test_json_to_create_node() {
        let obj: JsonValue = serde_json::from_str(
            r#"{"type":"node","label":"Person","properties":{"name":"Alice"}}"#,
        )
        .unwrap();
    ...
    fn test_json_to_create_edge() {
        let obj: JsonValue =
            serde_json::from_str(r#"{"type":"edge","from":"a","to":"b","edge_type":"KNOWS"}"#)
                .unwrap();
        let q = json_to_create(&obj).unwrap();
    ...
    fn test_import_csv_with_progress() {
        ...
        let count =
            import_csv_with_progress(&storage, Path::new(tmp), "Person", 2, Some(cb)).unwrap();
        assert_eq!(count, 3);
