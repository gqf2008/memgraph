...
use mgstorage::{WalRecord, WalAppender};
...
use crate::startup::{initialize, shutdown, persist_state, reload_config, start_background_tasks, ServerContext, StartupConfig};
use crate::query_cache::{QueryCache, PreparedStatementCache};
    ...
    fn append(&mut self, record: WalRecord) {
        let delta = match record {
            ...
            WalRecord::VertexDelete { gid } => {
                mgdurability::DeltaRecord::VertexDelete { gid }
            }
            ...
            WalRecord::EdgeCreate { gid, from_vertex, to_vertex, edge_type, timestamp } => {
                mgdurability::DeltaRecord::EdgeCreate { gid, from_vertex, to_vertex, edge_type, timestamp }
            }
            WalRecord::EdgeDelete { gid } => {
                mgdurability::DeltaRecord::EdgeDelete { gid }
            }
            WalRecord::EdgeSetProperty { gid, key, value } => {
                mgdurability::DeltaRecord::EdgeSetProperty { gid, key, value }
            }
            WalRecord::TransactionEnd { timestamp, commit_timestamp } => {
                mgdurability::DeltaRecord::TransactionEnd { timestamp, commit_timestamp }
            }
        };
...
async fn main() {
    ...
    match args.replication_role.as_str() {
        "main" => {
            ...
            match crate::replication::ReplicationManager::new_main(&bind_addr, repl_config, ctx.storage.clone()) {
                Ok(manager) => {
        ...
        "replica" => {
            if let Some(ref main_addr) = args.replica_of {
                match crate::replication::ReplicationManager::new_replica(main_addr, repl_config, ctx.storage.clone()) {
                    Ok(manager) => {
    ...
    if let Some(ref coordinator_id) = args.coordinator_id {
        ...
        if args.coordinator_use_raft {
            ...
            match start_raft_coordinator(coordinator_id, raft_id, instances, &ctx.data_directory).await
            {
                Ok((cm, state)) => {
                    ...
                    *ctx.cluster_state.write().unwrap() = state;
                    ...
                    let health_interval =
                        Duration::from_secs(args.coordinator_health_interval);
                    let health_timeout = Duration::from_secs(
                        args.coordinator_health_interval.saturating_mul(3),
                    );
                    tokio::spawn(async move {
            ...
            let coordinator = mgcoord::Coordinator::new(
                coordinator_id.clone(),
                Duration::from_secs(5),
            );

            let cluster_config = mgcoord::ClusterConfig::new(instances.clone(), "memgraph".to_string());
            let discovery_items: Vec<(String, std::net::SocketAddr, mgcoord::InstanceRole)> = instances
                .iter()
            ...
            *ctx.coordinator.lock().unwrap() = Some(coordinator);
            info!("Coordinator '{}' bootstrapped with {} peer(s)", coordinator_id, discovery_items.len());
            ...
            std::thread::Builder::new()
                ...
                .spawn(move || {
                    ...
                    loop {
                        ...
                        let mut guard = coord_ctx.lock().unwrap();
    ...
    if let Ok(wal_writer) = WalWriter::create(&wal_path) {
        ...
        if let Some(ref repl) = *repl_guard {
            ...
            drop(repl_guard);
            let repl_writer = crate::replication::ReplicatingWalWriter::new(
                base_writer,
    ...
    // Determine ports
    let bolt_port = args.bolt_port.or_else(|| if args.bolt { Some(bolt_server::DEFAULT_PORT) } else { None });
    ...
    let bolt_handle = if let Some(port) = bolt_port {
        ...
        Some(tokio::spawn(async move {
            bolt_server::run(bolt_storage, bolt_catalog, bolt_auth, bolt_admin, port, bolt_server::DEFAULT_MAX_CONNECTIONS).await;
        }))
    } else {
        None
    };
    ...
    let http_handle = if let Some(port) = args.http_port {
        ...
        Some(tokio::spawn(async move {
            let server = http::HttpServer::new(
                http_storage, http_admin, http_catalog,
                http_cluster_state, http_cluster_manager,
            );
...
async fn start_raft_coordinator(
    ...
    let mut state_machine = mgcoord::raft::PersistentCoordinatorStateMachine::open(&sm_dir)
        .map_err(|e| format!("failed to open state machine at {}: {}", sm_dir.display(), e))?;
...
fn execute_cached(ctx: &ServerContext, query: &str) -> Result<mginterp::QueryResult, String> {
    ...
    ctx.query_cache.put_with_result(query.to_string(), parsed, Some(plan), cached_result);
    Ok(result)
}
...
fn handle_repl_command(ctx: &ServerContext, input: &str) {
    ...
    match cmd {
        ...
        ":clear" => { print!("\x1B[2J\x1B[H"); let _ = io::stdout().flush(); }
        ...
        ":prepare" => {
            if parts.len() < 2 {
                ...
                match ctx.prepared_statements.prepare(query.to_string()) {
                    Ok(stmt) => println!("Prepared statement #{} with params {:?}", stmt.id, stmt.parameter_names),
...
fn print_active_queries(admin: &AdminState) {
    ...
    for q in queries {
        ...
        println!("  #{} conn={} elapsed={}s: {}", q.query_id, q.connection_id, elapsed, q.query_text);
    }
}
...
fn print_indexes(storage: &Storage, catalog: &Catalog) {
    ...
    for label in labels.iter() {
        println!("    :{} (id={})", catalog.label_name(*label), label.as_uint());
    }
    println!("  Label+Property indices: {}", lp.len());
    for (label, prop) in lp.iter() {
        println!("    :{} + {} (id={},{})",
            catalog.label_name(*label), catalog.property_name(*prop), label.as_uint(), prop.as_uint());
    }
}
...
fn print_constraints(storage: &Storage) {
    ...
    for c in constraints {
        println!("  {:?} on label={} property={}", c.kind, c.label.as_uint(), c.property.as_uint());
    }
}
    ...
    match execute_cached(ctx, query) {
        Ok(result) => {
            ...
            if elapsed > Duration::from_millis(0) {
                println!("  Rows/sec: {:.0}", result.rows.len() as f64 / elapsed.as_secs_f64());
            }
        }
        Err(e) => eprintln!("Error: {}", e),
    }
...
fn print_plan(plan: &mgplanner::LogicalPlan, depth: usize) {
    ...
    println!("{}{:?} (cost={:.1}, card={:.1})", indent, plan.op, plan.cost.total(), plan.cardinality);
}
...
fn print_cache_stats(query_cache: &QueryCache, prepared: &PreparedStatementCache) {
    ...
    for stmt in prepared.list() {
        println!("  #{}: exec_count={}, avg_ms={:.1}", stmt.id, stmt.execution_count, stmt.avg_duration_ms);
    }
}
...
fn print_result(result: &mginterp::QueryResult) {
    ...
    if result.rows.is_empty() { println!("(empty result)"); }
    else { println!("{} rows returned", result.rows.len()); }
}
...
mod tests {
    ...
    async fn test_start_raft_creates_cluster_manager() {
        ...
        let instances = vec![
            mgcoord::Instance::new("i1".into(), test_addr(7687), mgcoord::InstanceRole::Main),
        ];
    ...
    async fn test_start_raft_persists_across_restarts() {
        ...
        // Second start with the same directories should reopen persisted state
        {
            ...
            assert!(is_leader, "node should re-elect itself as leader after restart");
            assert_eq!(state.instance_count(), 1);
            assert_eq!(state.main_instance().unwrap().id, "i1");
        }
    ...
    async fn test_raft_health_check_proposes_failover() {
        let tmp =
            std::env::temp_dir().join(format!("mg_raft_health_{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&tmp);
    ...
    async fn test_raft_health_check_exits_on_shutdown() {
        let tmp = std::env::temp_dir()
            .join(format!("mg_raft_health_exit_{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&tmp);
        ...
        assert!(result.is_ok(), "health-check task should exit after shutdown");
