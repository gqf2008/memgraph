...
impl QueryMetrics {
    pub fn new(registry: &MetricsRegistry) -> Self {
        Self {
            ...
            failed_queries: registry
                .counter("memgraph_queries_failed_total", "Total failed queries"),
            query_duration: registry.histogram(
                "memgraph_query_duration_seconds",
                "Query execution time",
                &[0.001, 0.01, 0.1, 0.5, 1.0, 5.0, 10.0],
            ),
            active_transactions: registry.gauge(
                "memgraph_active_transactions",
...
impl StorageMetrics {
    pub fn new(registry: &MetricsRegistry) -> Self {
        Self {
            ...
            deltas_freed: registry
                .counter("memgraph_gc_deltas_freed_total", "Total deltas freed by GC"),
        }
    }
