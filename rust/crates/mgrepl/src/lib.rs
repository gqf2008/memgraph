...
use mgrpc::{DeltaBatch, DeltaStreamRequest, Heartbeat, MessageHeader, ProtocolVersion, RpcClient, RpcServer, SnapshotData, WalRequest, WalTransfer};

pub mod handler;

// Re-export new handler types for convenience.
pub use handler::{AckResult, DeltaBatcher, ReplicaHealth, RetryState, ReplicationMetrics, ReplicationFilter, FailoverDetector, StorageDeltaApplier};
...
pub enum ReplRequest {
    Heartbeat { epoch_id: u64, lsn: u64 },
    Snapshot { last_durable_timestamp: u64 },
    Wal { since_timestamp: u64 },
    DeltaStream { epoch_id: u64, since_timestamp: u64, batch_size_limit: u32 },
}

#[derive(Clone, Debug, PartialEq)]
pub enum ReplResponse {
    Heartbeat { epoch_id: u64, lsn: u64, main_uuid: String },
    Snapshot { data: Vec<u8> },
    Wal { file_name: String, data: Vec<u8> },
    DeltaBatch(DeltaBatch),
    DeltaStreamBatch { batch: DeltaBatch },
    DeltaStreamEnd,
    Ack,
...
impl SyncReplicationClient {
    ...
    pub fn send_and_wait(&mut self, batch: &DeltaBatch) -> Result<(), ReplError> {
        crate::send_delta_batch(&mut self.rpc, batch)
            .map_err(|e| ReplError::Io(e.to_string()))?;
...
fn resolve_edge_endpoints(
    storage: Option<&std::sync::Arc<mgstorage::storage::Storage>>,
    gid: mgcore::types::Gid,
) -> Option<(mgcore::types::Gid, mgcore::types::Gid, mgcore::types::EdgeTypeId)> {
    let storage = storage?;
    let tx = storage.begin_transaction(mgcore::delta::IsolationLevel::SnapshotIsolation);
...
impl ReplServer {
    ...
    ///
    /// In a real implementation this would be async and hold storage state.
    /// Here we provide the dispatch logic with placeholder storage access.
    pub fn handle_request(
        ...
        match req {
            ...
            ReplRequest::Snapshot { last_durable_timestamp } => {
                // Serialize current storage state into a snapshot.
                // Placeholder: return empty snapshot data.
                let data = Self::serialize_storage_state(_storage);
                ReplResponse::Snapshot { data }
            }
            ReplRequest::Wal { since_timestamp } => {
                // Return WAL records since the given timestamp.
                // Placeholder: return empty WAL transfer.
                let records = Self::fetch_wal_since(_storage, *since_timestamp, None);
                if records.is_empty() {
                    ReplResponse::Wal {
                                ...
                                if let Ok(modified) = meta.modified() {
                                    ...
                                    if ts >= _since_timestamp {
                                        if let Some(name) = path.file_name() {
                                            results.push((name.to_string_lossy().to_string(), data));
                                        }
                                    }
                                }
                            }
...
impl ReplicationClient {
    ...
    pub fn request_snapshot(&mut self, last_durable_ts: u64) -> std::io::Result<Vec<u8>> {
        ...
        let req = mgrpc::SnapshotRequest { last_durable_timestamp: last_durable_ts };
    ...
    pub fn request_wal(&mut self, since_ts: u64) -> std::io::Result<Vec<(String, Vec<u8>)>> {
        ...
        let req = WalRequest { epoch_id: self.epoch_id, since_timestamp: since_ts };
        self.rpc.send(&header, &req)?;
...
mod tests {
    ...
    fn test_replication_mode_roundtrip() {
        for mode in [ReplicationMode::Sync, ReplicationMode::Async, ReplicationMode::StrictSync] {
            let v = mode as u8;
    ...
    fn test_delta_stream_roundtrip() {
        ...
        let mut client = ReplicationClient::connect(&addr.to_string(), ReplicationConfig::default()).unwrap();
    ...
    fn test_epoch_validation() {
        ...
        assert!(
            matches!(result, Err(ReplError::EpochMismatch { expected: 7, actual: 5 })),
    ...
    fn test_snapshot_request_serialization() {
        let storage = std::sync::Arc::new(mgstorage::storage::Storage::new());
        let tx = storage.begin_transaction(mgcore::delta::IsolationLevel::SnapshotIsolation);
        storage.create_vertex(&tx, mgcore::types::Gid::from(42u64)).unwrap();
        storage.commit_transaction(&tx);
        ...
        match resp {
            ReplResponse::Snapshot { data } => {
                assert!(!data.is_empty(), "snapshot data should contain serialized vertices");
            }
    ...
    fn test_snapshot_roundtrip_includes_vertices_edges_and_properties() {
        ...
        ReplServer::apply_snapshot_data(&replica, &blob)
    ...
    fn test_strict_sync_majority() {
        ...
        let mut strict = StrictSyncReplicationClient::new(
            vec![sync1, sync2, sync3],
    ...
    fn test_repl_server_handle_heartbeat() {
        ...
        let req = ReplRequest::Heartbeat { epoch_id: 1, lsn: 0 };
        let resp = server.handle_request(&req, &storage);

        match resp {
            ReplResponse::Heartbeat { epoch_id, lsn, main_uuid } => {
    ...
    fn test_delta_gid_all_variants() {
        ...
        assert_eq!(delta_gid(&mgdurability::DeltaRecord::VertexCreate { gid: g, timestamp: 1 }), Some(g));
        assert_eq!(delta_gid(&mgdurability::DeltaRecord::VertexDelete { gid: g }), Some(g));
        assert_eq!(delta_gid(&mgdurability::DeltaRecord::VertexSetProperty { gid: g, key: mgcore::types::PropertyId::from(0u32), value: mgcore::property_value::PropertyValue::Null }), Some(g));
        assert_eq!(delta_gid(&mgdurability::DeltaRecord::VertexAddLabel { gid: g, label: mgcore::types::LabelId::from(1u32) }), Some(g));
        assert_eq!(delta_gid(&mgdurability::DeltaRecord::VertexRemoveLabel { gid: g, label: mgcore::types::LabelId::from(1u32) }), Some(g));
        assert_eq!(delta_gid(&mgdurability::DeltaRecord::EdgeCreate { gid: g, from_vertex: g, to_vertex: g, edge_type: mgcore::types::EdgeTypeId::from(1u32), timestamp: 1 }), Some(g));
        assert_eq!(delta_gid(&mgdurability::DeltaRecord::EdgeDelete { gid: g }), Some(g));
        assert_eq!(delta_gid(&mgdurability::DeltaRecord::EdgeSetProperty { gid: g, key: mgcore::types::PropertyId::from(0u32), value: mgcore::property_value::PropertyValue::Null }), Some(g));
        assert_eq!(delta_gid(&mgdurability::DeltaRecord::TransactionStart { timestamp: 1 }), None);
        assert_eq!(delta_gid(&mgdurability::DeltaRecord::LabelIndexCreate { label: mgcore::types::LabelId::from(1u32) }), None);
    }
    ...
    fn test_repl_error_display() {
        ...
        let e2 = ReplError::EpochMismatch { expected: 1, actual: 2 };
