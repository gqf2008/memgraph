...
use mgcore::types::{Gid, LabelId, PropertyId, EdgeTypeId};
use mgdurability::DeltaRecord;
use mgstorage::storage::Storage;
use mgstorage::transaction::Transaction;

use crate::{ReplicationConfig, ReplicationMode, ReplicaState};
...
impl FailoverDetector {
    ...
    pub fn is_main_failed(&self) -> bool {
        ...
        match self.last_successful_heartbeat {
            Some(last) => {
                Instant::now().duration_since(last) > self.max_heartbeat_interval * self.max_missed_heartbeats
            }
            None => false,
        }
    }
    ...
    pub fn time_since_last_heartbeat(&self) -> Option<Duration> {
        self.last_successful_heartbeat.map(|last| Instant::now().duration_since(last))
    }
}
...
impl ReplicationHandler {
    ...
    pub fn register_replica(
        ...
        let mut replicas = self.replicas.write().map_err(|e| format!("lock poisoned: {}", e))?;
        if replicas.contains_key(&id) {
            return Err(format!("replica '{}' already registered", id));
        }
        replicas.insert(id.clone(), ReplicaConn {
            id,
            rpc,
            mode,
    ...
    pub fn replicate_delta(&self, delta: &DeltaRecord) -> Result<(), String> {
            ...
            match conn.mode {
                ReplicationMode::Async => {
                    if let Err(e) = send_delta_batch(&mut conn.rpc, &batch) {
                        ...
                        Self::update_health_state(&mut conn.health, conn.retry_state.attempt, self.max_failures_before_stale);
                        ...
                        if conn.health.state == ReplicaState::Behind || conn.health.state == ReplicaState::Recovery {
        ...
        for id in to_remove {
            if let Some(conn) = replicas.remove(&id) {
                warn!("Replica '{}' removed after {} consecutive failures", id, conn.health.consecutive_failures);
            }
        }
            ...
            match conn.mode {
                ReplicationMode::Async => {
                    if let Err(e) = send_delta_batch(&mut conn.rpc, batch) {
                        ...
                        conn.health.consecutive_failures = 0;
                        conn.health.last_acknowledged_ts = batch.commit_timestamp;
                        conn.health.replication_lag_ms = now_ms.saturating_sub(batch.commit_timestamp);
                        conn.health.last_heartbeat_ms = now_ms;
                        conn.retry_state.record_success();
                        if conn.health.state == ReplicaState::Behind || conn.health.state == ReplicaState::Recovery {
                            conn.health.state = ReplicaState::Ready;
    ...
    pub fn heartbeat_all(&self) -> Vec<String> {
        ...
        for (id, conn) in replicas.iter_mut() {
            ...
            if conn.retry_state.attempt > 0 && !conn.retry_state.can_retry() {
                conn.health.replication_lag_ms = now_ms.saturating_sub(conn.health.last_acknowledged_ts);
                continue;
            }
        ...
        for id in &stale {
            if let Some(conn) = replicas.remove(id) {
                warn!("Replica '{}' removed after heartbeat failures (consecutive={})", id, conn.health.consecutive_failures);
            }
        }
    ...
    pub fn bump_epoch(&self) {
        let new_epoch = self.epoch_id.fetch_add(1, std::sync::atomic::Ordering::Relaxed) + 1;
    ...
    pub fn replicate_with_quorum(&self, batch: &DeltaBatch) -> Result<(), String> {
        ...
        for id in to_remove {
            if let Some(conn) = replicas.remove(&id) {
                warn!("Replica '{}' removed after quorum failures (consecutive={})", id, conn.health.consecutive_failures);
            }
        }

        // Check quorum for StrictSync
        let strict_sync_count = replicas.values().filter(|r| r.mode == ReplicationMode::StrictSync).count();
        if strict_sync_count > 0 {
            if acks >= majority {
                Ok(())
            } else {
                Err(format!("strict sync quorum failed: {}/{} acks", acks, total))
            }
        } else {
            Ok(())
        }
...
impl ReplicaHandler {
    ...
    pub fn catch_up_wal(&mut self) -> Result<u64, String> {
        ...
        for (_file_name, data) in wal_files {
            ...
            for rec in &records {
                match rec {
                    mgdurability::DeltaRecord::TransactionEnd { commit_timestamp, .. } => {
    ...
    pub fn sync_from_main(&mut self, since_timestamp: u64) -> Result<Option<u64>, String> {
        ...
        if since_timestamp == 0 {
            ...
            let snapshot_data = self.client.as_mut().ok_or("not connected")?
                .request_snapshot(0)
        ...
        match client.heartbeat("unknown") {
            ...
            Err(e) => {
                ...
                if self.failover_detector.is_main_failed() {
                    warn!("Main instance declared failed after {} missed heartbeats", self.failover_detector.max_missed_heartbeats);
                }
...
impl StorageDeltaApplier {
    ...
    {
        let tx = self.storage.begin_transaction(mgcore::delta::IsolationLevel::SnapshotIsolation);
        match f(&tx) {
            Ok(()) => {
        ...
        for delta in &batch.deltas {
            let result = match delta {
                ...
                DeltaRecord::VertexSetProperty { gid, key, value } => {
                    self.with_tx(|tx| self.storage.vertex_set_property(tx, *gid, *key, value.clone()))
                }
                ...
                DeltaRecord::VertexRemoveLabel { gid, label } => {
                    self.with_tx(|tx| self.storage.vertex_remove_label(tx, *gid, *label).map(|_| ()))
                }
                // ── Edge operations ────────────────────────────────────────
                DeltaRecord::EdgeCreate { gid, from_vertex, to_vertex, edge_type, timestamp: _ } => {
                    self.with_tx(|tx| self.storage.create_edge(tx, *gid, *from_vertex, *to_vertex, *edge_type).map(|_| ()))
                }
                DeltaRecord::EdgeDelete { gid } => {
                    self.with_tx(|tx| self.storage.delete_edge(tx, *gid).map(|_| ()))
                }
                DeltaRecord::EdgeSetProperty { gid, key, value } => {
                    self.with_tx(|tx| self.storage.edge_set_property(tx, *gid, *key, value.clone()))
                }
                // ── Transaction boundaries ─────────────────────────────────
                DeltaRecord::TransactionStart { timestamp: _ } => Ok(()),
                DeltaRecord::TransactionEnd { timestamp: _, commit_timestamp: _ } => Ok(()),
                // ── Label indices ──────────────────────────────────────────
                ...
                // ── Edge indices ───────────────────────────────────────────
                DeltaRecord::EdgeIndexCreate { .. } => {
                    warn!("EdgeIndexCreate delta not yet fully supported in Storage");
                    Ok(())
                }
                DeltaRecord::EdgeIndexDrop { .. } => {
                    warn!("EdgeIndexDrop delta not yet fully supported in Storage");
                    Ok(())
                }
                ...
                DeltaRecord::EdgePropertyIndexDrop { .. } => {
                    warn!("EdgePropertyIndexDrop delta not yet fully supported");
                    Ok(())
                }
                ...
                DeltaRecord::GlobalEdgePropertyIndexDrop { .. } => {
                    warn!("GlobalEdgePropertyIndexDrop delta not yet fully supported");
                    Ok(())
                }
                // ── Constraints ────────────────────────────────────────────
                DeltaRecord::ExistenceConstraintCreate { label, property } => {
                    self.storage.constraints.add_existence_constraint(*label, *property);
                    Ok(())
                }
                DeltaRecord::ExistenceConstraintDrop { label, property } => {
                    self.storage.constraints.drop_existence_constraint(*label, *property);
                    Ok(())
                }
                DeltaRecord::UniqueConstraintCreate { label, properties } => {
                    self.storage.constraints.add_unique_constraint(*label, properties.clone());
                    Ok(())
                }
                DeltaRecord::UniqueConstraintDrop { label, properties } => {
                    self.storage.constraints.remove_unique_constraint(*label, properties);
                    Ok(())
                }
                DeltaRecord::TypeConstraintCreate { label, property, type_tag } => {
                    ...
                    self.storage.constraints.add_type_constraint(*label, *property, ct);
                    Ok(())
                }
                DeltaRecord::TypeConstraintDrop { label, property } => {
                    self.storage.constraints.drop_type_constraint(*label, *property);
                    Ok(())
                }
                // ── Point indices ──────────────────────────────────────────
...
mod tests {
    ...
    fn test_delta_batcher_basic() {
        ...
        let d1 = DeltaRecord::VertexCreate { gid: Gid::from(1u64), timestamp: 100 };
    ...
    fn test_delta_batcher_flush_on_capacity() {
        ...
        let d1 = DeltaRecord::VertexCreate { gid: Gid::from(1u64), timestamp: 100 };
        let d2 = DeltaRecord::VertexCreate { gid: Gid::from(2u64), timestamp: 200 };
    ...
    fn test_delta_batcher_epoch() {
        ...
        let d = DeltaRecord::VertexCreate { gid: Gid::from(1u64), timestamp: 100 };
        let d2 = DeltaRecord::VertexCreate { gid: Gid::from(2u64), timestamp: 200 };
        batcher.push(d);
    ...
    fn test_replication_filter_defaults() {
        ...
        let d = DeltaRecord::VertexCreate { gid: Gid::from(1u64), timestamp: 1 };
    ...
    fn test_replication_filter_vertices_only() {
        ...
        let v = DeltaRecord::VertexCreate { gid: Gid::from(1u64), timestamp: 1 };
    ...
    fn test_replica_handler_failover_detector() {
        let storage = Arc::new(Storage::new());
        let handler = ReplicaHandler::new(storage, "127.0.0.1:9999".into(), ReplicationConfig::default());
    ...
    fn test_replica_handler_reconnect_backoff() {
        let storage = Arc::new(Storage::new());
        let mut handler = ReplicaHandler::new(storage, "127.0.0.1:1".into(), ReplicationConfig::default());
        assert!(!handler.is_connected());
    ...
    fn test_delta_applier_all_types() {
        ...
        let batch = DeltaBatch {
            ...
            deltas: vec![
                ...
                DeltaRecord::VertexAddLabel { gid: Gid::from(1u64), label: LabelId::from(1u32) },
                DeltaRecord::VertexSetProperty {
                    gid: Gid::from(1u64),
                    key: PropertyId::from(0u32),
                    value: mgcore::property_value::PropertyValue::Int(42),
                },
                DeltaRecord::LabelIndexCreate { label: LabelId::from(1u32) },
                DeltaRecord::LabelPropertyIndexCreate { label: LabelId::from(1u32), property: PropertyId::from(0u32) },
                DeltaRecord::ExistenceConstraintCreate { label: LabelId::from(1u32), property: PropertyId::from(0u32) },
                DeltaRecord::TtlOperation { label: LabelId::from(1u32), ttl_ms: 3600_000 },
                DeltaRecord::TransactionStart { timestamp: 100 },
                DeltaRecord::TransactionEnd { timestamp: 100, commit_timestamp: 101 },
            ],
        };
    ...
    fn test_delta_applier_edge_operations() {
        ...
        let batch = DeltaBatch {
            ...
            deltas: vec![
                ...
                DeltaRecord::EdgeDelete { gid: Gid::from(10u64) },
            ],
        };
    ...
    fn test_delta_applier_constraints() {
        ...
        assert!(storage.constraints.has_unique_constraint(LabelId::from(1u32), &[PropertyId::from(0u32)]));
        assert!(storage.constraints.has_type_constraint(LabelId::from(1u32), PropertyId::from(0u32)));
    }
    ...
    fn test_delta_applier_constraint_drops() {
        ...
        let create_batch = DeltaBatch {
            ...
            deltas: vec![
                DeltaRecord::ExistenceConstraintCreate { label: LabelId::from(1u32), property: PropertyId::from(0u32) },
                DeltaRecord::UniqueConstraintCreate { label: LabelId::from(1u32), properties: vec![PropertyId::from(0u32)] },
                DeltaRecord::TypeConstraintCreate { label: LabelId::from(1u32), property: PropertyId::from(0u32), type_tag: 0 },
            ],
        };
        ...
        let drop_batch = DeltaBatch {
            ...
            sequence_number: 0,
            deltas: vec![
                DeltaRecord::ExistenceConstraintDrop { label: LabelId::from(1u32), property: PropertyId::from(0u32) },
                DeltaRecord::UniqueConstraintDrop { label: LabelId::from(1u32), properties: vec![PropertyId::from(0u32)] },
                DeltaRecord::TypeConstraintDrop { label: LabelId::from(1u32), property: PropertyId::from(0u32) },
            ],
        };
        applier.apply_batch(&drop_batch).unwrap();

        assert!(!storage.constraints.has_existence_constraint(LabelId::from(1u32), PropertyId::from(0u32)));
        assert!(!storage.constraints.has_unique_constraint(LabelId::from(1u32), &[PropertyId::from(0u32)]));
        assert!(!storage.constraints.has_type_constraint(LabelId::from(1u32), PropertyId::from(0u32)));
    }
    ...
    fn test_delta_applier_label_index_drop() {
        ...
        let drop = DeltaBatch {
            ...
            sequence_number: 0,
            deltas: vec![
                DeltaRecord::LabelIndexDrop { label: LabelId::from(1u32) },
            ],
        };
    ...
    fn test_sync_from_main_applies_snapshot() {
        ...
        let main = Arc::new(Storage::new());
        {
            ...
            main.vertex_add_label(&tx, Gid::from(1u64), LabelId::from(10u32)).unwrap();
        ...
        let server = crate::ReplServer::bind("127.0.0.1:0", crate::ReplicationConfig::default()).unwrap();
        ...
        let handle = thread::spawn(move || {
            ...
            peer.send(&resp_hdr, &mgrpc::SnapshotData { data: blob }).unwrap();

            // DeltaStream request
            let (_hdr, _req): (mgrpc::MessageHeader, mgrpc::DeltaStreamRequest) = peer.recv().unwrap();
        ...
        let mut handler = ReplicaHandler::new(replica.clone(), addr, crate::ReplicationConfig::default());
    ...
    fn test_catch_up_wal_applies_records() {
        ...
        let storage = Arc::new(Storage::new());
        let mut handler = ReplicaHandler::new(storage.clone(), addr, crate::ReplicationConfig::default());
        handler.connect().expect("connect to mock main");
