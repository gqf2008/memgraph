...
impl Message {
    ...
    pub fn to_value(&self) -> Value {
        match self {
            ...
            Message::Pull { n, qid } => Value::Struct(
                self.signature(),
                vec![
                    Value::Map({
            ...
            Message::Discard { n, qid } => Value::Struct(
                self.signature(),
                vec![
                    Value::Map({
                        let mut m = HashMap::new();
            ...
            Message::Route {
                ...
                meta.insert(
                    "bookmarks".into(),
                    Value::List(bookmarks.iter().map(|b| Value::String(b.clone())).collect()),
                );
                for (k, v) in extra {
                    meta.insert(k.clone(), v.clone());
                }
...
mod tests {
    ...
    fn test_route_to_value() {
        ...
        routing.insert(
            "address".into(),
            Value::String("bolt://localhost:7687".into()),
        );
