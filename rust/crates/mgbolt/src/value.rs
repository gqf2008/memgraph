...
fn encode_string(w: &mut impl Write, s: &str) -> std::io::Result<()> {
    encode_size_prefix(w, s.len(), MARKER_TINY_STRING, MARKER_STRING8, MARKER_STRING16, MARKER_STRING32)?;
    w.write_all(s.as_bytes())
}

fn encode_bytes(w: &mut impl Write, b: &[u8]) -> std::io::Result<()> {
    encode_size_prefix(w, b.len(), 0x00, MARKER_BYTES8, MARKER_BYTES16, MARKER_BYTES32)?;
    w.write_all(b)
}

fn encode_list(w: &mut impl Write, items: &[Value]) -> std::io::Result<()> {
    encode_size_prefix(w, items.len(), MARKER_TINY_LIST, MARKER_LIST8, MARKER_LIST16, MARKER_LIST32)?;
    for item in items {
        item.encode(w)?;
    }
    Ok(())
...
fn encode_map(w: &mut impl Write, entries: &HashMap<String, Value>) -> std::io::Result<()> {
    encode_size_prefix(w, entries.len(), MARKER_TINY_MAP, MARKER_MAP8, MARKER_MAP16, MARKER_MAP32)?;
...
mod tests {
    ...
    fn test_encode_date() {
        let mut buf = Vec::new();
        Value::date(Date::from_days(19000)).encode(&mut buf).unwrap();
    ...
    fn test_encode_duration_layout() {
        ...
        let mut buf = Vec::new();
        Value::duration(Duration::new(0, 0, 0)).encode(&mut buf).unwrap();
