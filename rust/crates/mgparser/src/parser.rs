...
impl<'a> Parser<'a> {
    ...
    fn is_keyword(tok: &Token) -> bool {
        !matches!(tok,
            Token::Identifier(_) | Token::StringLiteral(_) | Token::IntLiteral(_) | Token::FloatLiteral(_)
            | Token::LParen | Token::RParen | Token::LBracket | Token::RBracket | Token::LBrace | Token::RBrace
            | Token::Comma | Token::Colon | Token::Semicolon | Token::Dot | Token::DotDot
            | Token::Plus | Token::Minus | Token::Star | Token::Slash | Token::Percent
            | Token::Eq | Token::Neq | Token::Lt | Token::Gt | Token::Lte | Token::Gte
            | Token::ArrowRight | Token::ArrowLeft | Token::PlusEq | Token::Tilde | Token::Pipe
        )
    }
    ...
    fn resolve_label(&self, name: &str) -> LabelId {
        self.catalog.map(|c| c.label(name)).unwrap_or(LabelId::from_uint(0))
    }

    fn resolve_property(&self, name: &str) -> PropertyId {
        self.catalog.map(|c| c.property(name)).unwrap_or(PropertyId::from_uint(0))
    }

    fn resolve_edge_type(&self, name: &str) -> EdgeTypeId {
        self.catalog.map(|c| c.edge_type(name)).unwrap_or(EdgeTypeId::from_uint(0))
    }
    ...
    fn parse_query(&mut self) -> Result<Query, ParseError> {
        ...
        while self.match_keyword(Token::Using) {
            if self.match_keyword(Token::Periodic) {
                ...
                let label_name = self.advance_name()
                ...
                let property = if self.match_keyword(Token::LParen) {
                    let prop_name = self.advance_name()
                ...
                return Err(self.error("expected PERIODIC COMMIT, HOPS LIMIT, or INDEX after USING"));
            }
        }
        ...
        let union = if self.match_keyword(Token::Union) {
            ...
            Some(UnionQuery { right: Box::new(right), all })
        } else {
            None
        };
        Ok(Query { clauses, union, mode, periodic_commit, hops_limit, index_hints })
    }
        ...
        match self.peek() {
            ...
            Some(Token::Load) => {
                ...
                match self.peek() {
                    ...
                    _ => return Err(self.error("expected CSV or JSONL after LOAD")),
                }
            }
            Some(Token::Drop) => {
                match self.peek_n(1) {
    ...
    fn parse_match(&mut self) -> Result<Clause, ParseError> {
        ...
        Ok(Clause::Match { pattern, where_clause })
    }
    ...
    fn parse_optional_match(&mut self) -> Result<Clause, ParseError> {
        ...
        Ok(Clause::OptionalMatch { pattern, where_clause })
    }
    ...
    fn parse_merge(&mut self) -> Result<Clause, ParseError> {
        ...
        Ok(Clause::Merge { pattern: MergePattern { pattern, on_create, on_match } })
    }
    ...
    fn parse_delete(&mut self) -> Result<Clause, ParseError> {
        ...
        Ok(Clause::Delete { expressions, detach })
    }
    ...
    fn parse_set_item(&mut self) -> Result<SetItem, ParseError> {
        ...
        if self.match_keyword(Token::PlusEq) {
            ...
            if let Expression::Identifier(alias) = left {
                Ok(SetItem::VariableUpdate { alias, expression: expr })
            } else {
            ...
            match left {
                Expression::Property { object, key } => {
                    if let Expression::Identifier(alias) = *object {
                        Ok(SetItem::Property { expression: Expression::Identifier(alias), key, value: expr })
                    } else {
                        Err(self.error("SET property must be on an identifier"))
                    }
                }
                Expression::Identifier(alias) => Ok(SetItem::Variable { alias, expression: expr }),
    ...
    fn parse_set_left(&mut self) -> Result<Expression, ParseError> {
        ...
        loop {
            match self.peek() {
                Some(Token::Dot) => {
                    self.advance();
                    let name = self.advance_name().ok_or_else(|| self.error("expected property name after ."))?;
                    let key = self.resolve_property(&name);
                    expr = Expression::Property { object: Box::new(expr), key };
                }
                Some(Token::Colon) => {
                    self.advance();
                    let name = self.advance_name().ok_or_else(|| self.error("expected label name after :"))?;
                    let label = self.resolve_label(&name);
                    expr = Expression::Label { object: Box::new(expr), label };
                }
                _ => break,
            }
        }
    ...
    fn parse_remove_item(&mut self) -> Result<RemoveItem, ParseError> {
        ...
        match expr {
            Expression::Property { object, key } => {
                if let Expression::Identifier(alias) = *object {
                    Ok(RemoveItem::Property { expression: Expression::Identifier(alias), key })
                } else {
    ...
    fn parse_return(&mut self) -> Result<Clause, ParseError> {
        ...
        Ok(Clause::Return { items, distinct, all })
    }
    ...
    fn parse_with(&mut self) -> Result<Clause, ParseError> {
        ...
        Ok(Clause::With { items, where_clause })
    }
    ...
    fn parse_unwind(&mut self) -> Result<Clause, ParseError> {
        ...
        let alias = self.advance_name().ok_or_else(|| self.error("expected alias after AS"))?;
    ...
    fn parse_order_by_item(&mut self) -> Result<OrderByItem, ParseError> {
        ...
        let ascending = if self.match_keyword(Token::Asc) {
            true
        } else if self.match_keyword(Token::Desc) {
            false
        } else {
            true
        };
        Ok(OrderByItem { expression, ascending })
    }
    ...
    fn parse_call(&mut self) -> Result<Clause, ParseError> {
        ...
        if self.match_keyword(Token::LBrace) {
            ...
            return Ok(Clause::CallSubquery { query: Query { clauses, union: None, mode: QueryMode::Standard, periodic_commit: None, hops_limit: None, index_hints: Vec::new() }, in_transactions });
        }
        // Procedure name can be dotted: algo.triangle_count
        let mut procedure_name = self.advance_name()
            .ok_or_else(|| self.error("expected procedure name"))?;
        while self.match_keyword(Token::Dot) {
            let part = self.advance_name()
        ...
        Ok(Clause::Call { procedure_name, arguments, yield_items, yield_all })
    }
    ...
    fn parse_foreach(&mut self) -> Result<Clause, ParseError> {
        ...
        self.expect(Token::LParen)?;
        let variable = self.advance_name()
        ...
        self.expect(Token::RParen)?;
        Ok(Clause::Foreach { variable, list, clauses })
    }
    ...
    fn parse_load_csv_rest(&mut self) -> Result<Clause, ParseError> {
        ...
        Ok(Clause::LoadCsv { url, with_headers, alias })
    }
    ...
    fn parse_constraint_kind(&mut self) -> Result<ConstraintKind, ParseError> {
        if self.match_keyword(Token::Unique) {
            ...
            let expected = self.advance_name()
    ...
    fn parse_create_ddl(&mut self) -> Result<Clause, ParseError> {
        ...
        if self.match_keyword(Token::Index) {
            ...
            Ok(Clause::CreateConstraint { label, property, constraint_type })
        } else {
    ...
    fn parse_drop(&mut self) -> Result<Clause, ParseError> {
        ...
        if self.match_keyword(Token::Index) {
            ...
            self.expect(Token::Is)?;
            let constraint_type = self.parse_constraint_kind()?;
            Ok(Clause::DropConstraint { label, property, constraint_type })
        } else {
    ...
    fn parse_show(&mut self) -> Result<Clause, ParseError> {
        ...
        match upper.as_str() {
            "DATABASES" => Ok(Clause::Show { show_type: ShowType::Databases }),
            "INDEXES" => Ok(Clause::Show { show_type: ShowType::Indexes }),
            "CONSTRAINTS" => Ok(Clause::Show { show_type: ShowType::Constraints }),
            "TRIGGERS" => Ok(Clause::Show { show_type: ShowType::Triggers }),
            "NODE_LABELS" => {
                let info = self.advance_name().ok_or_else(|| self.error("expected INFO after SHOW NODE_LABELS"))?;
                if !info.eq_ignore_ascii_case("INFO") {
                    return Err(self.error("expected INFO after SHOW NODE_LABELS"));
                }
                Ok(Clause::Show { show_type: ShowType::NodeLabels })
            }
            "EDGE_TYPES" => {
                let info = self.advance_name().ok_or_else(|| self.error("expected INFO after SHOW EDGE_TYPES"))?;
                if !info.eq_ignore_ascii_case("INFO") {
                    return Err(self.error("expected INFO after SHOW EDGE_TYPES"));
                }
                Ok(Clause::Show { show_type: ShowType::EdgeTypes })
            }
            "USERS" => Ok(Clause::ShowAuth { auth_type: ShowAuthType::Users }),
            "ROLES" => Ok(Clause::ShowAuth { auth_type: ShowAuthType::Roles }),
            "TRANSACTIONS" => Ok(Clause::ShowTransactions),
            "PRIVILEGES" => {
                let _ = self.match_keyword(Token::For);
                let (target_name, is_user) = self.parse_grant_target()?;
                Ok(Clause::ShowPrivileges { target_name, is_user })
            }
            "DATABASE" => {
                match self.peek() {
                    Some(Token::Settings) => {
                    ...
                    Some(Token::Setting) => {
                        self.advance();
                        let setting_name = self.advance_name()
                            .ok_or_else(|| self.error("expected setting name after SHOW DATABASE SETTING"))?;
    ...
    fn parse_create_user(&mut self) -> Result<Clause, ParseError> {
        ...
        let username = self.advance_name()
    ...
    fn parse_create_role(&mut self) -> Result<Clause, ParseError> {
        ...
        let role_name = self.advance_name()
    ...
    fn parse_grant(&mut self) -> Result<Clause, ParseError> {
        ...
        if self.match_keyword(Token::All) {
            ...
            self.expect(Token::To)?;
            let (target_name, is_user) = self.parse_grant_target()?;
            Ok(Clause::GrantPrivilege { privileges, target_name, is_user })
        } else {
            ...
            let username = self.advance_name()
                .ok_or_else(|| self.error("expected username"))?;
            Ok(Clause::GrantRole { role_name, username })
        }
    }
    ...
    fn parse_revoke(&mut self) -> Result<Clause, ParseError> {
        ...
        if self.match_keyword(Token::All) {
            ...
            self.expect(Token::From)?;
            let (target_name, is_user) = self.parse_revoke_target()?;
            Ok(Clause::RevokePrivilege { privileges, target_name, is_user })
        } else {
            let role_name = self.advance_name()
                .ok_or_else(|| self.error("expected role name"))?;
            self.expect(Token::From)?;
            let username = self.advance_name()
                .ok_or_else(|| self.error("expected username"))?;
            Ok(Clause::RevokeRole { role_name, username })
        }
    }
    ...
    fn parse_deny_privilege(&mut self) -> Result<Clause, ParseError> {
        ...
        self.expect(Token::To)?;
        let (target_name, is_user) = self.parse_grant_target()?;
        Ok(Clause::DenyPrivilege { privileges, target_name, is_user })
    }
    ...
    fn parse_alter_user(&mut self) -> Result<Clause, ParseError> {
        ...
        let action = if self.match_keyword(Token::Set) {
            ...
            let pw = match self.peek() {
                Some(Token::StringLiteral(s)) => { let v = s.clone(); self.advance(); v }
            ...
            self.expect(Token::To)?;
            let new_name = self.advance_name()
    ...
    fn parse_set_storage_mode(&mut self) -> Result<Clause, ParseError> {
        ...
        let mode = if self.match_keyword(Token::InMemoryAnalytical) {
            ...
            return Err(self.error("expected IN_MEMORY_ANALYTICAL, IN_MEMORY_TRANSACTIONAL, or ON_DISK_TRANSACTIONAL"));
        };
    ...
    fn parse_terminate_transaction(&mut self) -> Result<Clause, ParseError> {
        ...
        let transaction_id = match self.peek() {
            Some(Token::StringLiteral(s)) => { let id = s.clone(); self.advance(); id }
            _ => self.advance_name()
    ...
    fn parse_create_trigger(&mut self) -> Result<Clause, ParseError> {
        ...
        let target = match self.peek() {
            Some(Token::Vertex) => { self.advance(); TriggerTarget::Vertex }
            Some(Token::Edge) => { self.advance(); TriggerTarget::Edge }
            _ => return Err(self.error("expected VERTEX or EDGE after ON")),
        };

        // Parse event: CREATE, DELETE, UPDATE
        let event = match self.peek() {
            Some(Token::Create) => { self.advance(); TriggerEvent::Create }
            Some(Token::Delete) => { self.advance(); TriggerEvent::Delete }
            Some(Token::Set) => { self.advance(); TriggerEvent::Update }
        ...
        let timing = match self.peek() {
            Some(Token::Before) => { self.advance(); TriggerTiming::Before }
            Some(Token::After) => { self.advance(); TriggerTiming::After }
        ...
        Ok(Clause::CreateTrigger { name, target, event, timing, statement })
    }
    ...
    fn parse_match_pattern_element(&mut self) -> Result<PatternElement, ParseError> {
        ...
        } else if self.match_keyword(Token::AllShortestPaths) {
            self.expect(Token::LParen)?;
            Some(PathAlgorithm::AllShortest)
        } else {
            None
        ...
        while self.peek() == Some(&Token::Minus) || self.peek() == Some(&Token::ArrowLeft) {
            ...
            if shortest_algo.is_some() {
                edge.path_algorithm = shortest_algo.unwrap();
            }
        ...
        Ok(PatternElement { path_alias, node, edges })
    }
    ...
    fn parse_pattern_element(&mut self) -> Result<PatternElement, ParseError> {
        ...
        Ok(PatternElement { path_alias: None, node, edges })
    }
    ...
    fn parse_edge_and_node(&mut self) -> Result<(EdgePattern, NodePattern), ParseError> {
        if self.match_keyword(Token::ArrowLeft) {
            ...
            let edge = if self.peek() == Some(&Token::LBracket) {
                ...
                // Anonymous left edge: <--
                EdgePattern { alias: None, edge_types: Vec::new(), properties: Vec::new(), direction: Direction::Left, var_length: None, path_algorithm: PathAlgorithm::Default, kshortest_limit: None }
            };
            ...
            let mut edge = if self.peek() == Some(&Token::LBracket) {
                ...
                EdgePattern { alias: None, edge_types: Vec::new(), properties: Vec::new(), direction: Direction::Right, var_length: None, path_algorithm: PathAlgorithm::Default, kshortest_limit: None }
            };
    ...
    fn parse_edge_pattern(&mut self) -> Result<EdgePattern, ParseError> {
        ...
        Ok(EdgePattern { alias, edge_types, properties, direction: Direction::Right, var_length, path_algorithm, kshortest_limit })
    }
        ...
        loop {
            if self.match_keyword(Token::Dot) {
                ...
                if is_func {
                    ...
                    self.expect(Token::RParen)?;
                    expr = Expression::Function { name, arguments, distinct: false };
                    continue;
                }
                let name = self.advance_name()
                ...
                if self.match_keyword(Token::DotDot) {
                    ...
                    self.expect(Token::RBracket)?;
                    expr = Expression::Index { object: Box::new(expr), index: Box::new(start) };
                }
    ...
    fn parse_primary(&mut self) -> Result<Expression, ParseError> {
        match self.peek() {
            ...
            | Some(Token::Filter) | Some(Token::Extract) | Some(Token::Reduce) => {
                return self.parse_list_predicate_or_function();
            }
            Some(Token::Exists) if self.peek_n(1) == Some(&Token::LParen) && self.peek_n(2) == Some(&Token::LParen) => {
                ...
                self.expect(Token::RParen)?;
                let query = Query {
                    clauses: vec![Clause::Match { pattern, where_clause: None }],
                    union: None,
            ...
            Some(Token::Identifier(name)) if name.eq_ignore_ascii_case("count") && self.peek_n(1) == Some(&Token::LBrace) => {
                // COUNT { subquery }
                self.advance();
                self.expect(Token::LBrace)?;
                let query = self.parse_exists_subquery()?;
            ...
            Some(Token::Identifier(name)) => {
                ...
                if self.peek() == Some(&Token::LParen) {
                    ...
                    self.expect(Token::RParen)?;
                    Ok(Expression::Function { name, arguments, distinct })
                } else {
            ...
            Some(tok) if Self::is_keyword(tok) && self.peek_n(1) == Some(&Token::LParen) => {
                ...
                self.expect(Token::RParen)?;
                Ok(Expression::Function { name, arguments, distinct: false })
            }
                    ...
                    if self.peek_n(1) == Some(&Token::Dot) {
                        ...
                        loop {
                            ...
                            if self.match_keyword(Token::Star) {
                                ...
                                extra.push((prop_name.clone(), Expression::Property {
                        ...
                        let object = if let Some((_, Expression::Property { object, .. })) = extra.first() {
                            object.clone()
    ...
    fn parse_list_predicate_or_function(&mut self) -> Result<Expression, ParseError> {
        ...
        if name.eq_ignore_ascii_case("REDUCE") {
            let accumulator = self.advance_name()
            ...
            self.expect(Token::RParen)?;
            return Ok(Expression::Reduce { accumulator, initial, variable, list, expression });
        }
        ...
        if name.eq_ignore_ascii_case("EXTRACT") {
            ...
            let expression = Box::new(self.parse_expression()?);
            self.expect(Token::RParen)?;
            return Ok(Expression::Extract { variable, list, expression });
        }
        ...
        match name.to_ascii_uppercase().as_str() {
            "ALL" => Ok(Expression::All { variable, list, predicate }),
            "ANY" => Ok(Expression::Any { variable, list, predicate }),
            "NONE" => Ok(Expression::None { variable, list, predicate }),
            "SINGLE" => Ok(Expression::Single { variable, list, predicate }),
            "FILTER" => Ok(Expression::Filter { variable, list, predicate }),
    ...
    fn parse_pattern_comprehension(&mut self) -> Result<Expression, ParseError> {
        ...
        let expression = Box::new(self.parse_expression()?);
        self.expect(Token::RBracket)?;
        Ok(Expression::PatternComprehension { pattern, where_clause, expression })
    }
        ...
        match (predicate, expression) {
            (Some(pred), Some(expr)) => {
                ...
                Ok(Expression::Extract {
                    variable: var.clone(),
                    list: Box::new(Expression::Filter { variable: var, list, predicate: pred }),
                    expression: expr,
                })
            }
            (Some(pred), None) => Ok(Expression::Filter { variable, list, predicate: pred }),
            (None, Some(expr)) => Ok(Expression::Extract { variable, list, expression: expr }),
    ...
    fn parse_exists_subquery(&mut self) -> Result<Query, ParseError> {
        ...
        if self.match_keyword(Token::Return) {
            ...
            clauses.push(Clause::Return { items, distinct, all });
        }
        Ok(Query { clauses, union: None, mode: QueryMode::Standard, periodic_commit: None, hops_limit: None, index_hints: Vec::new() })
    }
    ...
    fn parse_case_expression(&mut self) -> Result<Expression, ParseError> {
        ...
        Ok(Expression::Case { expression, whens, else_branch })
    }
...
pub fn parse_query_with_catalog(input: &str, catalog: Option<&Catalog>) -> Result<Query, ParseError> {
    let mut lexer = Lexer::new(input);
    let tokens = lexer.tokenize();
...
mod tests {
    ...
    fn test_parse_exists() {
        let q = parse_query("MATCH (n) WHERE EXISTS { MATCH (n)-[:KNOWS]->(:Person) } RETURN n").unwrap();
        assert_eq!(q.clauses.len(), 2);
    }
    ...
    #[test]
    fn test_parse_union() {
        let q = parse_query("MATCH (n:Person) RETURN n.name AS name UNION MATCH (n:Employee) RETURN n.name AS name").unwrap();
    ...
    fn test_parse_reduce() {
        ...
        match &q.clauses[0] {
            Clause::Return { items, .. } => {
                match &items[0].expression {
                    Expression::Reduce { accumulator, variable, list, expression, .. } => {
