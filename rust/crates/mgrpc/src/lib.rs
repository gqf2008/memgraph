...
use mgslk::{slk_encode, Builder, Reader, SlkDecodeError, SlkLoad, SlkSave};
...
impl SlkLoad for ProtocolVersion {
    fn slk_load(reader: &mut Reader) -> Result<Self, SlkDecodeError> {
        ...
        Self::from_u8(v)
            .ok_or_else(|| SlkDecodeError::from(format!("unknown protocol version: {}", v)))
    }
}
...
pub trait RpcHandler {
    ...
    fn handle_delta_stream_request(
        &mut self,
...
mod tests {
    ...
    fn test_dispatch_unknown_message() {
        ...
        impl RpcHandler for DummyHandler {
            fn handle_heartbeat(&mut self, _hb: Heartbeat) -> Result<(), String> {
                Ok(())
            }
            fn handle_delta_stream_request(
                &mut self,
                _req: DeltaStreamRequest,
            ) -> Result<DeltaBatch, String> {
