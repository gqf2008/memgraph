...
impl Glue {
    ...
    pub fn execute_query(
        ...
        if self.auth.is_enabled() {
            if let Some(ref user) = session.user {
                ...
                self.auth
                    .check_permission(&user.username, &user.role, None, op)
                    .map_err(|e| format!("{}", e))?;
            }
        }
        ...
        let result = self.run_with_timeout(query_str, parameters, session.query_timeout);

        // Remove from active queries
        self.active_queries
            .write()
            .unwrap()
            .retain(|q| q.session_id != session_id || q.query_text != query_str);
    ...
    fn run_with_timeout(
        ...
        let result = if let Some(ref dbms) = self.dbms {
            ...
            mginterp::execute_with_catalog(&self.storage, query_str, Some(&self.catalog))
                .map_err(|e| format!("{}", e))?
        };
    ...
    pub fn pull_cursor(
        ...
        cursor_id: u64,
        n: usize,
    ) -> Option<(Vec<String>, Vec<HashMap<String, PropertyValue>>)> {
    ...
    pub fn get_cursor_summary(&self, cursor_id: u64) -> Option<ExecutionSummary> {
        ...
        cursors.get(&cursor_id).map(|c| c.summary.clone())
    }
    ...
    pub fn cleanup_idle_sessions(&self, max_idle: Duration) -> usize {
        ...
        self.active_queries
            .write()
            .unwrap()
            .retain(|q| !to_remove.contains(&q.session_id));
        to_remove.len()
    }
...
mod tests {
    ...
    fn test_session_auth() {
        ...
        glue.auth.add_user(
            "testuser",
            &mgauth::hash_password("testpass"),
            Role::ReadWrite,
        );

        let sid = glue.create_session();
        glue.authenticate_session(sid, "testuser", "testpass")
            .unwrap();
    ...
    fn test_cursor_streaming() {
        ...
        // Create some data
        glue.execute_query(sid, "CREATE (n:Person {name:'Alice'})", &HashMap::new())
            .unwrap();
        glue.execute_query(sid, "CREATE (n:Person {name:'Bob'})", &HashMap::new())
            .unwrap();
    ...
    fn test_discard_cursor() {
        ...
        glue.execute_query(sid, "CREATE (n:Person {name:'Alice'})", &HashMap::new())
            .unwrap();
        let cursor_id = glue
            .execute_query_streaming(sid, "MATCH (n) RETURN n", &HashMap::new())
            .unwrap();
    ...
    fn test_session_stats() {
        ...
        let sid = glue.create_session();
        glue.authenticate_session(sid, "anon", "").unwrap();
        glue.execute_query(sid, "CREATE (n)", &HashMap::new())
            .unwrap();
    ...
    fn test_readonly_cannot_write() {
        ...
        glue.auth.set_enabled(true);
        glue.auth
            .add_user("reader", &mgauth::hash_password("pw"), Role::ReadOnly);
    ...
    fn test_batch_execution() {
        ...
        let queries = vec![
            ...
            (
                "MATCH (n:Person) RETURN n.name AS name".into(),
                HashMap::new(),
            ),
        ];
