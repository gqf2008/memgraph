...
#[command(
    name = "memgraph",
    version,
    about = "Memgraph - in-memory graph database"
)]
pub struct Flags {
    // ─── General ───────────────────────────────────────────────────────
...
impl Flags {
    ...
    fn merge(&mut self, other: Flags) {
        ...
        if !other.data_directory.is_empty() {
            self.data_directory = other.data_directory;
        }
        if let Some(cf) = other.config_file {
            self.config_file = Some(cf);
        }
        if other.storage_mode != "IN_MEMORY_TRANSACTIONAL" {
            self.storage_mode = other.storage_mode;
        }
        if other.bolt_port != 7687 {
            self.bolt_port = other.bolt_port;
        }
        if other.bolt_server_address != "0.0.0.0" {
            self.bolt_server_address = other.bolt_server_address;
        }
        if other.bolt_max_connections != 100 {
            self.bolt_max_connections = other.bolt_max_connections;
        }
        if let Some(cert) = other.bolt_cert_file {
            self.bolt_cert_file = Some(cert);
        }
        if let Some(key) = other.bolt_key_file {
            self.bolt_key_file = Some(key);
        }
        if other.auth_enabled {
            self.auth_enabled = true;
        }
        if let Some(user) = other.bolt_user {
            self.bolt_user = Some(user);
        }
        if let Some(pass) = other.bolt_pass {
            self.bolt_pass = Some(pass);
        }
        if other.wal_enabled {
            self.wal_enabled = true;
        }
        if other.snapshot_interval_secs != 300 {
            self.snapshot_interval_secs = other.snapshot_interval_secs;
        }
        if other.snapshot_on_exit {
            self.snapshot_on_exit = true;
        }
        if other.replication_role != "MAIN" {
            self.replication_role = other.replication_role;
        }
        if other.replication_port != 10000 {
            self.replication_port = other.replication_port;
        }
        if other.coordinator_port != 12000 {
            self.coordinator_port = other.coordinator_port;
        }
        if other.log_level != "INFO" {
            self.log_level = other.log_level;
        }
        if other.log_to_stderr {
            self.log_to_stderr = true;
        }
        if other.telemetry_enabled {
            self.telemetry_enabled = true;
        }
        if other.metrics_port != 9091 {
            self.metrics_port = other.metrics_port;
        }
        if other.query_execution_timeout_secs != 600 {
            self.query_execution_timeout_secs = other.query_execution_timeout_secs;
        }
        if other.memory_limit != 0 {
            self.memory_limit = other.memory_limit;
        }
        if other.gc_interval_secs != 60 {
            self.gc_interval_secs = other.gc_interval_secs;
        }
        if other.ttl_interval_secs != 60 {
            self.ttl_interval_secs = other.ttl_interval_secs;
        }
        if other.query_plan_cache_size != 1024 {
            self.query_plan_cache_size = other.query_plan_cache_size;
        }
        if other.websocket_enabled {
            self.websocket_enabled = true;
        }
        if other.websocket_port != 7444 {
            self.websocket_port = other.websocket_port;
        }
        if other.websocket_address != "0.0.0.0" {
    ...
    pub fn list_settings(&self) -> Vec<(String, String)> {
        vec![
            ...
            (
                "bolt-server-address".into(),
                self.bolt_server_address.clone(),
            ),
            (
                "bolt-max-connections".into(),
                self.bolt_max_connections.to_string(),
            ),
            ...
            (
                ...
                self.telemetry_enabled.to_string(),
            ),
            (
                "query-execution-timeout-secs".into(),
                self.query_execution_timeout_secs.to_string(),
            ),
            ("query-profiling".into(), self.query_profiling.to_string()),
            (
                "query-plan-cache-enabled".into(),
                self.query_plan_cache_enabled.to_string(),
            ),
            (
                "query-plan-cache-size".into(),
                self.query_plan_cache_size.to_string(),
            ),
            ("metrics-address".into(), self.metrics_address.clone()),
            ("metrics-port".into(), self.metrics_port.to_string()),
            (
                "websocket-enabled".into(),
                self.websocket_enabled.to_string(),
            ),
    ...
    pub fn set_setting(&mut self, name: &str, value: &str) -> Result<(), String> {
        match name.to_ascii_lowercase().as_str() {
            ...
            "bolt-max-connections" => {
                self.bolt_max_connections =
                    value.parse().map_err(|e| format!("invalid count: {}", e))?;
            }
            "memory-limit" => {
                self.memory_limit = value.parse().map_err(|e| format!("invalid limit: {}", e))?;
            }
            "memory-warning-threshold" => {
                self.memory_warning_threshold = value
                    .parse()
            ...
            "snapshot-on-exit" => {
                self.snapshot_on_exit =
                    value.parse().map_err(|e| format!("invalid bool: {}", e))?;
            }
            "gc-interval-secs" => {
                self.gc_interval_secs = value
                    .parse()
                    .map_err(|e| format!("invalid seconds: {}", e))?;
            }
            "ttl-interval-secs" => {
                self.ttl_interval_secs = value
                    .parse()
                    .map_err(|e| format!("invalid seconds: {}", e))?;
            }
            "log-level" => {
            ...
            "query-execution-timeout-secs" => {
                self.query_execution_timeout_secs = value
                    .parse()
                    .map_err(|e| format!("invalid seconds: {}", e))?;
            }
            "query-profiling" => {
            ...
            "query-plan-cache-enabled" => {
                self.query_plan_cache_enabled =
                    value.parse().map_err(|e| format!("invalid bool: {}", e))?;
            }
            "query-plan-cache-size" => {
                self.query_plan_cache_size =
                    value.parse().map_err(|e| format!("invalid size: {}", e))?;
            }
            "metrics-port" => {
            ...
            _ => {
                return Err(format!(
                    "setting '{}' is unknown or not mutable at runtime",
                    name
                ))
            }
        }
