...
impl Catalog {
    ...
    pub fn label(&self, name: &str) -> LabelId {
        ...
        drop(mapper);
        // NameIdMapper uses interior mutability; read lock is sufficient.
        let mapper = self.labels.read().unwrap();
    ...
    pub fn dump_mappings(
        ...
        Vec<(String, LabelId)>,
        Vec<(String, PropertyId)>,
        Vec<(String, EdgeTypeId)>,
    ) {
        (
    ...
    pub fn load_schema_state(&self, state: &SchemaState) {
        ...
        *self.procedures.write().unwrap() = state
            .procedures
            .iter()
