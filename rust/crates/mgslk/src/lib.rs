...
pub fn is_cpp_version(v: u64) -> bool {
    v >= CPP_OLDEST_VERSION && v <= CPP_NEWEST_VERSION
}
...
mod tests {
    ...
    fn test_varint_roundtrip() {
        let values = vec![
            0u64, 1, 127, 128, 255, 256, 16383, 16384,
    ...
    fn test_slk_encode_decode_nested() {
        let val: Vec<Option<(i32, String)>> = vec![
            Some((1, "one".into())),
            None,
            Some((3, "three".into())),
        ];
    ...
    fn test_random_roundtrip_10k() {
        ...
        for n in 0..10_000u64 {
            let val: Vec<Option<(i64, String)>> = (0..(fuzz_lcg(&mut seed) % 20) as usize)
                ...
                .collect();
            let bytes = slk_to_bytes(&val);
            let back: Vec<Option<(i64, String)>> =
                slk_from_bytes(&bytes).unwrap_or_else(|e| {
                    panic!("deserialize failed at iteration {}: {}", n, e)
                });
...
pub fn decode_varint(data: &[u8]) -> Result<(u64, usize), SlkDecodeError> {
    ...
    for (i, &byte) in data.iter().enumerate() {
        ...
        shift += 7;
        if shift >= 64 { return Err(SlkDecodeError("varint overflow".into())); }
    }
