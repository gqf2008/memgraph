...
impl Distance {
    pub fn compute(&self, a: &[f32], b: &[f32]) -> f32 {
        match self {
            ...
            Distance::Hamming => a.iter().zip(b).filter(|(x, y)| x != y).count() as f32,
        }
    }
    ...
    pub fn is_minimizing(&self) -> bool {
        matches!(
            self,
            Distance::Cosine | Distance::Euclidean | Distance::Manhattan | Distance::Hamming
        )
    }
}
...
impl HnswIndex {
    ...
    pub fn insert(&mut self, vector: &[f32]) -> NodeId {
        ...
        for lc in (0..=level.min(self.max_level)).rev() {
            let neighbors =
                self.search_layer_beam(vector, curr_ep, self.config.ef_construction, lc);
    ...
    pub fn compact(&mut self) {
        let deleted_ids: Vec<NodeId> = self
            .nodes
            .values()
            .filter(|n| n.deleted)
            .map(|n| n.id)
        ...
        let all_nodes: Vec<(NodeId, Vec<f32>)> = self
            .nodes
            .values()
    ...
    fn select_neighbors(&self, candidates: &[SearchEntry], max_conn: usize) -> Vec<SearchEntry> {
    ...
    fn prune_neighbors(&mut self, node_id: NodeId, level: usize) {
        if let Some(node) = self.nodes.get(&node_id) {
            ...
            if node.layers[level].len() <= self.config.m0 {
                return;
            }
            ...
            let mut entries: Vec<SearchEntry> = neighbors
                .iter()
                .filter_map(|nid| {
                    self.nodes.get(nid).map(|n| SearchEntry {
                        id: *nid,
...
mod tests {
    ...
    fn test_multi_insert_search() {
        ...
        assert!(
            exact_matches >= 30,
            "expected at least 30 exact matches, got {}",
            exact_matches
        );
    }
    ...
    fn test_batch_insert() {
        ...
        assert!(
            exact_matches >= 35,
            "expected at least 35 exact matches, got {}",
            exact_matches
        );
    }
