...
pub fn assign_round_robin(members: &[Member], partitions: &[Partition]) -> Vec<(String, Partition)> {
...
pub fn elect_leader(current_leader: i32, isr: &[i32], _live_brokers: &[i32]) -> LeaderElectionResult {
    if isr.is_empty() {
...
mod tests {
    ...
    fn make_members(ids: &[&str]) -> Vec<Member> {
        ids.iter().map(|id| Member { member_id: id.to_string() }).collect()
    }
