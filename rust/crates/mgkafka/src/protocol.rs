...
pub fn parse_create_topics_response(data: &[u8]) -> Result<Vec<CreateTopicResponse>, KafkaError> {
    let mut r = Reader::new(data);
    let _correlation_id = r.read_i32()?;
    let _throttle = r.read_i32()?;
    let topic_count = r.read_i32()?;
...
pub fn parse_delete_topics_response(data: &[u8]) -> Result<Vec<DeleteTopicResponse>, KafkaError> {
    let mut r = Reader::new(data);
    let _correlation_id = r.read_i32()?;
    let _throttle = r.read_i32()?;
    let topic_count = r.read_i32()?;
