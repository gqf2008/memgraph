...
pub use coordinator::{
    assign_partitions, CommittedOffset, ConsumerGroup, CoordinatorError, GroupCoordinator,
    GroupMember, JoinGroupResult, MemberState, MemberSubscription, OffsetStore, SyncGroupResult,
};
pub use partition::{
...
impl KafkaProducer {
    ...
    ) -> Result<Vec<ProduceResponseTopic>, KafkaError> {
        if records.is_empty() {
            return Ok(Vec::new());
        }

        // Group records by (topic, partition)
        let mut groups: std::collections::HashMap<(String, i32), Vec<&ProducerRecord>> =
    ...
    pub fn send(
        ...
        self.send_batch(std::slice::from_ref(record))
    }
}
...
impl KafkaAdmin {
    ...
    pub fn list_offsets(
        ...
        let mut stream = TcpStream::connect(&self.broker).map_err(KafkaError::Io)?;
        let request =
            build_list_offsets_request(self.next_correlation_id(), &self.client_id, topics);
        stream.write_all(&request).map_err(KafkaError::Io)?;
        let response = read_response(&mut stream)?;
    ...
    pub fn metadata(&mut self, topics: &[&str]) -> Result<TopicMetadataList, KafkaError> {
        let mut stream = TcpStream::connect(&self.broker).map_err(KafkaError::Io)?;
        let request = build_metadata_request(self.next_correlation_id(), &self.client_id, topics);
        stream.write_all(&request).map_err(KafkaError::Io)?;
        let response = read_response(&mut stream)?;
...
pub fn run_consumer(storage: Arc<Storage>, broker: &str, topic: &str) -> Result<(), KafkaError> {
