...
impl AuthStore {
    ...
    pub fn authenticate(
        ...
        if verify_password(password, &user_clone.password_hash) {
            ...
            self.record_audit(username, AuditEventType::LoginFailure, Some("bad password".into()));
        ...
        match operation {
            AuthOperation::Read => {
                if !user.role.can_read() {
                    drop(users);
                    self.record_audit(username, AuditEventType::PermissionDenied, Some("role".into()));
            ...
            match operation {
                AuthOperation::Read => {
                    if !user.label_permissions.can_read_label(lbl) {
                        drop(users);
                        self.record_audit(username, AuditEventType::PermissionDenied, Some(format!("label read {}", lbl)));
                        return Err(AuthError::PermissionDenied);
                    }
                }
                AuthOperation::Write | AuthOperation::Create | AuthOperation::Delete => {
                    if !user.label_permissions.can_write_label(lbl) {
                        drop(users);
                        self.record_audit(username, AuditEventType::PermissionDenied, Some(format!("label write {}", lbl)));
                        return Err(AuthError::PermissionDenied);
                    }
                }
            }
    ...
    pub fn authenticate_ldap(
        ...
        let url = ldap_url.as_ref().ok_or(AuthError::NotConfigured)?;
    ...
    pub fn grant_privileges(
        ...
        if is_user {
            ...
            let user = users.get_mut(&target_name)
            ...
            let entry = privs.entry(target_name.clone()).or_insert_with(HashSet::new);
            for p in privileges {
    ...
    pub fn deny_privileges(
        ...
        if is_user {
            ...
            let mut privs = self.role_denied_privileges.write().map_err(|e| format!("{}", e))?;
            let entry = privs.entry(target_name.clone()).or_insert_with(HashSet::new);
            for p in privileges {
                entry.insert(format!("{:?}", p));
            }
    ...
    pub fn list_privileges(&self, target_name: &str, is_user: bool) -> Vec<(mgparser::ast::Privilege, bool)> {
        let mut result = Vec::new();
        if is_user {
            let users = self.users.read().unwrap();
            if let Some(user) = users.get(target_name) {
                for p_name in &user.granted_privileges {
                    if let Some(p) = mgparser::ast::Privilege::from_str(p_name) {
                        result.push((p, true));
                    }
                }
    ...
    pub fn rename_user(&self, old_name: &str, new_name: &str) -> Result<(), String> {
        let mut users = self.users.write().map_err(|e| format!("{}", e))?;
        let mut user = users.remove(old_name)
...
mod tests {
    ...
    fn test_auth_disabled_bypasses() {
        ...
        assert!(store.check_permission("anyone", &Role::ReadOnly, None, AuthOperation::Write).is_ok());
    }
    ...
    fn test_admin_has_all_permissions() {
        ...
        assert!(store.check_permission("admin", &Role::Admin, None, AuthOperation::Write).is_ok());
        assert!(store.check_permission("admin", &Role::Admin, Some("secret"), AuthOperation::Delete).is_ok());
    }
    ...
    fn test_readonly_cannot_write() {
        ...
        assert!(store.check_permission("reader", &Role::ReadOnly, None, AuthOperation::Read).is_ok());
        assert!(store.check_permission("reader", &Role::ReadOnly, None, AuthOperation::Write).is_err());
    }
    ...
    fn test_readwrite_can_write() {
        ...
        assert!(store.check_permission("writer", &Role::ReadWrite, None, AuthOperation::Read).is_ok());
        assert!(store.check_permission("writer", &Role::ReadWrite, None, AuthOperation::Write).is_ok());
        assert!(store.check_permission("writer", &Role::ReadWrite, None, AuthOperation::Create).is_ok());
        assert!(store.check_permission("writer", &Role::ReadWrite, None, AuthOperation::Delete).is_ok());
    }
    ...
    fn test_label_permissions() {
        ...
        assert!(store.check_permission("limited", &Role::ReadWrite, Some("Public"), AuthOperation::Read).is_ok());
        assert!(store.check_permission("limited", &Role::ReadWrite, Some("Public"), AuthOperation::Write).is_ok());

        // Denied on Secret label
        assert!(store.check_permission("limited", &Role::ReadWrite, Some("Secret"), AuthOperation::Read).is_err());
        assert!(store.check_permission("limited", &Role::ReadWrite, Some("Secret"), AuthOperation::Write).is_err());
    }
    ...
    fn test_remove_user() {
        ...
        assert!(matches!(store.authenticate("temp", "pw"), Err(AuthError::UserNotFound)));
    }

    #[test]
    fn test_custom_role() {
        let role = Role::from_str("analyst");
    ...
    fn test_add_user_with_password_policy() {
        ...
        assert!(store.add_user_with_password("alice", "weak", Role::ReadOnly).is_err());
        assert!(store.add_user_with_password("alice", "secret123", Role::ReadOnly).is_ok());
    }
    ...
    fn test_audit_log() {
        ...
        assert!(log.iter().any(|e| e.event_type == AuditEventType::LoginFailure));
        assert!(log.iter().any(|e| e.event_type == AuditEventType::LoginSuccess));
        assert!(log.iter().any(|e| e.event_type == AuditEventType::UserRemoved));
