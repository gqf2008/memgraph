...
use mgparser::ast::{
    Clause, Expression, Query, MatchPattern, PatternElement, SetItem, RemoveItem, ReturnItem,
};
...
pub enum SemanticError {
    ...
    AggregateInRemove,
    AggregateInDelete,
    AggregateWithoutGrouping { expression: String },
    PrivilegeInsufficient { action: String, required: String },
...
impl AnalysisContext {
    ...
    fn check_aggregate_in_where(&mut self, query: &Query) {
        for clause in &query.clauses {
            match clause {
                Clause::Match { where_clause: Some(expr), .. }
                | Clause::OptionalMatch { where_clause: Some(expr), .. } => {
                    if self.contains_aggregate(expr) {
                        self.errors.push(SemanticError::AggregateInWhere);
                    }
                }
                Clause::With { where_clause: Some(expr), .. } => {
                    if self.contains_aggregate(expr) {
                        self.errors.push(SemanticError::AggregateInWhere);
                    }
                }
    ...
    fn contains_aggregate(&self, expr: &Expression) -> bool {
        match expr {
            ...
            Expression::And(a, b) | Expression::Or(a, b) | Expression::Add(a, b)
            | Expression::Sub(a, b) | Expression::Mul(a, b) | Expression::Div(a, b)
            | Expression::Mod(a, b) | Expression::Eq(a, b) | Expression::Neq(a, b)
            | Expression::Lt(a, b) | Expression::Gt(a, b) | Expression::Lte(a, b)
            | Expression::Gte(a, b) | Expression::In(a, b)
            | Expression::StartsWith(a, b) | Expression::EndsWith(a, b)
            | Expression::Contains(a, b) | Expression::RegexMatch(a, b) => {
                self.contains_aggregate(a) || self.contains_aggregate(b)
            }
            Expression::Neg(a) | Expression::Not(a) | Expression::IsNull(a)
            | Expression::IsNotNull(a) | Expression::Property { object: a, .. }
            ...
            Expression::MapProjection { object, extra, .. } => self.contains_aggregate(object) || extra.iter().any(|(_, e)| self.contains_aggregate(e)),
            Expression::Case { expression, whens, else_branch } => {
                expression.as_ref().map_or(false, |e| self.contains_aggregate(e))
                    || whens.iter().any(|(w, t)| self.contains_aggregate(w) || self.contains_aggregate(t))
                    || else_branch.as_ref().map_or(false, |e| self.contains_aggregate(e))
            }
            Expression::All { list, predicate, .. }
            | Expression::Any { list, predicate, .. }
            | Expression::None { list, predicate, .. }
            | Expression::Single { list, predicate, .. }
            | Expression::Filter { list, predicate, .. }
            | Expression::Extract { list, expression: predicate, .. }
            | Expression::Reduce { list, expression: predicate, .. } => {
            ...
            Expression::Exists(query) | Expression::CountSubquery(query) => query.clauses.iter().any(|c| self.contains_aggregate_in_clause(c)),
            _ => false,
        }
    }
    ...
    fn contains_aggregate_in_clause(&self, clause: &Clause) -> bool {
        match clause {
            Clause::Match { where_clause: Some(expr), .. }
            | Clause::OptionalMatch { where_clause: Some(expr), .. } => self.contains_aggregate(expr),
            Clause::With { items, where_clause, .. } => {
                items.iter().any(|i| self.contains_aggregate(&i.expression))
                    || where_clause.as_ref().map_or(false, |e| self.contains_aggregate(e))
            }
            Clause::Return { items, .. } => items.iter().any(|i| self.contains_aggregate(&i.expression)),
            Clause::Set { items } => items.iter().any(|i| match i {
                SetItem::Property { expression, value, .. } => self.contains_aggregate(expression) || self.contains_aggregate(value),
                SetItem::Variable { expression, .. } | SetItem::VariableUpdate { expression, .. } => self.contains_aggregate(expression),
                SetItem::Label { .. } => false,
            }),
            ...
            Clause::Delete { expressions, .. } => expressions.iter().any(|e| self.contains_aggregate(e)),
            Clause::OrderBy { items } => items.iter().any(|i| self.contains_aggregate(&i.expression)),
            _ => false,
        }
    }
    ...
    fn require_label_privilege(&mut self, elem: &PatternElement, catalog: &Catalog, required: Privilege, action: &str) {
    ...
    fn privilege_covers(&self, granted: &Privilege, required: &Privilege) -> bool {
        match (granted, required) {
            ...
            (Privilege::Write, Privilege::Write) => true,
            _ => false,
        }
    }
    ...
    fn validate_aggregates(&mut self, query: &Query) {
        // Detect grouping
        self.has_grouping = query.clauses.iter().any(|c| matches!(c, Clause::Return { distinct: true, .. }));
        // Collect group keys from RETURN non-aggregate items
        if let Some(Clause::Return { items, .. }) = query.clauses.iter().find(|c| matches!(c, Clause::Return { .. })) {
            for item in items {
                ...
                Clause::Set { items } => {
                    for item in items {
                        let exprs: Vec<&Expression> = match item {
                            SetItem::Property { expression, value, .. } => vec![expression, value],
                            SetItem::Variable { expression, .. } | SetItem::VariableUpdate { expression, .. } => vec![expression],
                            SetItem::Label { .. } => vec![],
                        };
                        for e in exprs {
        ...
        match expr {
            ...
            Expression::Function { name, arguments, .. } => {
                ...
                if agg_names.iter().any(|n| name.eq_ignore_ascii_case(n)) {
                    ...
                    arguments.iter().flat_map(|a| self.collect_non_aggregate_identifiers(a)).collect()
                }
            }
            Expression::And(a, b) | Expression::Or(a, b) | Expression::Add(a, b)
            | Expression::Sub(a, b) | Expression::Mul(a, b) | Expression::Div(a, b)
            ...
            Expression::List(items) => items.iter().flat_map(|e| self.collect_non_aggregate_identifiers(e)).collect(),
            Expression::Map(pairs) => pairs.iter().flat_map(|(_, e)| self.collect_non_aggregate_identifiers(e)).collect(),
            Expression::MapProjection { object, extra, .. } => {
                let mut res = self.collect_non_aggregate_identifiers(object);
                for (_, e) in extra { res.extend(self.collect_non_aggregate_identifiers(e)); }
                res
            }
            Expression::Case { expression, whens, else_branch } => {
                let mut res = expression.as_ref().map_or(vec![], |e| self.collect_non_aggregate_identifiers(e));
                for (w, t) in whens {
    ...
    fn contains_nested_aggregate(&self, expr: &Expression) -> bool {
        match expr {
            ...
            Expression::MapProjection { object, extra, .. } => self.contains_nested_aggregate(object) || extra.iter().any(|(_, e)| self.contains_nested_aggregate(e)),
            Expression::Case { expression, whens, else_branch } => {
                expression.as_ref().map_or(false, |e| self.contains_nested_aggregate(e))
                    || whens.iter().any(|(w, t)| self.contains_nested_aggregate(w) || self.contains_nested_aggregate(t))
                    || else_branch.as_ref().map_or(false, |e| self.contains_nested_aggregate(e))
            }
            Expression::All { list, predicate, .. }
            | Expression::Any { list, predicate, .. }
            | Expression::None { list, predicate, .. }
            ...
            Expression::Exists(query) | Expression::CountSubquery(query) => query.clauses.iter().any(|c| self.contains_nested_aggregate_in_clause(c)),
            _ => false,
        }
    }
    ...
    fn contains_nested_aggregate_in_clause(&self, clause: &Clause) -> bool {
        match clause {
            Clause::Match { where_clause: Some(expr), .. }
            | Clause::OptionalMatch { where_clause: Some(expr), .. } => self.contains_nested_aggregate(expr),
            Clause::With { items, where_clause, .. } => {
                items.iter().any(|i| self.contains_nested_aggregate(&i.expression))
                    || where_clause.as_ref().map_or(false, |e| self.contains_nested_aggregate(e))
            }
            Clause::Return { items, .. } => items.iter().any(|i| self.contains_nested_aggregate(&i.expression)),
            Clause::Set { items } => items.iter().any(|i| match i {
                SetItem::Property { expression, value, .. } => self.contains_nested_aggregate(expression) || self.contains_nested_aggregate(value),
                SetItem::Variable { expression, .. } | SetItem::VariableUpdate { expression, .. } => self.contains_nested_aggregate(expression),
                SetItem::Label { .. } => false,
            }),
            Clause::Remove { items } => items.iter().any(|i| match i {
                RemoveItem::Property { expression, .. } => self.contains_nested_aggregate(expression),
                RemoveItem::Label { .. } => false,
            }),
            Clause::Delete { expressions, .. } => expressions.iter().any(|e| self.contains_nested_aggregate(e)),
            _ => false,
        }
    }
        ...
        for clause in &query.clauses {
            match clause {
                ...
                Clause::Set { items } => {
                    for item in items {
                        match item {
                            SetItem::Property { expression, value, .. } => {
                                self.analyze_subquery_expressions(expression);
                                self.analyze_subquery_expressions(value);
                            }
                            SetItem::Variable { expression, .. } | SetItem::VariableUpdate { expression, .. } => {
                                self.analyze_subquery_expressions(expression);
                            }
                            _ => {}
                        }
    ...
    fn analyze_subquery_expressions(&mut self, expr: &Expression) {
        match expr {
            ...
            Expression::List(items) => items.iter().for_each(|e| self.analyze_subquery_expressions(e)),
            Expression::Map(pairs) => pairs.iter().for_each(|(_, e)| self.analyze_subquery_expressions(e)),
            Expression::MapProjection { object, extra, .. } => {
                self.analyze_subquery_expressions(object);
                for (_, e) in extra { self.analyze_subquery_expressions(e); }
            }
            Expression::Function { arguments, .. } => arguments.iter().for_each(|a| self.analyze_subquery_expressions(a)),
            Expression::Case { expression, whens, else_branch } => {
    ...
    fn analyze_subquery(&mut self, subquery: &Query, kind: SubqueryKind) {
        ...
        let undefined_in_subquery: Vec<String> = sub_ctx.errors.iter()
        ...
        for name in undefined_in_subquery {
            self.errors.push(SemanticError::CorrelatedReferenceNotFound(name));
        }
        ...
        match kind {
            SubqueryKind::Exists => {
                ...
                let has_return = subquery.clauses.iter().any(|c| matches!(c, Clause::Return { .. }));
            ...
            SubqueryKind::Scalar => {
                ...
                if let Some(Clause::Return { items, .. }) = subquery.clauses.iter().find(|c| matches!(c, Clause::Return { .. })) {
                    if items.len() != 1 {
    ...
    fn check_bound_variables(&mut self, expr: &Expression, bound_vars: &std::collections::HashSet<String>) {
        match expr {
            Expression::Identifier(name) => {
                if !bound_vars.contains(name) && !self.symbols.contains_key(name) {
                    self.errors.push(SemanticError::UnboundPatternVariable(name.clone()));
                }
            }
            ...
            Expression::List(items) => items.iter().for_each(|e| self.check_bound_variables(e, bound_vars)),
            Expression::Map(pairs) => pairs.iter().for_each(|(_, e)| self.check_bound_variables(e, bound_vars)),
            Expression::MapProjection { object, extra, .. } => {
                self.check_bound_variables(object, bound_vars);
                for (_, e) in extra { self.check_bound_variables(e, bound_vars); }
            }
            Expression::Function { arguments, .. } => arguments.iter().for_each(|a| self.check_bound_variables(a, bound_vars)),
            Expression::Case { expression, whens, else_branch } => {
                if let Some(e) = expression {
            ...
            Expression::Exists(query) | Expression::CountSubquery(query) => {
                ...
                for clause in &query.clauses {
                    match clause {
                        Clause::Return { items, .. } => {
                            for item in items {
                                self.check_bound_variables_in_subquery(&item.expression, &sub_bound);
                            }
                        }
                        Clause::With { items, where_clause, .. } => {
                            for item in items {
    ...
    fn check_bound_variables_in_subquery(&mut self, expr: &Expression, bound_vars: &std::collections::HashSet<String>) {
        match expr {
            Expression::Identifier(name) => {
                if !bound_vars.contains(name) && !self.symbols.contains_key(name) {
                    self.errors.push(SemanticError::UnboundPatternVariable(name.clone()));
            ...
            Expression::List(items) => items.iter().for_each(|e| self.check_bound_variables_in_subquery(e, bound_vars)),
            Expression::Map(pairs) => pairs.iter().for_each(|(_, e)| self.check_bound_variables_in_subquery(e, bound_vars)),
            Expression::MapProjection { object, extra, .. } => {
                self.check_bound_variables_in_subquery(object, bound_vars);
                for (_, e) in extra { self.check_bound_variables_in_subquery(e, bound_vars); }
            }
            Expression::Function { arguments, .. } => arguments.iter().for_each(|a| self.check_bound_variables_in_subquery(a, bound_vars)),
            Expression::Case { expression, whens, else_branch } => {
                if let Some(e) = expression {
    ...
    fn analyze_clause(&mut self, clause: &Clause) {
        match clause {
            ...
            Clause::Remove { items } => {
                for item in items {
                    match item {
                        RemoveItem::Property { expression, .. } => self.check_expression(expression),
                        RemoveItem::Label { .. } => {}
                    }
                }
            }
            ...
            Clause::Call { arguments, yield_items, .. } => {
            ...
            | Clause::CreateConstraint { .. } | Clause::DropConstraint { .. }
            | Clause::Show { .. } | Clause::ShowAuth { .. }
            | Clause::ShowSetting { .. } | Clause::ShowSettings
            | Clause::SetSetting { .. } | Clause::ShowTransactions
            | Clause::TerminateTransaction { .. }
            | Clause::CreateUser { .. } | Clause::DropUser { .. }
            | Clause::CreateRole { .. } | Clause::DropRole { .. }
            | Clause::GrantRole { .. } | Clause::RevokeRole { .. }
            | Clause::CreateTrigger { .. } | Clause::DropTrigger { .. }
            | Clause::CreateDatabase { .. } | Clause::DropDatabase { .. }
            | Clause::LoadJsonl { .. }
            | Clause::GrantPrivilege { .. } | Clause::RevokePrivilege { .. }
            | Clause::DenyPrivilege { .. } | Clause::ShowPrivileges { .. }
            | Clause::AlterUser { .. }
    ...
    fn define_variable(&mut self, name: &str, typ: CypherType) {
        if let Some(existing) = self.symbols.get(name) {
            if existing.defined_in_clause == self.clause_index {
                self.errors.push(SemanticError::DuplicateVariable(name.to_string()));
            } else {
                self.warnings.push(SemanticError::ShadowedVariable(name.to_string()));
            }
        }
    ...
    fn check_expression(&mut self, expr: &Expression) {
        match expr {
            ...
            Expression::Eq(a, b) | Expression::Neq(a, b)
            | Expression::Lt(a, b) | Expression::Gt(a, b)
            | Expression::Lte(a, b) | Expression::Gte(a, b)
            | Expression::In(a, b)
            | Expression::StartsWith(a, b) | Expression::EndsWith(a, b)
            | Expression::Contains(a, b) | Expression::RegexMatch(a, b) => {
                self.check_expression(a);
            ...
            Expression::Map(pairs) => {
                for (_, v) in pairs { self.check_expression(v); }
            }
            Expression::MapProjection { object, extra, .. } => {
                self.check_expression(object);
                for (_, e) in extra { self.check_expression(e); }
            }
            Expression::Function { arguments, .. } => {
                for a in arguments { self.check_expression(a); }
            }
            Expression::Case { expression, whens, else_branch } => {
                if let Some(e) = expression {
            ...
            | Expression::Single { variable, list, predicate } => {
                self.check_expression(list);
                self.symbols.insert(variable.clone(), Symbol { typ: CypherType::Any, defined_in_clause: self.clause_index });
            ...
            | Expression::Extract { variable, list, expression: predicate } => {
                self.check_expression(list);
                self.symbols.insert(variable.clone(), Symbol { typ: CypherType::Any, defined_in_clause: self.clause_index });
                self.check_expression(predicate);
            }
            Expression::Reduce { accumulator, initial, variable, list, expression } => {
                ...
                self.symbols.insert(accumulator.clone(), Symbol { typ: CypherType::Any, defined_in_clause: self.clause_index });
                self.symbols.insert(variable.clone(), Symbol { typ: CypherType::Any, defined_in_clause: self.clause_index });
                self.check_expression(expression);
            }
            Expression::PatternComprehension { pattern, where_clause, expression } => {
                ...
                for elem in &pattern.elements {
                    for (_, expr) in &elem.node.properties { self.check_expression(expr); }
                    for (edge, node) in &elem.edges {
                        for (_, expr) in &edge.properties { self.check_expression(expr); }
                        for (_, expr) in &node.properties { self.check_expression(expr); }
                    }
                }
            ...
            Expression::Parameter(_) => {}
            Expression::Null | Expression::Bool(_)
            | Expression::Int(_) | Expression::Double(_)
            | Expression::String(_) => {}
        }
    }
    ...
    fn infer_type(&self, expr: &Expression) -> CypherType {
        match expr {
            ...
            Expression::Identifier(name) => {
                self.symbols.get(name).map(|s| s.typ.clone()).unwrap_or(CypherType::Any)
            }
            Expression::Parameter(_) => CypherType::Any,
            Expression::Property { .. } | Expression::Index { .. } | Expression::Slice { .. } => CypherType::Any,
            Expression::Function { name, .. } => self.infer_function_type(name),
            Expression::CountStar => CypherType::Int,
            Expression::Eq(_, _) | Expression::Neq(_, _)
            | Expression::Lt(_, _) | Expression::Gt(_, _)
            | Expression::Lte(_, _) | Expression::Gte(_, _)
            | Expression::And(_, _) | Expression::Or(_, _)
            | Expression::Not(_) | Expression::IsNull(_)
            | Expression::IsNotNull(_) | Expression::StartsWith(_, _)
            | Expression::EndsWith(_, _) | Expression::Contains(_, _)
            | Expression::In(_, _) | Expression::RegexMatch(_, _) => CypherType::Bool,
            ...
            Expression::Sub(_, _) | Expression::Mul(_, _) | Expression::Div(_, _) | Expression::Mod(_, _) | Expression::Neg(_) => CypherType::Any,
            ...
            Expression::Case { whens, else_branch, .. } => {
                ...
                let mut branch_types: Vec<CypherType> = whens.iter().map(|(_, t)| self.infer_type(t)).collect();
                if let Some(e) = else_branch {
    ...
    fn infer_function_type(&self, name: &str) -> CypherType {
        match () {
            ...
            _ if name.eq_ignore_ascii_case("labels") => CypherType::List(Box::new(CypherType::String)),
            _ if name.eq_ignore_ascii_case("type") => CypherType::String,
            _ if name.eq_ignore_ascii_case("properties") => CypherType::Map,
            _ if name.eq_ignore_ascii_case("size") | name.eq_ignore_ascii_case("length") => CypherType::Int,
            _ if name.eq_ignore_ascii_case("tostring") => CypherType::String,
            _ if name.eq_ignore_ascii_case("tolower") | name.eq_ignore_ascii_case("toupper") => CypherType::String,
            _ if name.eq_ignore_ascii_case("tofloat") => CypherType::Double,
            _ if name.eq_ignore_ascii_case("tointeger") | name.eq_ignore_ascii_case("toint") => CypherType::Int,
            _ if name.eq_ignore_ascii_case("abs") | name.eq_ignore_ascii_case("ceil")
                | name.eq_ignore_ascii_case("floor") | name.eq_ignore_ascii_case("round")
                | name.eq_ignore_ascii_case("sqrt") | name.eq_ignore_ascii_case("sign") => CypherType::Double,
            _ if name.eq_ignore_ascii_case("range") => CypherType::List(Box::new(CypherType::Int)),
            _ if name.eq_ignore_ascii_case("split") => CypherType::List(Box::new(CypherType::String)),
            _ if name.eq_ignore_ascii_case("replace") | name.eq_ignore_ascii_case("trim")
                | name.eq_ignore_ascii_case("ltrim") | name.eq_ignore_ascii_case("rtrim")
                | name.eq_ignore_ascii_case("substring") | name.eq_ignore_ascii_case("left")
                | name.eq_ignore_ascii_case("right") | name.eq_ignore_ascii_case("reverse")
                | name.eq_ignore_ascii_case("repeat") | name.eq_ignore_ascii_case("concat") => CypherType::String,
            _ if name.eq_ignore_ascii_case("head") | name.eq_ignore_ascii_case("last") => CypherType::Any,
            _ if name.eq_ignore_ascii_case("tail") => CypherType::List(Box::new(CypherType::Any)),
            _ if name.eq_ignore_ascii_case("pi") | name.eq_ignore_ascii_case("e")
                | name.eq_ignore_ascii_case("rand") | name.eq_ignore_ascii_case("log")
                | name.eq_ignore_ascii_case("log10") | name.eq_ignore_ascii_case("exp")
                | name.eq_ignore_ascii_case("pow") => CypherType::Double,
            // Statistical functions
            _ if name.eq_ignore_ascii_case("percentilecont") | name.eq_ignore_ascii_case("percentile_cont")
                | name.eq_ignore_ascii_case("percentiledisc") | name.eq_ignore_ascii_case("percentile_disc")
                | name.eq_ignore_ascii_case("stdev") | name.eq_ignore_ascii_case("stddev")
                | name.eq_ignore_ascii_case("stdevp") | name.eq_ignore_ascii_case("stddevp") => CypherType::Double,
            // Safe conversion functions (may return Null)
            _ if name.eq_ignore_ascii_case("tofloatornull") | name.eq_ignore_ascii_case("tofloatornull") => CypherType::Double,
            _ if name.eq_ignore_ascii_case("tointegerornull") | name.eq_ignore_ascii_case("tointornull") => CypherType::Int,
            _ if name.eq_ignore_ascii_case("tobooleanornull") => CypherType::Bool,
            // Temporal extractors
            _ if name.eq_ignore_ascii_case("year") | name.eq_ignore_ascii_case("month")
                | name.eq_ignore_ascii_case("day") | name.eq_ignore_ascii_case("hour")
                | name.eq_ignore_ascii_case("minute") | name.eq_ignore_ascii_case("second")
                | name.eq_ignore_ascii_case("millisecond") | name.eq_ignore_ascii_case("microsecond")
                | name.eq_ignore_ascii_case("dayofweek") | name.eq_ignore_ascii_case("dayofyear")
                | name.eq_ignore_ascii_case("week") | name.eq_ignore_ascii_case("quarter")
                | name.eq_ignore_ascii_case("epochmillis") | name.eq_ignore_ascii_case("epochmilli")
                | name.eq_ignore_ascii_case("epochseconds") | name.eq_ignore_ascii_case("epochsecond") => CypherType::Int,
            // Trigonometric / hyperbolic
            _ if name.eq_ignore_ascii_case("sin") | name.eq_ignore_ascii_case("cos")
                | name.eq_ignore_ascii_case("tan") | name.eq_ignore_ascii_case("asin")
                | name.eq_ignore_ascii_case("acos") | name.eq_ignore_ascii_case("atan")
                | name.eq_ignore_ascii_case("atan2") | name.eq_ignore_ascii_case("cot")
                | name.eq_ignore_ascii_case("haversin") | name.eq_ignore_ascii_case("degrees")
                | name.eq_ignore_ascii_case("radians") | name.eq_ignore_ascii_case("cosh")
                | name.eq_ignore_ascii_case("sinh") | name.eq_ignore_ascii_case("tanh")
                | name.eq_ignore_ascii_case("acosh") | name.eq_ignore_ascii_case("asinh")
                | name.eq_ignore_ascii_case("atanh") => CypherType::Double,
            // String predicates
            _ if name.eq_ignore_ascii_case("startswith") | name.eq_ignore_ascii_case("endswith")
                | name.eq_ignore_ascii_case("regexmatch") | name.eq_ignore_ascii_case("isempty")
                | name.eq_ignore_ascii_case("contains") => CypherType::Bool,
            // Coalesce returns Any (depends on arguments)
            _ if name.eq_ignore_ascii_case("coalesce") => CypherType::Any,
            // Date/time constructors
            _ if name.eq_ignore_ascii_case("date") | name.eq_ignore_ascii_case("time")
                | name.eq_ignore_ascii_case("localtime") | name.eq_ignore_ascii_case("localdatetime")
                | name.eq_ignore_ascii_case("datetime") | name.eq_ignore_ascii_case("duration")
                | name.eq_ignore_ascii_case("timestamp") => CypherType::Any,
            // Point / distance
...
mod tests {
    ...
    fn test_undefined_variable() {
        ...
        assert!(result.errors.iter().any(|e| matches!(e, SemanticError::UndefinedVariable(v) if v == "x")));
    }
    ...
    fn test_relationship_type() {
        ...
        let result = analyze(&q);
        assert!(result.errors.is_empty());
        assert_eq!(result.variable_types.get("r"), Some(&CypherType::Relationship));
    ...
    fn test_return_position() {
        ...
        let result = analyze(&q);
        assert!(result.errors.iter().any(|e| matches!(e, SemanticError::InvalidClauseOrder(_))));
    }
    ...
    fn test_aggregate_in_where() {
        ...
        let result = analyze(&q);
        assert!(result.errors.iter().any(|e| matches!(e, SemanticError::AggregateInWhere)));
    }
    ...
    fn test_privilege_set_requires_write() {
        ...
        assert!(result.is_err());
        let errs = result.unwrap_err();
        assert!(errs.iter().any(|e| matches!(e, SemanticError::PrivilegeInsufficient { action, .. } if action == "SET")));
    }
    ...
    fn test_nested_aggregate_not_allowed() {
        ...
        let result = analyze(&q);
        assert!(result.errors.iter().any(|e| matches!(e, SemanticError::NestedAggregate)));
    }
    ...
    fn test_aggregate_in_set_not_allowed() {
        ...
        let result = analyze(&q);
        assert!(result.errors.iter().any(|e| matches!(e, SemanticError::AggregateInSet)));
    }
    ...
    fn test_aggregate_in_remove_not_allowed() {
        ...
        assert!(!result.errors.iter().any(|e| matches!(e, SemanticError::AggregateInRemove)));
    }
    ...
    fn test_aggregate_in_delete_not_allowed() {
        ...
        let q = parse_query("MATCH (n) DELETE n").unwrap();
        let result = analyze(&q);
        assert!(!result.errors.iter().any(|e| matches!(e, SemanticError::AggregateInDelete)));
    }
    ...
    fn test_subquery_inherits_outer_bindings() {
        let q = parse_query("MATCH (n) RETURN EXISTS { MATCH (n)-[:KNOWS]->(m) RETURN m }").unwrap();
        let result = analyze(&q);
        assert!(result.errors.is_empty());
    }
    ...
    fn test_correlated_reference_not_found() {
        let q = parse_query("MATCH (n) RETURN EXISTS { MATCH (m)-[:KNOWS]->(o) WHERE o.name = x.name RETURN o }").unwrap();
        let result = analyze(&q);
        assert!(result.errors.iter().any(|e| matches!(e, SemanticError::UndefinedVariable(v) if v == "x")));
    }
    ...
    fn test_unbound_pattern_variable_detected() {
        ...
        let result = analyze(&q);
        assert!(result.errors.iter().any(|e| matches!(e, SemanticError::UndefinedVariable(v) if v == "m")));
    }
    ...
    fn test_statistical_function_types() {
        let q = parse_query("RETURN percentileCont([1,2,3], 0.5) AS p, stDev([1,2,3]) AS s").unwrap();
        let result = analyze(&q);
        assert!(result.errors.is_empty());
    }
    ...
    fn test_string_predicate_types() {
        let q = parse_query("RETURN startsWith('hello', 'he'), endsWith('hello', 'lo'), contains('hello', 'll')").unwrap();
        let result = analyze(&q);
        assert!(result.errors.is_empty());
    }
}
