...
pub unsafe extern "C" fn mgp_stream_make_empty(result: *mut *mut mgp_stream) -> MgpError {
    ...
    *result = Box::into_raw(Box::new(mgp_stream {
        batches: Vec::new(),
        pos: 0,
    }));
    MgpError::NoError
...
#[no_mangle]
pub unsafe extern "C" fn mgp_stream_batch_make_empty(
    result: *mut *mut mgp_stream_batch,
) -> MgpError {
    if result.is_null() {
        return MgpError::InvalidArgument;
    }
    *result = Box::into_raw(Box::new(mgp_stream_batch {
        records: Vec::new(),
    }));
    MgpError::NoError
}
...
#[no_mangle]
pub unsafe extern "C" fn mgp_stream_batch_size(
    batch: *const mgp_stream_batch,
    result: *mut usize,
) -> MgpError {
    if batch.is_null() || result.is_null() {
        return MgpError::InvalidArgument;
    }
...
macro_rules! trigger_iter_fn {
    ($fn_name:ident, $field:ident, $iter_ty:ident) => {
        ...
        pub unsafe extern "C" fn $fn_name(
            ...
            let graph = (*ctx).graph;
            *result = Box::into_raw(Box::new($iter_ty {
                graph,
                keys,
                pos: 0,
            }));
...
trigger_iter_fn!(
    mgp_trigger_context_created_vertices,
    created_vertices,
    mgp_vertices_iterator
);
trigger_iter_fn!(
    mgp_trigger_context_created_edges,
    created_edges,
    mgp_edges_iterator
);
trigger_iter_fn!(
    mgp_trigger_context_deleted_vertices,
    deleted_vertices,
    mgp_vertices_iterator
);
trigger_iter_fn!(
    mgp_trigger_context_deleted_edges,
    deleted_edges,
    mgp_edges_iterator
);
...
pub unsafe extern "C" fn mgp_trigger_context_set_property_vertices(
    ...
    let graph = (*ctx).graph;
    *result = Box::into_raw(Box::new(mgp_set_property_vertices_iterator {
        items,
        pos: 0,
        graph,
    }));
...
#[no_mangle]
pub unsafe extern "C" fn mgp_set_property_vertices_iterator_destroy(
    it: *mut mgp_set_property_vertices_iterator,
) {
    if it.is_null() {
        return;
...
pub unsafe extern "C" fn mgp_trigger_context_removed_property_vertices(
    ...
    let graph = (*ctx).graph;
    *result = Box::into_raw(Box::new(mgp_removed_property_vertices_iterator {
        items,
        pos: 0,
        graph,
    }));
...
#[no_mangle]
pub unsafe extern "C" fn mgp_removed_property_vertices_iterator_destroy(
    it: *mut mgp_removed_property_vertices_iterator,
) {
    if it.is_null() {
        return;
...
pub unsafe extern "C" fn mgp_trigger_context_destroy(ctx: *mut mgp_trigger_context) {
    ...
    for (_, _, old, new) in &c.set_property_vertices {
        if !old.is_null() {
            mgp_value_destroy(*old);
        }
        if !new.is_null() {
            mgp_value_destroy(*new);
        }
    }
    ...
    if let Some(v) = c.vertex_before {
        mgp_vertex_destroy(v);
    }
    if let Some(v) = c.vertex_after {
        mgp_vertex_destroy(v);
    }
    if let Some(e) = c.edge_before {
        mgp_edge_destroy(e);
    }
    if let Some(e) = c.edge_after {
        mgp_edge_destroy(e);
    }
}
