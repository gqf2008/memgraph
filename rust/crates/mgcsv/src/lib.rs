...
impl<R: Read> CsvReader<R> {
    pub fn new(reader: R, _has_headers: bool) -> Self {
        Self {
    ...
    pub fn with_delimiter(reader: R, _has_headers: bool, delimiter: u8) -> Self {
        Self {
            reader: csv::ReaderBuilder::new()
                .has_headers(false)
                .flexible(true)
    ...
    pub fn read_chunks<F>(&mut self, chunk_size: usize, mut f: F) -> Result<usize, String>
    where
...
fn try_parse_date(s: &str) -> Option<PropertyValue> {
    ...
    if s.len() >= 19 && s.chars().nth(10) == Some('T') {
        if let Ok(dt) = chrono::DateTime::parse_from_rfc3339(s) {
            ...
            return Some(PropertyValue::ZonedDateTime(
                mgcore::temporal::ZonedDateTime::new(us, offset as i16, "UTC".into()),
            ));
        }
        if let Ok(naive) = chrono::NaiveDateTime::parse_from_str(s, "%Y-%m-%dT%H:%M:%S") {
            let us = naive.and_utc().timestamp_micros();
            return Some(PropertyValue::ZonedDateTime(
                mgcore::temporal::ZonedDateTime::new(us, 0, "UTC".into()),
            ));
        }
    }
    ...
    if s.len() >= 5 && s.chars().nth(2) == Some(':') {
        if let Ok(naive) = chrono::NaiveTime::parse_from_str(s, "%H:%M:%S") {
            ...
            let us = naive
                ...
                .num_microseconds()
                .unwrap_or(0);
            return Some(PropertyValue::LocalTime(
                mgcore::temporal::LocalTime::from_microseconds(us),
            ));
        }
...
mod tests {
    ...
    fn test_infer_datetime() {
        assert!(matches!(
            infer_type("2024-01-15T10:30:00Z"),
            PropertyValue::ZonedDateTime(_)
        ));
        assert!(matches!(
            infer_type("2024-01-15T10:30:00"),
            PropertyValue::ZonedDateTime(_)
        ));
    }
