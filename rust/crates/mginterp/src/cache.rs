...
use mgparser::ast::{Query, QueryMode};
    ...
    while i < bytes.len() {
        ...
        match b {
            ...
            b'0'..=b'9' | b'-' => {
                ...
                let is_standalone = i == 0 || matches!(bytes[i - 1], b' ' | b'\t' | b'\n' | b'(' | b'=' | b'<' | b'>' | b'+' | b'-' | b'*' | b'/' | b',' | b'{');
                if is_standalone && b != b'-' || (b == b'-' && i + 1 < bytes.len() && bytes[i + 1].is_ascii_digit()) {
...
mod tests {
    ...
    fn test_cache_eviction() {
        ...
        let q = Query { clauses: vec![], union: None, mode: QueryMode::Standard, periodic_commit: None, hops_limit: None, index_hints: Vec::new() };
    ...
    fn test_prepared_statement() {
        ...
        let q = Query { clauses: vec![], union: None, mode: QueryMode::Standard, periodic_commit: None, hops_limit: None, index_hints: Vec::new() };
        let id = reg.prepare("MATCH (n) WHERE n.id = $id RETURN n".into(), q, vec!["id".into()]);
        let stmt = reg.get(id);
        assert!(stmt.is_some());
