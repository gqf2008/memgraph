...
fn increment_hops(n: u64) -> Result<(), ExecError> {
    HOPS_COUNTER.with(|c| {
        let new_val = c.get() + n;
        c.set(new_val);
    ...
    for clause in &query.clauses {
        match clause {
            Clause::Return { items, distinct, .. } => {
                ...
                if items.iter().any(|item| is_aggregate(&item.expression)) {
                    return None;
                }
            ...
            Clause::Match { pattern, .. } => {
                // Reject edge patterns — physical executor does graph traversal
                if pattern.elements.iter().any(|e| !e.edges.is_empty()) {
                    return None;
                }
                // Reject node property constraints in patterns — planner does not
                // generate Filter/LabelPropertyScan for them consistently
                if pattern.elements.iter().any(|e| !e.node.properties.is_empty()) {
                    return None;
                }
                // Reject path aliases (e.g. MATCH p = (a))
