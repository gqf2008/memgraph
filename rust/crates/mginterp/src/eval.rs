...
fn us_to_naive_datetime(us: i64) -> chrono::NaiveDateTime {
    chrono::DateTime::from_timestamp_micros(us).unwrap_or_default().naive_utc()
}
...
pub fn set_active_catalog(catalog: Option<&Catalog>) -> ActiveCatalogGuard {
    ...
    ACTIVE_CATALOG.with(|c| {
        c.set(catalog.map(|c| c as *const Catalog).unwrap_or(std::ptr::null()));
    });
    guard
}
    ...
    match expr {
        ...
        Expression::Map(entries) => {
            let vals: Vec<(String, PropertyValue)> = entries
                .iter()
                .map(|(k, v)| (k.clone(), eval_expression_with_catalog(v, bindings, storage, catalog)))
                .collect();
        ...
        Expression::Index { object, index } => {
            ...
            match (obj, idx) {
                (PropertyValue::List(list), PropertyValue::Int(i)) => {
                    let usize_idx = if i < 0 { list.len().saturating_sub((-i) as usize) } else { i as usize };
                    list.get(usize_idx).cloned().unwrap_or(PropertyValue::Null)
                }
                (PropertyValue::String(s), PropertyValue::Int(i)) => {
                    let usize_idx = if i < 0 { s.len().saturating_sub((-i) as usize) } else { i as usize };
                    s.chars().nth(usize_idx).map(|c| PropertyValue::String(c.to_string())).unwrap_or(PropertyValue::Null)
                }
                _ => PropertyValue::Null,
            }
        }
        Expression::Slice { object, start, end } => {
            ...
            match (obj, start_val, end_val) {
                (PropertyValue::List(list), PropertyValue::Int(s), PropertyValue::Int(e)) => {
                    let start_idx = if s < 0 { list.len().saturating_sub((-s) as usize) } else { s as usize };
                    let end_idx = if e < 0 { list.len().saturating_sub((-e) as usize) } else { e as usize };
                ...
                (PropertyValue::String(s), PropertyValue::Int(st), PropertyValue::Int(en)) => {
                    let start_idx = if st < 0 { s.len().saturating_sub((-st) as usize) } else { st as usize };
                    let end_idx = if en < 0 { s.len().saturating_sub((-en) as usize) } else { en as usize };
        ...
        // ─── Other ───────────────────────────────────────────────
        Expression::IsNull(a) => {
            PropertyValue::Bool(matches!(eval_expression_with_catalog(a, bindings, storage, catalog), PropertyValue::Null))
        }
        ...
        Expression::StartsWith(a, b) => string_predicate(a, b, bindings, storage, catalog, StringPred::StartsWith),
        Expression::EndsWith(a, b) => string_predicate(a, b, bindings, storage, catalog, StringPred::EndsWith),
        Expression::Contains(a, b) => string_predicate(a, b, bindings, storage, catalog, StringPred::Contains),
        ...
        Expression::Case { expression, whens, else_branch } => {
            let compare_val = expression.as_ref().map(|e| eval_expression_with_catalog(e, bindings, storage, catalog));
            ...
            match list_val {
                PropertyValue::List(items) => {
                    ...
                    for item in &items {
                        ...
                        let pred_val = eval_expression_with_catalog(predicate, &local, storage, catalog);
            ...
            match list_val {
                PropertyValue::List(items) => {
                    ...
                    for item in &items {
                        let mut local = bindings.clone();
                        local.insert(variable.clone(), item.clone());
                        result.push(eval_expression_with_catalog(expression, &local, storage, catalog));
                    }
...
fn cmp_expr(
    ...
    match (&va, &vb) {
        ...
        (PropertyValue::Int(x), PropertyValue::Double(y)) => {
            let ord = (*x as f64).partial_cmp(y).unwrap_or(std::cmp::Ordering::Equal);
            let result = match ord {
                std::cmp::Ordering::Less => op(-1, 0),
                std::cmp::Ordering::Equal => op(0, 0),
                std::cmp::Ordering::Greater => op(1, 0),
        ...
        (PropertyValue::Double(x), PropertyValue::Int(y)) => {
            let ord = x.partial_cmp(&(*y as f64)).unwrap_or(std::cmp::Ordering::Equal);
            let result = match ord {
                std::cmp::Ordering::Less => op(-1, 0),
                std::cmp::Ordering::Equal => op(0, 0),
                std::cmp::Ordering::Greater => op(1, 0),
...
fn compare_values(a: &PropertyValue, b: &PropertyValue) -> std::cmp::Ordering {
    match (a, b) {
        ...
        (PropertyValue::Double(a), PropertyValue::Double(b)) => a.partial_cmp(b).unwrap_or(std::cmp::Ordering::Equal),
        (PropertyValue::Int(a), PropertyValue::Double(b)) => (*a as f64).partial_cmp(b).unwrap_or(std::cmp::Ordering::Equal),
        (PropertyValue::Double(a), PropertyValue::Int(b)) => a.partial_cmp(&(*b as f64)).unwrap_or(std::cmp::Ordering::Equal),
...
fn arithmetic(
    ...
    match (a, b) {
        ...
        (PropertyValue::Int(x), PropertyValue::Double(y)) => PropertyValue::Double(float_op(*x as f64, *y)),
        (PropertyValue::Double(x), PropertyValue::Int(y)) => PropertyValue::Double(float_op(*x, *y as f64)),
        (PropertyValue::Double(x), PropertyValue::Double(y)) => PropertyValue::Double(float_op(*x, *y)),
        _ => PropertyValue::Null,
    }
}
...
fn temporal_arithmetic(a: &PropertyValue, b: &PropertyValue, is_add: bool) -> Option<PropertyValue> {
    ...
    match (a, b) {
        ...
        (PropertyValue::Date(date), PropertyValue::Duration(dur)) if is_add => {
            Some(PropertyValue::Date(Date::from_days(date.days_since_epoch + dur.days)))
        }
        (PropertyValue::Date(date), PropertyValue::Duration(dur)) => {
            Some(PropertyValue::Date(Date::from_days(date.days_since_epoch - dur.days)))
        }
        // Duration + Date (commutative)
        (PropertyValue::Duration(dur), PropertyValue::Date(date)) if is_add => {
            Some(PropertyValue::Date(Date::from_days(date.days_since_epoch + dur.days)))
        }
        ...
        (PropertyValue::LocalDateTime(ldt), PropertyValue::Duration(dur)) if is_add => {
            ...
            Some(PropertyValue::LocalDateTime(LocalDateTime::from_microseconds(ldt.microseconds + delta_us)))
        }
...
fn extract_property(obj: &PropertyValue, key: mgcore::types::PropertyId, storage: Option<&Storage>, catalog: Option<&Catalog>) -> PropertyValue {
    match obj {
        PropertyValue::Map(entries) => {
            // Look up by string key using catalog name resolution.
            let key_name = catalog.map(|c| c.property_name(key)).unwrap_or_default();
            entries.iter().find(|(k, _)| k == &key_name).map(|(_, v)| v.clone()).unwrap_or(PropertyValue::Null)
        }
        PropertyValue::Vertex(vref) => {
            if let Some(s) = storage {
                let tx = get_read_tx(s);
...
fn extract_temporal_property(obj: &PropertyValue, key: &str) -> PropertyValue {
    ...
    match obj {
        PropertyValue::Date(date) => {
            let nd = chrono::NaiveDate::from_num_days_from_ce_opt((date.days_since_epoch + 719162) as i32);
            match nd {
                Some(d) => match key_lower.as_str() {
                    ...
                    "dayofweek" | "day_of_week" => PropertyValue::Int(d.weekday().num_days_from_monday() as i64 + 1),
        ...
        PropertyValue::LocalDateTime(ldt) => {
            ...
            match key_lower.as_str() {
                ...
                "microsecond" => PropertyValue::Int(((ndt.nanosecond() as i64) % 1_000_000) / 1_000),
                ...
                "epochseconds" | "epoch_seconds" => PropertyValue::Int(ldt.microseconds / 1_000_000),
                "dayofweek" | "day_of_week" => PropertyValue::Int(ndt.weekday().num_days_from_monday() as i64 + 1),
        ...
        PropertyValue::ZonedDateTime(zdt) => {
            let dt = chrono::DateTime::from_timestamp_micros(zdt.utc_microseconds).unwrap_or_default();
            match key_lower.as_str() {
                ...
                "offset" => PropertyValue::String(format!("{:+03}:{:02}", zdt.offset_minutes / 60, zdt.offset_minutes.abs() % 60)),
                ...
                "epochseconds" | "epoch_seconds" => PropertyValue::Int(zdt.utc_microseconds / 1_000_000),
                "dayofweek" | "day_of_week" => PropertyValue::Int(dt.weekday().num_days_from_monday() as i64 + 1),
...
enum StringPred { StartsWith, EndsWith, Contains }
...
fn string_predicate(
    ...
    let vb = eval_expression_with_catalog(b, bindings, storage, catalog);
    match (&va, &vb) {
        (PropertyValue::String(s), PropertyValue::String(pattern)) => PropertyValue::Bool(match pred {
...
fn get_entity_properties(
    ...
    let fetch = |gid, is_edge: bool| -> Vec<(PropertyId, PropertyValue)> {
        storage.and_then(|s| {
            let tx = get_read_tx(s);
            if is_edge {
                s.get_edge(gid, &tx).map(|snap| snap.properties.iter().map(|(k, v)| (k, v.clone())).collect())
            } else {
                s.get_vertex(gid, &tx).map(|snap| snap.properties.iter().map(|(k, v)| (k, v.clone())).collect())
            }
        }).unwrap_or_default()
    };
    ...
    if let Some(cat) = catalog {
        props.into_iter().map(|(pid, val)| (cat.property_name(pid), val)).collect()
    } else {
        vec![]
    }
}
...
static COUNTERS: std::sync::OnceLock<std::sync::Mutex<std::collections::HashMap<String, i64>>> = std::sync::OnceLock::new();
...
fn eval_function(
    ...
    if name.eq_ignore_ascii_case("id") {
        return args.first().map(|a| eval_expression_with_catalog(a, bindings, storage, catalog)).map(|v| match v {
            ...
            match val {
                PropertyValue::Vertex(vr) => {
                    let label_names: Vec<PropertyValue> = if let Some(cat) = catalog {
                        vr.labels.iter().map(|lid| PropertyValue::String(cat.label_name(*lid))).collect()
                    } else {
                        vec![]
                    };
    ...
    if name.eq_ignore_ascii_case("properties") {
        return args.first().map(|a| eval_expression_with_catalog(a, bindings, storage, catalog)).map(|v| {
            let entries: Vec<(String, PropertyValue)> = get_entity_properties(&v, storage, catalog);
            PropertyValue::Map(entries)
        }).unwrap_or(PropertyValue::Null);
    }
    if name.eq_ignore_ascii_case("keys") {
        return args.first().map(|a| eval_expression_with_catalog(a, bindings, storage, catalog)).map(|v| {
            let keys: Vec<PropertyValue> = match &v {
                PropertyValue::Map(entries) => {
                    entries.iter().map(|(k, _)| PropertyValue::String(k.clone())).collect()
                }
                _ => get_entity_properties(&v, storage, catalog)
                    .into_iter().map(|(k, _)| PropertyValue::String(k)).collect(),
            };
    ...
    if name.eq_ignore_ascii_case("startnode") {
        return args.first().map(|a| eval_expression_with_catalog(a, bindings, storage, catalog)).map(|v| match v {
            PropertyValue::Edge(er) => PropertyValue::Vertex(mgcore::property_value::VertexRef::new(er.from_vertex, vec![], mgcore::property_store::PropertyStore::default())),
            _ => PropertyValue::Null,
        }).unwrap_or(PropertyValue::Null);
    }
    if name.eq_ignore_ascii_case("endnode") {
        return args.first().map(|a| eval_expression_with_catalog(a, bindings, storage, catalog)).map(|v| match v {
            PropertyValue::Edge(er) => PropertyValue::Vertex(mgcore::property_value::VertexRef::new(er.to_vertex, vec![], mgcore::property_store::PropertyStore::default())),
            _ => PropertyValue::Null,
        }).unwrap_or(PropertyValue::Null);
    }
    ...
    if name.eq_ignore_ascii_case("tobooleanornull") {
        return args.first().map(|a| eval_expression_with_catalog(a, bindings, storage, catalog)).map(|v| match v {
            PropertyValue::String(s) if s.eq_ignore_ascii_case("true") => PropertyValue::Bool(true),
            PropertyValue::String(s) if s.eq_ignore_ascii_case("false") => PropertyValue::Bool(false),
            PropertyValue::Bool(b) => PropertyValue::Bool(b),
            _ => PropertyValue::Null,
        }).unwrap_or(PropertyValue::Null);
    }
    ...
    if name.eq_ignore_ascii_case("reverse") {
        return args.first().map(|a| eval_expression_with_catalog(a, bindings, storage, catalog)).map(|v| match v {
            ...
            PropertyValue::List(mut l) => { l.reverse(); PropertyValue::List(l) },
            _ => v,
        }).unwrap_or(PropertyValue::Null);
    }
    ...
    if name.eq_ignore_ascii_case("nodes") && !args.is_empty() {
        return args.first().map(|a| eval_expression_with_catalog(a, bindings, storage, catalog)).map(|v| match v {
            PropertyValue::Path(p) => PropertyValue::List(p.vertices.iter().map(|vr| PropertyValue::Vertex(vr.clone())).collect()),
            _ => PropertyValue::List(vec![]),
        }).unwrap_or(PropertyValue::List(vec![]));
    }
    if name.eq_ignore_ascii_case("relationships") && !args.is_empty() {
        return args.first().map(|a| eval_expression_with_catalog(a, bindings, storage, catalog)).map(|v| match v {
            PropertyValue::Path(p) => PropertyValue::List(p.edges.iter().map(|er| PropertyValue::Edge(er.clone())).collect()),
            _ => PropertyValue::List(vec![]),
        }).unwrap_or(PropertyValue::List(vec![]));
    }
            ...
            PropertyValue::Edge(er) => {
                if let Some(s) = storage {
                    ...
                    s.get_vertex(er.from_vertex, &tx)
                        .map(|snap| PropertyValue::Vertex(mgcore::property_value::VertexRef::new(snap.gid, snap.labels, snap.properties.clone())))
                        .unwrap_or(PropertyValue::Null)
                } else {
                    PropertyValue::Null
                }
    ...
    if name.eq_ignore_ascii_case("values") {
        return args.first().map(|a| eval_expression_with_catalog(a, bindings, storage, catalog)).map(|v| match v {
            PropertyValue::Map(entries) => PropertyValue::List(entries.iter().map(|(_, val)| val.clone()).collect()),
            _ => PropertyValue::List(vec![]),
        }).unwrap_or(PropertyValue::List(vec![]));
    }
    if name.eq_ignore_ascii_case("valuetype") {
        return args.first().map(|a| eval_expression_with_catalog(a, bindings, storage, catalog)).map(|v| {
            PropertyValue::String(match v {
    ...
    if name.eq_ignore_ascii_case("randomuuid") {
        ...
        let uuid = format!(
            ...
            d1 as u32, (d1 >> 32) as u16, d2 as u16, d3 as u16, d4 as u32, d5 as u16,
        );
    ...
    if matches!(name, n if n.eq_ignore_ascii_case("sin") || n.eq_ignore_ascii_case("cos") || n.eq_ignore_ascii_case("tan")
        ...
        return args.first().map(|a| eval_expression_with_catalog(a, bindings, storage, catalog)).map(|v| {
            let x = match v { PropertyValue::Double(f) => f, PropertyValue::Int(n) => n as f64, _ => return PropertyValue::Null };
            let result = match name {
                n if n.eq_ignore_ascii_case("sin") => x.sin(),
                n if n.eq_ignore_ascii_case("cos") => x.cos(),
                n if n.eq_ignore_ascii_case("tan") => x.tan(),
                n if n.eq_ignore_ascii_case("asin") => if x >= -1.0 && x <= 1.0 { x.asin() } else { return PropertyValue::Null },
                n if n.eq_ignore_ascii_case("acos") => if x >= -1.0 && x <= 1.0 { x.acos() } else { return PropertyValue::Null },
                ...
                n if n.eq_ignore_ascii_case("atan2") => {
                    let y = match args.get(1).map(|a| eval_expression_with_catalog(a, bindings, storage, catalog)) {
        ...
        return args.first()
            .map(|a| {
                ...
                match v {
                    ...
                    PropertyValue::String(s) => PropertyValue::List(s.chars().map(|c| PropertyValue::String(c.to_string())).collect()),
                    PropertyValue::Map(entries) => PropertyValue::List(entries.into_iter().map(|(k, v)| PropertyValue::List(vec![PropertyValue::String(k), v])).collect()),
                    PropertyValue::Null => PropertyValue::Null,
                    _ => PropertyValue::Null,
                }
            })
    ...
    if name.eq_ignore_ascii_case("range") {
        if args.len() >= 2 {
            let start = match eval_expression_with_catalog(&args[0], bindings, storage, catalog) {
                PropertyValue::Int(n) => n, _ => return PropertyValue::Null,
            };
            let end = match eval_expression_with_catalog(&args[1], bindings, storage, catalog) {
                PropertyValue::Int(n) => n, _ => return PropertyValue::Null,
            };
            let step = args.get(2).map(|a| match eval_expression_with_catalog(a, bindings, storage, catalog) {
                PropertyValue::Int(n) => n, _ => 1,
            }).unwrap_or(1);
    ...
    match name {
        ...
        n if n.eq_ignore_ascii_case("contains") => {
            let a = args.get(0).map(|e| eval_expression_with_catalog(e, bindings, storage, catalog));
            let b = args.get(1).map(|e| eval_expression_with_catalog(e, bindings, storage, catalog));
            match (a, b) {
                (Some(PropertyValue::List(items)), Some(val)) => PropertyValue::Bool(items.contains(&val)),
                (Some(PropertyValue::String(s)), Some(PropertyValue::String(sub))) => PropertyValue::Bool(s.contains(&sub)),
                _ => PropertyValue::Null,
            }
        }
        ...
        n if n.eq_ignore_ascii_case("size") | n.eq_ignore_ascii_case("length") => {
            args.first()
                .map(|a| match eval_expression_with_catalog(a, bindings, storage, catalog) {
                    PropertyValue::String(s) => PropertyValue::Int(s.len() as i64),
                    PropertyValue::List(l) => PropertyValue::Int(l.len() as i64),
                    PropertyValue::Path(p) => PropertyValue::Int(p.edges.len() as i64),
                    _ => PropertyValue::Int(0),
        ...
        // String functions
        n if n.eq_ignore_ascii_case("trim") => {
            args.first().map(|a| match eval_expression_with_catalog(a, bindings, storage, catalog) {
                PropertyValue::String(s) => PropertyValue::String(s.trim().to_string()),
                v => v,
            }).unwrap_or(PropertyValue::Null)
        }
        n if n.eq_ignore_ascii_case("ltrim") => {
            args.first().map(|a| match eval_expression_with_catalog(a, bindings, storage, catalog) {
                PropertyValue::String(s) => PropertyValue::String(s.trim_start().to_string()),
                v => v,
            }).unwrap_or(PropertyValue::Null)
        }
        n if n.eq_ignore_ascii_case("rtrim") => {
            args.first().map(|a| match eval_expression_with_catalog(a, bindings, storage, catalog) {
        ...
        n if n.eq_ignore_ascii_case("split") => {
            if args.len() == 2 {
                let s = match eval_expression_with_catalog(&args[0], bindings, storage, catalog) {
                    PropertyValue::String(s) => s, _ => return PropertyValue::Null,
                };
                let sep = match eval_expression_with_catalog(&args[1], bindings, storage, catalog) {
                    PropertyValue::String(s) => s, _ => return PropertyValue::Null,
                };
                PropertyValue::List(s.split(&sep).map(|p| PropertyValue::String(p.to_string())).collect())
            } else { PropertyValue::Null }
        }
        ...
        n if n.eq_ignore_ascii_case("substring") => {
            if args.len() >= 2 {
                ...
                let start = match eval_expression_with_catalog(&args[1], bindings, storage, catalog) {
                    PropertyValue::Int(n) => n as usize, _ => return PropertyValue::Null,
                };
                let len = args.get(2).map(|a| match eval_expression_with_catalog(a, bindings, storage, catalog) {
                    PropertyValue::Int(n) => n as usize, _ => 0,
                });
                if let Some(l) = len {
        ...
        n if n.eq_ignore_ascii_case("right") => {
            if args.len() == 2 {
                ...
                PropertyValue::String(s.chars().rev().take(n).collect::<String>().chars().rev().collect())
            } else { PropertyValue::Null }
        }
        n if n.eq_ignore_ascii_case("reverse") => {
            args.first().map(|a| match eval_expression_with_catalog(a, bindings, storage, catalog) {
                PropertyValue::String(s) => PropertyValue::String(s.chars().rev().collect()),
        ...
        n if n.eq_ignore_ascii_case("tofloat") => {
            args.first().map(|a| match eval_expression_with_catalog(a, bindings, storage, catalog) {
                PropertyValue::Int(n) => PropertyValue::Double(n as f64),
                PropertyValue::String(s) => s.parse::<f64>().ok().map(PropertyValue::Double).unwrap_or(PropertyValue::Null),
                v => v,
            }).unwrap_or(PropertyValue::Null)
        }
        n if n.eq_ignore_ascii_case("tointeger") | n.eq_ignore_ascii_case("toint") => {
            args.first().map(|a| match eval_expression_with_catalog(a, bindings, storage, catalog) {
                PropertyValue::Double(f) => PropertyValue::Int(f as i64),
                PropertyValue::String(s) => s.parse::<i64>().ok().map(PropertyValue::Int).unwrap_or(PropertyValue::Null),
                v => v,
            }).unwrap_or(PropertyValue::Null)
        }
        n if n.eq_ignore_ascii_case("tostringlist") => {
            args.first().map(|a| match eval_expression_with_catalog(a, bindings, storage, catalog) {
                PropertyValue::List(items) => PropertyValue::List(items.into_iter().map(|v| match v {
        ...
        n if n.eq_ignore_ascii_case("tointegerlist") | n.eq_ignore_ascii_case("tointlist") => {
            args.first().map(|a| match eval_expression_with_catalog(a, bindings, storage, catalog) {
                PropertyValue::List(items) => PropertyValue::List(items.into_iter().map(|v| match v {
        ...
        n if n.eq_ignore_ascii_case("tofloatlist") => {
            args.first().map(|a| match eval_expression_with_catalog(a, bindings, storage, catalog) {
                PropertyValue::List(items) => PropertyValue::List(items.into_iter().map(|v| match v {
        ...
        n if n.eq_ignore_ascii_case("tobooleanlist") => {
            args.first().map(|a| match eval_expression_with_catalog(a, bindings, storage, catalog) {
                PropertyValue::List(items) => PropertyValue::List(items.into_iter().map(|v| match v {
                    PropertyValue::Bool(b) => PropertyValue::Bool(b),
                    PropertyValue::String(s) if s.eq_ignore_ascii_case("true") => PropertyValue::Bool(true),
        ...
        n if n.eq_ignore_ascii_case("floor") => {
            args.first().map(|a| match eval_expression_with_catalog(a, bindings, storage, catalog) {
        ...
        n if n.eq_ignore_ascii_case("round") => {
            let val = args.first().map(|a| eval_expression_with_catalog(a, bindings, storage, catalog));
            let precision = args.get(1).map(|a| match eval_expression_with_catalog(a, bindings, storage, catalog) {
        ...
        n if n.eq_ignore_ascii_case("sqrt") => {
            args.first().map(|a| match eval_expression_with_catalog(a, bindings, storage, catalog) {
        ...
        n if n.eq_ignore_ascii_case("sign") | n.eq_ignore_ascii_case("signum") => {
            args.first().map(|a| match eval_expression_with_catalog(a, bindings, storage, catalog) {
        ...
        n if n.eq_ignore_ascii_case("log") | n.eq_ignore_ascii_case("ln") => {
            args.first().map(|a| match eval_expression_with_catalog(a, bindings, storage, catalog) {
        ...
        n if n.eq_ignore_ascii_case("log10") => {
            args.first().map(|a| match eval_expression_with_catalog(a, bindings, storage, catalog) {
        ...
        n if n.eq_ignore_ascii_case("exp") => {
            args.first().map(|a| match eval_expression_with_catalog(a, bindings, storage, catalog) {
        ...
        n if n.eq_ignore_ascii_case("pow") | n.eq_ignore_ascii_case("power") => {
            if args.len() == 2 {
                let base = match eval_expression_with_catalog(&args[0], bindings, storage, catalog) {
                    PropertyValue::Double(f) => f, PropertyValue::Int(n) => n as f64, _ => return PropertyValue::Null,
                };
        ...
        // List functions
        n if n.eq_ignore_ascii_case("head") => {
            args.first().map(|a| match eval_expression_with_catalog(a, bindings, storage, catalog) {
                PropertyValue::List(l) => l.into_iter().next().unwrap_or(PropertyValue::Null),
                PropertyValue::Path(p) => p.vertices.first().cloned().map(PropertyValue::Vertex).unwrap_or(PropertyValue::Null),
                _ => PropertyValue::Null,
            }).unwrap_or(PropertyValue::Null)
        }
        n if n.eq_ignore_ascii_case("last") => {
            args.first().map(|a| match eval_expression_with_catalog(a, bindings, storage, catalog) {
                PropertyValue::List(l) => l.into_iter().last().unwrap_or(PropertyValue::Null),
                PropertyValue::Path(p) => p.vertices.last().cloned().map(PropertyValue::Vertex).unwrap_or(PropertyValue::Null),
                _ => PropertyValue::Null,
            }).unwrap_or(PropertyValue::Null)
        }
        n if n.eq_ignore_ascii_case("tail") => {
            args.first().map(|a| match eval_expression_with_catalog(a, bindings, storage, catalog) {
                PropertyValue::List(mut l) => { if !l.is_empty() { l.remove(0); } PropertyValue::List(l) },
                _ => PropertyValue::Null,
            }).unwrap_or(PropertyValue::Null)
        }
        ...
        n if n.eq_ignore_ascii_case("apoc_coll_union") || n.eq_ignore_ascii_case("union") => {
            if args.len() == 2 {
                let a = match eval_expression_with_catalog(&args[0], bindings, storage, catalog) {
                    PropertyValue::List(l) => l, _ => return PropertyValue::Null,
                };
        ...
        n if n.eq_ignore_ascii_case("apoc_coll_intersection") || n.eq_ignore_ascii_case("intersection") => {
            if args.len() == 2 {
                let a = match eval_expression_with_catalog(&args[0], bindings, storage, catalog) {
                    PropertyValue::List(l) => l, _ => return PropertyValue::Null,
                };
        ...
        n if n.eq_ignore_ascii_case("apoc_coll_containsall") || n.eq_ignore_ascii_case("contains_all") => {
            if args.len() == 2 {
                let list = match eval_expression_with_catalog(&args[0], bindings, storage, catalog) {
                    PropertyValue::List(l) => l, _ => return PropertyValue::Null,
                };
        ...
        n if n.eq_ignore_ascii_case("apoc_coll_set") || n.eq_ignore_ascii_case("set_collection") => {
            args.first().map(|a| match eval_expression_with_catalog(a, bindings, storage, catalog) {
        ...
        n if n.eq_ignore_ascii_case("apoc_coll_sort") || n.eq_ignore_ascii_case("sort") => {
            args.first().map(|a| match eval_expression_with_catalog(a, bindings, storage, catalog) {
                PropertyValue::List(mut l) => {
                    l.sort_by(|a, b| match (a, b) {
                        (PropertyValue::Int(x), PropertyValue::Int(y)) => x.cmp(y),
                        (PropertyValue::Double(x), PropertyValue::Double(y)) => x.partial_cmp(y).unwrap_or(std::cmp::Ordering::Equal),
        ...
        n if n.eq_ignore_ascii_case("apoc_coll_remove") || n.eq_ignore_ascii_case("remove") => {
            if args.len() == 2 {
                let mut list = match eval_expression_with_catalog(&args[0], bindings, storage, catalog) {
                    PropertyValue::List(l) => l, _ => return PropertyValue::Null,
                };
        ...
        n if n.eq_ignore_ascii_case("apoc_map_merge") || n.eq_ignore_ascii_case("merge_maps") || n.eq_ignore_ascii_case("merge") => {
            let mut result = Vec::new();
            for a in args {
                if let PropertyValue::Map(entries) = eval_expression_with_catalog(a, bindings, storage, catalog) {
                    for (k, v) in entries {
                        // Later maps override earlier ones
                        if let Some(pos) = result.iter().position(|(key, _): &(String, PropertyValue)| key == &k) {
                            result[pos] = (k, v);
                        } else {
                            result.push((k, v));
                        }
        ...
        n if n.eq_ignore_ascii_case("apoc_text_join") || n.eq_ignore_ascii_case("text_join") || n.eq_ignore_ascii_case("join") => {
            if args.len() == 2 {
                let list = match eval_expression_with_catalog(&args[0], bindings, storage, catalog) {
                    PropertyValue::List(l) => l, _ => return PropertyValue::Null,
                };
        ...
        n if n.eq_ignore_ascii_case("apoc_text_capitalize") || n.eq_ignore_ascii_case("capitalize") => {
            args.first().map(|a| match eval_expression_with_catalog(a, bindings, storage, catalog) {
                PropertyValue::String(s) => {
                    let mut chars = s.chars();
                    match chars.next() {
                        Some(first) => PropertyValue::String(format!("{}{}", first.to_uppercase(), chars.as_str().to_lowercase())),
        ...
        n if n.eq_ignore_ascii_case("apoc_text_decapitalize") || n.eq_ignore_ascii_case("decapitalize") => {
            args.first().map(|a| match eval_expression_with_catalog(a, bindings, storage, catalog) {
                PropertyValue::String(s) => {
                    let mut chars = s.chars();
                    match chars.next() {
                        Some(first) => PropertyValue::String(format!("{}{}", first.to_lowercase(), chars.as_str())),
                        None => PropertyValue::String(s),
                    }
                }
                _ => PropertyValue::Null,
        ...
        n if n.eq_ignore_ascii_case("apoc_text_format") || n.eq_ignore_ascii_case("apoc_date_format") || n.eq_ignore_ascii_case("format") => {
            if args.is_empty() { return PropertyValue::Null; }
            match eval_expression_with_catalog(&args[0], bindings, storage, catalog) {
                // apoc.date.format(timestamp, pattern)
                PropertyValue::Int(ts) => {
                    if args.len() < 2 { return PropertyValue::Null; }
                ...
                PropertyValue::String(fmt) => {
                    let vals: Vec<String> = args[1..].iter().map(|a| match eval_expression_with_catalog(a, bindings, storage, catalog) {
        ...
        n if n.eq_ignore_ascii_case("apoc_date_currenttimestamp") || n.eq_ignore_ascii_case("current_timestamp_ms") || n.eq_ignore_ascii_case("currentTimestamp") => {
            let now = std::time::SystemTime::now()
                .duration_since(std::time::UNIX_EPOCH)
                .unwrap_or_default()
                .as_millis() as i64;
        ...
        n if n.eq_ignore_ascii_case("apoc_date_parse") || n.eq_ignore_ascii_case("date_parse") => {
            if args.len() >= 2 {
                ...
                let tz = if args.len() >= 3 {
                    match eval_expression_with_catalog(&args[2], bindings, storage, catalog) {
                        PropertyValue::String(s) => Some(s), _ => None,
                    }
                } else { None };
        ...
        n if n.eq_ignore_ascii_case("apoc_date_format") || n.eq_ignore_ascii_case("date_format") => {
            if args.len() >= 2 {
        ...
        n if n.eq_ignore_ascii_case("apoc_date_add") || n.eq_ignore_ascii_case("date_add") => {
            if args.len() == 3 {
                ...
                let unit = match eval_expression_with_catalog(&args[2], bindings, storage, catalog) {
                    PropertyValue::String(s) => s.to_lowercase(), _ => return PropertyValue::Null,
                };
        ...
        n if n.eq_ignore_ascii_case("apoc_date_fields") || n.eq_ignore_ascii_case("date_fields") => {
            if args.len() >= 1 {
                let ts = match eval_expression_with_catalog(&args[0], bindings, storage, catalog) {
                    PropertyValue::Int(n) => n, _ => return PropertyValue::Null,
                };
                let tz = if args.len() >= 2 {
                    match eval_expression_with_catalog(&args[1], bindings, storage, catalog) {
                        PropertyValue::String(s) => match s.parse::<chrono::FixedOffset>() {
                            Ok(off) => Some(off),
                            _ => None,
                        },
                        _ => None,
                ...
                let dt: Option<chrono::DateTime<chrono::FixedOffset>> = if let Some(off) = tz {
                    ...
                    chrono::DateTime::from_timestamp_millis(ts).map(|d| d.with_timezone(&chrono::FixedOffset::east_opt(0).unwrap()))
                };
                match dt {
                    Some(dt) => {
                        let mut fields = Vec::new();
                        fields.push(("year".to_string(), PropertyValue::Int(dt.year() as i64)));
                        fields.push(("month".to_string(), PropertyValue::Int(dt.month() as i64)));
                        fields.push(("day".to_string(), PropertyValue::Int(dt.day() as i64)));
                        fields.push(("hour".to_string(), PropertyValue::Int(dt.hour() as i64)));
                        fields.push(("minute".to_string(), PropertyValue::Int(dt.minute() as i64)));
                        fields.push(("second".to_string(), PropertyValue::Int(dt.second() as i64)));
                        fields.push(("millisecond".to_string(), PropertyValue::Int((ts % 1000).abs())));
                        fields.push(("dayOfWeek".to_string(), PropertyValue::Int(dt.weekday().num_days_from_monday() as i64 + 1)));
                        fields.push(("dayOfYear".to_string(), PropertyValue::Int(dt.ordinal() as i64)));
        ...
        n if n.eq_ignore_ascii_case("apoc_coll_sum") || n.eq_ignore_ascii_case("sum_list") => {
            args.first().map(|a| match eval_expression_with_catalog(a, bindings, storage, catalog) {
                PropertyValue::List(l) => {
                    let mut total = 0i64;
                    for item in l {
                        match item {
                            ...
                            PropertyValue::Double(d) => return PropertyValue::Double(total as f64 + d),
                            _ => {}
                        }
                    }
        ...
        n if n.eq_ignore_ascii_case("apoc_coll_avg") || n.eq_ignore_ascii_case("avg_list") => {
            args.first().map(|a| match eval_expression_with_catalog(a, bindings, storage, catalog) {
                PropertyValue::List(l) => {
                    ...
                    for item in l {
                        match item {
                            PropertyValue::Int(n) => { total += n as f64; count += 1; }
                            PropertyValue::Double(d) => { total += d; count += 1; }
                            _ => {}
                        }
                    }
                    if count > 0 { PropertyValue::Double(total / count as f64) } else { PropertyValue::Null }
                }
                _ => PropertyValue::Null,
            }).unwrap_or(PropertyValue::Null)
        }
        n if n.eq_ignore_ascii_case("apoc_map_setKey") || n.eq_ignore_ascii_case("set_key") || n.eq_ignore_ascii_case("setkey") => {
            if args.len() == 3 {
                let mut map = match eval_expression_with_catalog(&args[0], bindings, storage, catalog) {
                    PropertyValue::Map(m) => m, _ => return PropertyValue::Null,
                };
                ...
                if let Some(pos) = map.iter().position(|(k, _): &(String, PropertyValue)| k == &key) {
                    map[pos] = (key, value);
                } else {
                    map.push((key, value));
                }
        ...
        n if n.eq_ignore_ascii_case("apoc_text_replace") || n.eq_ignore_ascii_case("replace_all") => {
            if args.len() == 3 {
                let text = match eval_expression_with_catalog(&args[0], bindings, storage, catalog) {
                    PropertyValue::String(s) => s, _ => return PropertyValue::Null,
                };
        ...
        n if n.eq_ignore_ascii_case("apoc_text_split") || n.eq_ignore_ascii_case("split_text") => {
            if args.len() == 2 {
                ...
                PropertyValue::List(text.split(&sep).map(|s| PropertyValue::String(s.to_string())).collect())
            } else { PropertyValue::Null }
        }
            ...
            if args.len() == 2 {
                ...
                match (arg0, arg1) {
                    ...
                    (PropertyValue::List(list), needle) => {
                        PropertyValue::Int(list.iter().position(|x| x == &needle).map(|i| i as i64).unwrap_or(-1))
                    }
                    _ => PropertyValue::Null,
                }
            } else { PropertyValue::Null }
        ...
        n if n.eq_ignore_ascii_case("apoc_text_regreplace") || n.eq_ignore_ascii_case("regreplace") => {
            if args.len() == 3 {
                let text = match eval_expression_with_catalog(&args[0], bindings, storage, catalog) {
                    PropertyValue::String(s) => s, _ => return PropertyValue::Null,
                };
                let pattern = match eval_expression_with_catalog(&args[1], bindings, storage, catalog) {
                    PropertyValue::String(s) => s, _ => return PropertyValue::Null,
                };
                let replacement = match eval_expression_with_catalog(&args[2], bindings, storage, catalog) {
                    PropertyValue::String(s) => s, _ => return PropertyValue::Null,
                };
        ...
        n if n.eq_ignore_ascii_case("apoc_text_random") || n.eq_ignore_ascii_case("random_string") => {
            let len = if args.len() >= 1 {
                match eval_expression_with_catalog(&args[0], bindings, storage, catalog) {
                    PropertyValue::Int(n) => n as usize, _ => 10,
                }
            } else { 10 };
            ...
            let s: String = (0..len).map(|_| *chars.choose(&mut rng).unwrap_or(&'a')).collect();
            PropertyValue::String(s)
        }
        n if n.eq_ignore_ascii_case("apoc_text_compareignorecase") || n.eq_ignore_ascii_case("compareignorecase") => {
            if args.len() == 2 {
                let a = match eval_expression_with_catalog(&args[0], bindings, storage, catalog) {
                    PropertyValue::String(s) => s, _ => return PropertyValue::Null,
                };
        ...
        n if n.eq_ignore_ascii_case("apoc_text_charat") || n.eq_ignore_ascii_case("charat") => {
            if args.len() == 2 {
                ...
                PropertyValue::String(text.chars().nth(idx).map(|c| c.to_string()).unwrap_or_default())
            } else { PropertyValue::Null }
        }
        n if n.eq_ignore_ascii_case("apoc_text_camelcase") || n.eq_ignore_ascii_case("camelcase") => {
            if args.len() == 1 {
                let text = match eval_expression_with_catalog(&args[0], bindings, storage, catalog) {
                    PropertyValue::String(s) => s, _ => return PropertyValue::Null,
                };
        ...
        n if n.eq_ignore_ascii_case("apoc_text_snakecase") || n.eq_ignore_ascii_case("snakecase") => {
            if args.len() == 1 {
                let text = match eval_expression_with_catalog(&args[0], bindings, storage, catalog) {
                    PropertyValue::String(s) => s, _ => return PropertyValue::Null,
                };
                ...
                match eval_expression_with_catalog(&args[0], bindings, storage, catalog) {
                    PropertyValue::String(s) => {
                        if let Ok(dt) = s.parse::<chrono::DateTime<chrono::Utc>>() {
                            PropertyValue::ZonedDateTime(ZonedDateTime::new(dt.timestamp_micros(), 0, "UTC".into()))
                        } else {
                            PropertyValue::Null
                        }
                    }
                    PropertyValue::Map(entries) => {
                        ...
                        let mut offset_min = 0i16;
                        let mut has_date_time_field = false;
                        let mut has_timezone = false;
                        for (k, v) in &entries {
                            match (k.as_str(), v) {
                                ("year", PropertyValue::Int(y)) => { year = *y as i32; has_date_time_field = true; }
                                ("month", PropertyValue::Int(m)) => { month = *m as u32; has_date_time_field = true; }
                                ("day", PropertyValue::Int(d)) => { day = *d as u32; has_date_time_field = true; }
                                ("hour", PropertyValue::Int(h)) => { hour = *h as u32; has_date_time_field = true; }
                                ("minute", PropertyValue::Int(m)) => { minute = *m as u32; has_date_time_field = true; }
                                ("second", PropertyValue::Int(s)) => { second = *s as u32; has_date_time_field = true; }
                                ("millisecond", PropertyValue::Int(ms)) => { nanosecond = (*ms as u32) * 1_000_000; has_date_time_field = true; }
                                ("microsecond", PropertyValue::Int(us)) => { nanosecond = (*us as u32) * 1_000; has_date_time_field = true; }
                                ("nanosecond", PropertyValue::Int(ns)) => { nanosecond = *ns as u32; has_date_time_field = true; }
                                ("timezone", PropertyValue::String(s)) => { tz = s.clone(); has_timezone = true; }
                                _ => {}
                            }
                        }
                        ...
                        if !has_date_time_field && has_timezone {
                            let now = chrono::Utc::now();
                            let us = now.timestamp_micros();
                            return PropertyValue::ZonedDateTime(ZonedDateTime::new(us, offset_min, tz));
                        }
        ...
        n if n.eq_ignore_ascii_case("localdatetime") => {
            if args.is_empty() {
                ...
                PropertyValue::LocalDateTime(LocalDateTime::from_microseconds(now.timestamp_micros()))
            } else {
                match eval_expression_with_catalog(&args[0], bindings, storage, catalog) {
                    PropertyValue::String(s) => {
                        if let Ok(dt) = s.parse::<chrono::NaiveDateTime>() {
                            PropertyValue::LocalDateTime(LocalDateTime::from_microseconds(dt.and_utc().timestamp_micros()))
                        } else {
                            PropertyValue::Null
                        }
                    }
                    PropertyValue::Map(entries) => {
                        ...
                        for (k, v) in entries {
                            match (k.as_str(), v) {
                                ...
                                ("millisecond", PropertyValue::Int(ms)) => nanosecond = (ms as u32) * 1_000_000,
                                ("microsecond", PropertyValue::Int(us)) => nanosecond = (us as u32) * 1_000,
                ...
                match eval_expression_with_catalog(&args[0], bindings, storage, catalog) {
                    PropertyValue::String(s) => {
                        if let Ok(t) = s.parse::<chrono::NaiveTime>() {
                            let us = t.num_seconds_from_midnight() as i64 * 1_000_000 + t.nanosecond() as i64 / 1_000;
                            PropertyValue::LocalTime(LocalTime::from_microseconds(us))
                        } else {
                            PropertyValue::Null
                        }
                    ...
                    PropertyValue::Map(entries) => {
                        ...
                        if let Some(t) = chrono::NaiveTime::from_hms_nano_opt(hour, minute, second, nanosecond) {
                            let us = t.num_seconds_from_midnight() as i64 * 1_000_000 + t.nanosecond() as i64 / 1_000;
                            PropertyValue::LocalTime(LocalTime::from_microseconds(us))
                        } else {
                            PropertyValue::Null
                    ...
                    PropertyValue::Map(entries) => {
                        ...
                        for (k, v) in entries {
                            match (k.as_str(), v) {
                                ...
                                ("hours", PropertyValue::Int(h)) => microseconds += h * 3_600_000_000,
                                ("minutes", PropertyValue::Int(m)) => microseconds += m * 60_000_000,
                                ("seconds", PropertyValue::Int(s)) => microseconds += s * 1_000_000,
                                ("milliseconds", PropertyValue::Int(ms)) => microseconds += ms * 1_000,
                                ("microseconds", PropertyValue::Int(us)) => microseconds += us,
                                ("nanoseconds", PropertyValue::Int(ns)) => microseconds += ns / 1_000,
                                _ => {}
                            }
                        }
        ...
        n if n.eq_ignore_ascii_case("startswith") => {
            if args.len() == 2 {
                ...
                let prefix = match eval_expression_with_catalog(&args[1], bindings, storage, catalog) {
                    PropertyValue::String(s) => s, _ => return PropertyValue::Null,
                };
        ...
        n if n.eq_ignore_ascii_case("endswith") => {
            if args.len() == 2 {
                ...
                let suffix = match eval_expression_with_catalog(&args[1], bindings, storage, catalog) {
                    PropertyValue::String(s) => s, _ => return PropertyValue::Null,
                };
                PropertyValue::Bool(s.ends_with(&suffix))
            } else { PropertyValue::Null }
        }
        n if n.eq_ignore_ascii_case("apoc_text_startswith") || n.eq_ignore_ascii_case("starts_with") => {
            if args.len() == 2 {
                let s = match eval_expression_with_catalog(&args[0], bindings, storage, catalog) {
                    PropertyValue::String(s) => s, _ => return PropertyValue::Null,
                };
        ...
        n if n.eq_ignore_ascii_case("apoc_text_endswith") || n.eq_ignore_ascii_case("ends_with") => {
            if args.len() == 2 {
                let s = match eval_expression_with_catalog(&args[0], bindings, storage, catalog) {
                    PropertyValue::String(s) => s, _ => return PropertyValue::Null,
                };
        ...
        n if n.eq_ignore_ascii_case("apoc_text_urlencode") || n.eq_ignore_ascii_case("urlencode") => {
            if args.len() == 1 {
                let s = match eval_expression_with_catalog(&args[0], bindings, storage, catalog) {
                    PropertyValue::String(s) => s, _ => return PropertyValue::Null,
                };
        ...
        n if n.eq_ignore_ascii_case("apoc_text_urldecode") || n.eq_ignore_ascii_case("urldecode") => {
            if args.len() == 1 {
                let s = match eval_expression_with_catalog(&args[0], bindings, storage, catalog) {
                    PropertyValue::String(s) => s, _ => return PropertyValue::Null,
                };
        ...
        n if n.eq_ignore_ascii_case("apoc_text_base64encode") || n.eq_ignore_ascii_case("base64encode") => {
            if args.len() == 1 {
                let s = match eval_expression_with_catalog(&args[0], bindings, storage, catalog) {
                    PropertyValue::String(s) => s, _ => return PropertyValue::Null,
                };
        ...
        n if n.eq_ignore_ascii_case("apoc_text_base64decode") || n.eq_ignore_ascii_case("base64decode") => {
            if args.len() == 1 {
                let s = match eval_expression_with_catalog(&args[0], bindings, storage, catalog) {
                    PropertyValue::String(s) => s, _ => return PropertyValue::Null,
                };
        ...
        n if n.eq_ignore_ascii_case("apoc_text_striptags") || n.eq_ignore_ascii_case("strip_tags") => {
            if args.len() == 1 {
                let s = match eval_expression_with_catalog(&args[0], bindings, storage, catalog) {
                    PropertyValue::String(s) => s, _ => return PropertyValue::Null,
                };
        ...
        n if n.eq_ignore_ascii_case("apoc_text_doublemetaphone") || n.eq_ignore_ascii_case("double_metaphone") => {
            if args.len() == 1 {
                let s = match eval_expression_with_catalog(&args[0], bindings, storage, catalog) {
                    PropertyValue::String(s) => s, _ => return PropertyValue::Null,
                };
                let (primary, secondary) = double_metaphone(&s);
                PropertyValue::List(vec![PropertyValue::String(primary), PropertyValue::String(secondary)])
            } else { PropertyValue::Null }
        }
                        ...
                        min_val = Some(match min_val {
                            Some(ref current) => {
                                let is_less = match (current, &candidate) {
                                    ...
                                    (PropertyValue::Double(x), PropertyValue::Double(y)) => y < x,
                                    (PropertyValue::String(x), PropertyValue::String(y)) => y < x,
                                    _ => false,
                                };
                                if is_less { candidate } else { current.clone() }
                            }
                            None => candidate,
                        });
                    }
        ...
        n if n.eq_ignore_ascii_case("max") => {
            args.first().map(|a| match eval_expression_with_catalog(a, bindings, storage, catalog) {
                PropertyValue::List(l) => {
                    ...
                    for item in l {
                        ...
                        max_val = Some(match max_val {
                            Some(ref current) => {
                                ...
                                if is_greater { candidate } else { current.clone() }
                            }
                            None => candidate,
                        });
                    }
        ...
        n if n.eq_ignore_ascii_case("reduce") => {
            if args.len() == 4 {
                ...
                let acc_initial = eval_expression_with_catalog(&args[0], bindings, storage, catalog);
        ...
        n if n.eq_ignore_ascii_case("filter") => {
            if args.len() >= 3 {
                ...
                let result: Vec<PropertyValue> = list.into_iter().filter(|item| {
                    let mut local = bindings.clone();
        ...
        n if n.eq_ignore_ascii_case("nodes") && args.is_empty() => {
            if let Some(s) = storage {
                ...
                PropertyValue::List(vertices.into_iter().map(|(gid, labels, _)| {
                    PropertyValue::Vertex(VertexRef::new(gid, labels, mgcore::property_store::PropertyStore::default()))
                }).collect())
            } else { PropertyValue::List(vec![]) }
        }
        n if n.eq_ignore_ascii_case("relationships") && args.is_empty() => {
            if let Some(s) = storage {
                let edges = s.all_edges();
                PropertyValue::List(edges.into_iter().map(|(gid, from, to, etype, _)| {
                    PropertyValue::Edge(EdgeRefValue::new(gid, etype, from, to, mgcore::property_store::PropertyStore::default()))
                }).collect())
            } else { PropertyValue::List(vec![]) }
        }
        ...
        n if n.eq_ignore_ascii_case("apoc_map_setkey") => {
            let a = args.get(0).map(|e| eval_expression_with_catalog(e, bindings, storage, catalog));
            let k = args.get(1).map(|e| eval_expression_with_catalog(e, bindings, storage, catalog));
            let v = args.get(2).map(|e| eval_expression_with_catalog(e, bindings, storage, catalog));
            match (a, k, v) {
        ...
        n if n.eq_ignore_ascii_case("apoc_text_join") => {
            let a = args.get(0).map(|e| eval_expression_with_catalog(e, bindings, storage, catalog));
            let sep = args.get(1).map(|e| eval_expression_with_catalog(e, bindings, storage, catalog));
            match (a, sep) {
                (Some(PropertyValue::List(items)), Some(PropertyValue::String(s))) => {
                    let strings: Vec<String> = items.iter().filter_map(|v| match v {
        ...
        n if n.eq_ignore_ascii_case("apoc_text_replace") => {
            let a = args.get(0).map(|e| eval_expression_with_catalog(e, bindings, storage, catalog));
            let old = args.get(1).map(|e| eval_expression_with_catalog(e, bindings, storage, catalog));
            let new = args.get(2).map(|e| eval_expression_with_catalog(e, bindings, storage, catalog));
            match (a, old, new) {
                (Some(PropertyValue::String(s)), Some(PropertyValue::String(o)), Some(PropertyValue::String(n))) => {
        ...
        n if n.eq_ignore_ascii_case("apoc_coll_sortmaps") || n.eq_ignore_ascii_case("sort_maps") => {
            if args.len() == 2 {
                let list = match eval_expression_with_catalog(&args[0], bindings, storage, catalog) {
                    PropertyValue::List(l) => l, _ => return PropertyValue::Null,
                };
                ...
                let mut maps: Vec<(PropertyValue, Vec<(String, PropertyValue)>)> = list.into_iter()
                    .filter_map(|v| match v {
                        PropertyValue::Map(entries) => {
                            let val = entries.iter().find(|(k, _)| k == &key).map(|(_, v)| v.clone());
                ...
                PropertyValue::List(maps.into_iter().map(|(_, e)| PropertyValue::Map(e)).collect())
            } else { PropertyValue::Null }
        }
        ...
        n if n.eq_ignore_ascii_case("apoc_coll_remove") => {
            let a = args.get(0).map(|e| eval_expression_with_catalog(e, bindings, storage, catalog));
            let idx = args.get(1).map(|e| eval_expression_with_catalog(e, bindings, storage, catalog));
            match (a, idx) {
                (Some(PropertyValue::List(mut items)), Some(PropertyValue::Int(i))) => {
                    let pos = if i >= 0 { i as usize } else { (items.len() as i64 + i) as usize };
                    if pos < items.len() {
                        items.remove(pos);
                    }
                    PropertyValue::List(items)
        ...
        n if n.eq_ignore_ascii_case("apoc_coll_sum") => {
            ...
            match a {
                Some(PropertyValue::List(items)) => {
                    let sum: i64 = items.iter().filter_map(|v| match v {
        ...
        n if n.eq_ignore_ascii_case("apoc_coll_avg") => {
            ...
            match a {
                Some(PropertyValue::List(items)) => {
                    let ints: Vec<i64> = items.iter().filter_map(|v| match v {
                        PropertyValue::Int(n) => Some(*n),
                        _ => None,
                    }).collect();
                    if ints.is_empty() { PropertyValue::Null }
                    else { PropertyValue::Double(ints.iter().sum::<i64>() as f64 / ints.len() as f64) }
                }
                _ => PropertyValue::Null,
            }
        }
        // ─── More APOC list functions ──────────────────────────────
        n if n.eq_ignore_ascii_case("apoc_coll_different") || n.eq_ignore_ascii_case("different") => {
            args.first().map(|a| match eval_expression_with_catalog(a, bindings, storage, catalog) {
                PropertyValue::List(l) => {
                    ...
                    for item in l {
                        let key = format!("{:?}", item);
                        if seen.insert(key) { result.push(item); }
                    }
                    PropertyValue::List(result)
                }
                _ => PropertyValue::Null,
        ...
        n if n.eq_ignore_ascii_case("apoc_coll_frequencies") || n.eq_ignore_ascii_case("frequencies") => {
            args.first().map(|a| match eval_expression_with_catalog(a, bindings, storage, catalog) {
                PropertyValue::List(l) => {
                    ...
                    let result: Vec<PropertyValue> = counts.into_values()
                        .map(|(item, count)| PropertyValue::Map(vec![
                            ("item".to_string(), item),
        ...
        n if n.eq_ignore_ascii_case("apoc_coll_frequencies_as_map") || n.eq_ignore_ascii_case("frequencies_as_map") || n.eq_ignore_ascii_case("frequenciesAsMap") => {
            args.first().map(|a| match eval_expression_with_catalog(a, bindings, storage, catalog) {
                PropertyValue::List(l) => {
                    ...
                    let result: Vec<(String, PropertyValue)> = counts.into_iter()
                        .map(|(k, v)| (k, PropertyValue::Int(v))).collect();
                    PropertyValue::Map(result)
                }
                _ => PropertyValue::Null,
            }).unwrap_or(PropertyValue::Null)
        ...
        n if n.eq_ignore_ascii_case("apoc_coll_pairs") || n.eq_ignore_ascii_case("pairs") => {
            args.first().map(|a| match eval_expression_with_catalog(a, bindings, storage, catalog) {
                PropertyValue::List(l) => {
                    let mut result = Vec::new();
                    for i in 0..l.len() {
        ...
        n if n.eq_ignore_ascii_case("apoc_coll_max") || n.eq_ignore_ascii_case("list_max") => {
            args.first().map(|a| match eval_expression_with_catalog(a, bindings, storage, catalog) {
                PropertyValue::List(l) => {
                    let mut max_val: Option<&PropertyValue> = None;
                    for item in &l {
                        match item {
                            PropertyValue::Int(_) | PropertyValue::Double(_) | PropertyValue::String(_) => {
                                if let Some(current) = max_val {
                                    if compare_values(item, current) == std::cmp::Ordering::Greater {
                                        max_val = Some(item);
                                    }
                                } else {
                                    max_val = Some(item);
        ...
        n if n.eq_ignore_ascii_case("apoc_coll_min") || n.eq_ignore_ascii_case("list_min") => {
            args.first().map(|a| match eval_expression_with_catalog(a, bindings, storage, catalog) {
                PropertyValue::List(l) => {
                    let mut min_val: Option<&PropertyValue> = None;
                    for item in &l {
        ...
        n if n.eq_ignore_ascii_case("apoc_convert_toset") || n.eq_ignore_ascii_case("toset") || n.eq_ignore_ascii_case("to_set") => {
            args.first().map(|a| match eval_expression_with_catalog(a, bindings, storage, catalog) {
                PropertyValue::List(l) => {
                    let mut seen = std::collections::HashSet::new();
                    let mut result = Vec::new();
        ...
        n if n.eq_ignore_ascii_case("apoc_coll_combinations") || n.eq_ignore_ascii_case("combinations") => {
            if args.len() == 2 {
                let list = match eval_expression_with_catalog(&args[0], bindings, storage, catalog) {
                    PropertyValue::List(l) => l, _ => return PropertyValue::Null,
                };
                ...
                let result = combos(&list, k).into_iter().map(PropertyValue::List).collect();
                PropertyValue::List(result)
            } else { PropertyValue::Null }
        }
        n if n.eq_ignore_ascii_case("apoc_coll_subtract") || n.eq_ignore_ascii_case("subtract") => {
            if args.len() == 2 {
                ...
                let b_keys: std::collections::HashSet<String> = b.iter().map(|v| format!("{:?}", v)).collect();
                let result: Vec<PropertyValue> = a.into_iter()
                    .filter(|v| !b_keys.contains(&format!("{:?}", v)))
                    .collect();
                PropertyValue::List(result)
            } else { PropertyValue::Null }
        }
        n if n.eq_ignore_ascii_case("apoc_coll_unionall") || n.eq_ignore_ascii_case("unionall") || n.eq_ignore_ascii_case("union_all") => {
            if args.len() == 2 {
                let a = match eval_expression_with_catalog(&args[0], bindings, storage, catalog) {
                    PropertyValue::List(l) => l, _ => return PropertyValue::Null,
                };
        ...
        n if n.eq_ignore_ascii_case("apoc_coll_insertall") || n.eq_ignore_ascii_case("insert_all") => {
            if args.len() == 3 {
                let mut base = match eval_expression_with_catalog(&args[0], bindings, storage, catalog) {
                    PropertyValue::List(l) => l, _ => return PropertyValue::Null,
                };
                let idx = match eval_expression_with_catalog(&args[1], bindings, storage, catalog) {
                    PropertyValue::Int(n) => n as usize, _ => return PropertyValue::Null,
                };
                let insert = match eval_expression_with_catalog(&args[2], bindings, storage, catalog) {
                    PropertyValue::List(l) => l, _ => return PropertyValue::Null,
                };
        ...
        n if n.eq_ignore_ascii_case("apoc_coll_partition") || n.eq_ignore_ascii_case("partition") => {
            if args.len() == 2 {
                let list = match eval_expression_with_catalog(&args[0], bindings, storage, catalog) {
                    PropertyValue::List(l) => l, _ => return PropertyValue::Null,
                };
                let size = match eval_expression_with_catalog(&args[1], bindings, storage, catalog) {
                    PropertyValue::Int(n) if n > 0 => n as usize, _ => return PropertyValue::Null,
                };
                let chunks: Vec<PropertyValue> = list.chunks(size)
                    .map(|chunk| PropertyValue::List(chunk.to_vec()))
                    .collect();
                PropertyValue::List(chunks)
            } else { PropertyValue::Null }
        }
        n if n.eq_ignore_ascii_case("apoc_number_format") || n.eq_ignore_ascii_case("format_number") => {
            let val = args.get(0).map(|e| eval_expression_with_catalog(e, bindings, storage, catalog));
            let pattern = args.get(1).map(|e| eval_expression_with_catalog(e, bindings, storage, catalog));
            match (val, pattern) {
        ...
        n if n.eq_ignore_ascii_case("apoc_number_parseint") || n.eq_ignore_ascii_case("parse_int") => {
            if args.len() >= 1 {
                let s = match eval_expression_with_catalog(&args[0], bindings, storage, catalog) {
                    PropertyValue::String(s) => s,
        ...
        n if n.eq_ignore_ascii_case("apoc_number_parsefloat") || n.eq_ignore_ascii_case("parse_float") => {
            if args.len() == 1 {
                let s = match eval_expression_with_catalog(&args[0], bindings, storage, catalog) {
                    PropertyValue::String(s) => s,
        ...
        n if n.eq_ignore_ascii_case("apoc_number_sign") || n.eq_ignore_ascii_case("sign_value") => {
            if args.len() == 1 {
                match eval_expression_with_catalog(&args[0], bindings, storage, catalog) {
                    PropertyValue::Int(n) => PropertyValue::Int(if n > 0 { 1 } else if n < 0 { -1 } else { 0 }),
                    PropertyValue::Double(d) => PropertyValue::Int(if d > 0.0 { 1 } else if d < 0.0 { -1 } else { 0 }),
                    _ => PropertyValue::Null,
                }
            } else { PropertyValue::Null }
        }
        n if n.eq_ignore_ascii_case("apoc_text_levenshtein") || n.eq_ignore_ascii_case("levenshtein") || n.eq_ignore_ascii_case("levenshteindistance") => {
            if args.len() == 2 {
                let a = match eval_expression_with_catalog(&args[0], bindings, storage, catalog) {
                    PropertyValue::String(s) => s, _ => return PropertyValue::Null,
                };
                ...
                let m = a.chars().count();
                let n = b.chars().count();
                if m == 0 { return PropertyValue::Int(n as i64); }
                if n == 0 { return PropertyValue::Int(m as i64); }
        ...
        n if n.eq_ignore_ascii_case("apoc_coll_randomitem") || n.eq_ignore_ascii_case("random_item") => {
            args.first().map(|a| match eval_expression_with_catalog(a, bindings, storage, catalog) {
                PropertyValue::List(l) => {
                    if l.is_empty() { PropertyValue::Null }
                    else {
        ...
        // ─── APOC map functions ────────────────────────────────────
        n if n.eq_ignore_ascii_case("apoc_map_fromlists") || n.eq_ignore_ascii_case("from_lists") => {
            if args.len() == 2 {
        ...
        n if n.eq_ignore_ascii_case("apoc_map_frompairs") || n.eq_ignore_ascii_case("from_pairs") => {
            args.first().map(|a| match eval_expression_with_catalog(a, bindings, storage, catalog) {
        ...
        n if n.eq_ignore_ascii_case("apoc_map_get") || n.eq_ignore_ascii_case("map_get") || n.eq_ignore_ascii_case("get") => {
            if args.len() >= 2 {
                ...
                let default = args.get(2).map(|a| eval_expression_with_catalog(a, bindings, storage, catalog));
                map.iter().find(|(k, _)| k == &key).map(|(_, v)| v.clone())
                    .or(default).unwrap_or(PropertyValue::Null)
            } else { PropertyValue::Null }
        }
        ...
        n if n.eq_ignore_ascii_case("apoc_map_clean") || n.eq_ignore_ascii_case("map_clean") => {
            args.first().map(|a| match eval_expression_with_catalog(a, bindings, storage, catalog) {
                PropertyValue::Map(m) => {
                    let cleaned: Vec<(String, PropertyValue)> = m.into_iter()
        ...
        n if n.eq_ignore_ascii_case("apoc_map_invert") || n.eq_ignore_ascii_case("map_invert") => {
            args.first().map(|a| match eval_expression_with_catalog(a, bindings, storage, catalog) {
                PropertyValue::Map(m) => {
                    let inverted: Vec<(String, PropertyValue)> = m.into_iter()
        ...
        n if n.eq_ignore_ascii_case("apoc_map_values") || n.eq_ignore_ascii_case("map_values") => {
            if args.len() >= 2 {
                ...
                for key in keys {
                    if let PropertyValue::String(k) = key {
                        let val = map.iter().find(|(mk, _)| mk == &k).map(|(_, v)| v.clone())
                            .unwrap_or(PropertyValue::Null);
                        result.push(val);
                    }
                }
        ...
        n if n.eq_ignore_ascii_case("apoc_map_flatten") || n.eq_ignore_ascii_case("map_flatten") => {
            if args.len() >= 1 {
                let map = match eval_expression_with_catalog(&args[0], bindings, storage, catalog) {
                    PropertyValue::Map(m) => m, _ => return PropertyValue::Null,
                };
                let delimiter = if args.len() >= 2 {
                    match eval_expression_with_catalog(&args[1], bindings, storage, catalog) {
                        PropertyValue::String(s) => s, _ => ".".to_string(),
                    }
                } else { ".".to_string() };
                let mut result = Vec::new();
                fn flatten_map(prefix: &str, map: &[(String, PropertyValue)], delimiter: &str, result: &mut Vec<(String, PropertyValue)>) {
                    for (k, v) in map {
                        let key = if prefix.is_empty() { k.clone() } else { format!("{}{}{}", prefix, delimiter, k) };
                        match v {
                            PropertyValue::Map(inner) => flatten_map(&key, inner, delimiter, result),
        ...
        n if n.eq_ignore_ascii_case("apoc_map_groupby") || n.eq_ignore_ascii_case("map_groupby") => {
            if args.len() >= 2 {
                let list = match eval_expression_with_catalog(&args[0], bindings, storage, catalog) {
                    PropertyValue::List(l) => l, _ => return PropertyValue::Null,
                };
                ...
                for item in list {
                    ...
                    let key = match eval_expression_with_catalog(key_expr, &local_bindings, storage, catalog) {
                        PropertyValue::String(s) => s,
                ...
                let result: Vec<(String, PropertyValue)> = groups.into_iter()
                    .map(|(k, v)| (k, PropertyValue::List(v)))
                    .collect();
                PropertyValue::Map(result)
            } else { PropertyValue::Null }
        }
        n if n.eq_ignore_ascii_case("apoc_convert_toboolean") || n.eq_ignore_ascii_case("to_boolean") => {
            if args.len() == 1 {
                match eval_expression_with_catalog(&args[0], bindings, storage, catalog) {
                    PropertyValue::Bool(b) => PropertyValue::Bool(b),
                    PropertyValue::String(s) => PropertyValue::Bool(s.eq_ignore_ascii_case("true") || s == "1"),
                    PropertyValue::Int(n) => PropertyValue::Bool(n != 0),
        ...
        n if n.eq_ignore_ascii_case("apoc_convert_tostring") || n.eq_ignore_ascii_case("to_string") => {
            if args.len() == 1 {
                let val = eval_expression_with_catalog(&args[0], bindings, storage, catalog);
                PropertyValue::String(format!("{}", val))
            } else { PropertyValue::Null }
        }
        n if n.eq_ignore_ascii_case("apoc_convert_tointeger") || n.eq_ignore_ascii_case("to_integer") => {
            if args.len() == 1 {
                match eval_expression_with_catalog(&args[0], bindings, storage, catalog) {
                    PropertyValue::Int(n) => PropertyValue::Int(n),
                    PropertyValue::Double(d) => PropertyValue::Int(d as i64),
                    PropertyValue::String(s) => s.parse::<i64>().map(PropertyValue::Int).unwrap_or(PropertyValue::Null),
                    _ => PropertyValue::Null,
                }
            } else { PropertyValue::Null }
        }
        n if n.eq_ignore_ascii_case("apoc_convert_tofloat") || n.eq_ignore_ascii_case("to_float") => {
            if args.len() == 1 {
                match eval_expression_with_catalog(&args[0], bindings, storage, catalog) {
                    PropertyValue::Int(n) => PropertyValue::Double(n as f64),
                    PropertyValue::Double(d) => PropertyValue::Double(d),
                    PropertyValue::String(s) => s.parse::<f64>().map(PropertyValue::Double).unwrap_or(PropertyValue::Null),
                    _ => PropertyValue::Null,
                }
            } else { PropertyValue::Null }
        }
        n if n.eq_ignore_ascii_case("apoc_convert_tolist") || n.eq_ignore_ascii_case("to_list") => {
            if args.len() == 1 {
                match eval_expression_with_catalog(&args[0], bindings, storage, catalog) {
                    PropertyValue::List(l) => PropertyValue::List(l),
                    PropertyValue::Map(m) => PropertyValue::List(m.into_iter().map(|(k, v)| PropertyValue::List(vec![PropertyValue::String(k), v])).collect()),
                    PropertyValue::String(s) => PropertyValue::List(s.chars().map(|c| PropertyValue::String(c.to_string())).collect()),
        ...
        n if n.eq_ignore_ascii_case("apoc_meta_types") || n.eq_ignore_ascii_case("meta_types") => {
            if args.len() >= 1 {
                ...
                if args.len() >= 2 {
                    let expected = match eval_expression_with_catalog(&args[1], bindings, storage, catalog) {
                        PropertyValue::String(s) => s, _ => return PropertyValue::Null,
                    };
        ...
        n if n.eq_ignore_ascii_case("apoc_meta_istype") || n.eq_ignore_ascii_case("meta_istype") => {
            if args.len() == 2 {
                let val = eval_expression_with_catalog(&args[0], bindings, storage, catalog);
                let expected = match eval_expression_with_catalog(&args[1], bindings, storage, catalog) {
                    PropertyValue::String(s) => s.to_uppercase(), _ => return PropertyValue::Null,
                };
                let type_name = match val {
                    PropertyValue::Null => "NULL",
                    PropertyValue::Bool(_) => "BOOLEAN",
        ...
        n if n.eq_ignore_ascii_case("year") => {
            args.first().map(|a| match eval_expression_with_catalog(a, bindings, storage, catalog) {
                PropertyValue::Date(d) => PropertyValue::Int(d.days() / 365 + 1970),
                PropertyValue::LocalDateTime(dt) => PropertyValue::Int(us_to_naive_datetime(dt.microseconds()).year() as i64),
                PropertyValue::ZonedDateTime(zdt) => {
                    let dt = chrono::DateTime::from_timestamp_micros(zdt.microseconds()).unwrap_or_default();
                    PropertyValue::Int(dt.year() as i64)
                }
                _ => PropertyValue::Null,
            }).unwrap_or(PropertyValue::Null)
        }
        n if n.eq_ignore_ascii_case("month") => {
            args.first().map(|a| match eval_expression_with_catalog(a, bindings, storage, catalog) {
                PropertyValue::Date(d) => {
                    let naive = chrono::NaiveDate::from_num_days_from_ce_opt((d.days() + 719162) as i32)
                        .unwrap_or_default();
                    PropertyValue::Int(naive.month() as i64)
                }
                PropertyValue::LocalDateTime(dt) => PropertyValue::Int(us_to_naive_datetime(dt.microseconds()).month() as i64),
                PropertyValue::ZonedDateTime(zdt) => {
                    let dt = chrono::DateTime::from_timestamp_micros(zdt.microseconds()).unwrap_or_default();
        ...
        n if n.eq_ignore_ascii_case("day") => {
            args.first().map(|a| match eval_expression_with_catalog(a, bindings, storage, catalog) {
                PropertyValue::Date(d) => {
                    let naive = chrono::NaiveDate::from_num_days_from_ce_opt((d.days() + 719162) as i32)
                        .unwrap_or_default();
                ...
                PropertyValue::LocalDateTime(dt) => PropertyValue::Int(us_to_naive_datetime(dt.microseconds()).day() as i64),
                PropertyValue::ZonedDateTime(zdt) => {
                    let dt = chrono::DateTime::from_timestamp_micros(zdt.microseconds()).unwrap_or_default();
        ...
        n if n.eq_ignore_ascii_case("hour") => {
            args.first().map(|a| match eval_expression_with_catalog(a, bindings, storage, catalog) {
                PropertyValue::LocalTime(t) => PropertyValue::Int(t.microseconds() / 3_600_000_000),
                PropertyValue::LocalDateTime(dt) => PropertyValue::Int(us_to_naive_datetime(dt.microseconds()).hour() as i64),
                PropertyValue::ZonedDateTime(zdt) => {
                    let dt = chrono::DateTime::from_timestamp_micros(zdt.microseconds()).unwrap_or_default();
        ...
        n if n.eq_ignore_ascii_case("minute") => {
            args.first().map(|a| match eval_expression_with_catalog(a, bindings, storage, catalog) {
                PropertyValue::LocalTime(t) => PropertyValue::Int((t.microseconds() / 60_000_000) % 60),
                PropertyValue::LocalDateTime(dt) => PropertyValue::Int(us_to_naive_datetime(dt.microseconds()).minute() as i64),
                PropertyValue::ZonedDateTime(zdt) => {
                    let dt = chrono::DateTime::from_timestamp_micros(zdt.microseconds()).unwrap_or_default();
        ...
        n if n.eq_ignore_ascii_case("second") => {
            args.first().map(|a| match eval_expression_with_catalog(a, bindings, storage, catalog) {
                PropertyValue::LocalTime(t) => PropertyValue::Int((t.microseconds() / 1_000_000) % 60),
                PropertyValue::LocalDateTime(dt) => PropertyValue::Int(us_to_naive_datetime(dt.microseconds()).second() as i64),
                PropertyValue::ZonedDateTime(zdt) => {
                    let dt = chrono::DateTime::from_timestamp_micros(zdt.microseconds()).unwrap_or_default();
        ...
        n if n.eq_ignore_ascii_case("millisecond") => {
            args.first().map(|a| match eval_expression_with_catalog(a, bindings, storage, catalog) {
                PropertyValue::LocalTime(t) => PropertyValue::Int((t.microseconds() / 1_000) % 1_000),
                PropertyValue::LocalDateTime(dt) => PropertyValue::Int((us_to_naive_datetime(dt.microseconds()).nanosecond() as i64 / 1_000_000) % 1_000),
                PropertyValue::ZonedDateTime(zdt) => {
                    let dt = chrono::DateTime::from_timestamp_micros(zdt.microseconds()).unwrap_or_default();
        ...
        n if n.eq_ignore_ascii_case("microsecond") => {
            args.first().map(|a| match eval_expression_with_catalog(a, bindings, storage, catalog) {
                PropertyValue::LocalTime(t) => PropertyValue::Int(t.microseconds() % 1_000_000),
                PropertyValue::LocalDateTime(dt) => PropertyValue::Int((us_to_naive_datetime(dt.microseconds()).nanosecond() as i64 / 1_000) % 1_000_000),
                PropertyValue::ZonedDateTime(zdt) => {
                    let dt = chrono::DateTime::from_timestamp_micros(zdt.microseconds()).unwrap_or_default();
        ...
        n if n.eq_ignore_ascii_case("dayofweek") || n.eq_ignore_ascii_case("dayofweek_iso") => {
            args.first().map(|a| match eval_expression_with_catalog(a, bindings, storage, catalog) {
                PropertyValue::Date(d) => {
                    let naive = chrono::NaiveDate::from_num_days_from_ce_opt((d.days() + 719162) as i32)
                        .unwrap_or_default();
                ...
                PropertyValue::LocalDateTime(dt) => PropertyValue::Int(us_to_naive_datetime(dt.microseconds()).weekday().number_from_monday() as i64),
                _ => PropertyValue::Null,
            }).unwrap_or(PropertyValue::Null)
        }
        n if n.eq_ignore_ascii_case("dayofyear") => {
            args.first().map(|a| match eval_expression_with_catalog(a, bindings, storage, catalog) {
                PropertyValue::Date(d) => {
                    let naive = chrono::NaiveDate::from_num_days_from_ce_opt((d.days() + 719162) as i32)
                        .unwrap_or_default();
                ...
                PropertyValue::LocalDateTime(dt) => PropertyValue::Int(us_to_naive_datetime(dt.microseconds()).ordinal() as i64),
                _ => PropertyValue::Null,
            }).unwrap_or(PropertyValue::Null)
        }
        n if n.eq_ignore_ascii_case("week") => {
            args.first().map(|a| match eval_expression_with_catalog(a, bindings, storage, catalog) {
                PropertyValue::Date(d) => {
                    let naive = chrono::NaiveDate::from_num_days_from_ce_opt((d.days() + 719162) as i32)
                        .unwrap_or_default();
                ...
                PropertyValue::LocalDateTime(dt) => PropertyValue::Int(us_to_naive_datetime(dt.microseconds()).iso_week().week() as i64),
                _ => PropertyValue::Null,
            }).unwrap_or(PropertyValue::Null)
        }
        n if n.eq_ignore_ascii_case("quarter") => {
            args.first().map(|a| match eval_expression_with_catalog(a, bindings, storage, catalog) {
                PropertyValue::Date(d) => {
                    let naive = chrono::NaiveDate::from_num_days_from_ce_opt((d.days() + 719162) as i32)
                        .unwrap_or_default();
                ...
                PropertyValue::LocalDateTime(dt) => PropertyValue::Int(((us_to_naive_datetime(dt.microseconds()).month() - 1) / 3 + 1) as i64),
                _ => PropertyValue::Null,
            }).unwrap_or(PropertyValue::Null)
        }
        n if n.eq_ignore_ascii_case("epochmillis") || n.eq_ignore_ascii_case("epochmilli") => {
            args.first().map(|a| match eval_expression_with_catalog(a, bindings, storage, catalog) {
                PropertyValue::Date(d) => PropertyValue::Int(d.days() * 86_400_000),
                PropertyValue::LocalDateTime(dt) => PropertyValue::Int(us_to_naive_datetime(dt.microseconds()).and_utc().timestamp_millis()),
                PropertyValue::ZonedDateTime(zdt) => PropertyValue::Int(zdt.microseconds() / 1_000),
                _ => PropertyValue::Null,
            }).unwrap_or(PropertyValue::Null)
        }
        n if n.eq_ignore_ascii_case("epochseconds") || n.eq_ignore_ascii_case("epochsecond") => {
            args.first().map(|a| match eval_expression_with_catalog(a, bindings, storage, catalog) {
                ...
                PropertyValue::LocalDateTime(dt) => PropertyValue::Int(us_to_naive_datetime(dt.microseconds()).and_utc().timestamp()),
                PropertyValue::ZonedDateTime(zdt) => PropertyValue::Int(zdt.microseconds() / 1_000_000),
                _ => PropertyValue::Null,
            }).unwrap_or(PropertyValue::Null)
        }
        ...
        n if n.eq_ignore_ascii_case("lpad") => {
            if args.len() == 3 {
                ...
                let pad = match eval_expression_with_catalog(&args[2], bindings, storage, catalog) {
                    PropertyValue::String(p) => p, _ => return PropertyValue::Null,
                };
                if s.len() >= len { PropertyValue::String(s) }
                else {
                    let mut result = String::new();
        ...
        n if n.eq_ignore_ascii_case("md5") => {
            args.first().map(|a| match eval_expression_with_catalog(a, bindings, storage, catalog) {
                PropertyValue::String(s) => {
        ...
        n if n.eq_ignore_ascii_case("tobase64") || n.eq_ignore_ascii_case("base64encode") => {
            args.first().map(|a| match eval_expression_with_catalog(a, bindings, storage, catalog) {
                PropertyValue::String(s) => PropertyValue::String(base64_encode(&s)),
                _ => PropertyValue::Null,
            }).unwrap_or(PropertyValue::Null)
        }
        n if n.eq_ignore_ascii_case("frombase64") || n.eq_ignore_ascii_case("base64decode") => {
            args.first().map(|a| match eval_expression_with_catalog(a, bindings, storage, catalog) {
                PropertyValue::String(s) => PropertyValue::String(base64_decode(&s).unwrap_or_default()),
                _ => PropertyValue::Null,
            }).unwrap_or(PropertyValue::Null)
        }
        n if n.eq_ignore_ascii_case("urlencode") => {
            args.first().map(|a| match eval_expression_with_catalog(a, bindings, storage, catalog) {
        ...
        n if n.eq_ignore_ascii_case("cosh") => {
            args.first().map(|a| match eval_expression_with_catalog(a, bindings, storage, catalog) {
        ...
        n if n.eq_ignore_ascii_case("sinh") => {
            args.first().map(|a| match eval_expression_with_catalog(a, bindings, storage, catalog) {
        ...
        n if n.eq_ignore_ascii_case("tanh") => {
            args.first().map(|a| match eval_expression_with_catalog(a, bindings, storage, catalog) {
        ...
        n if n.eq_ignore_ascii_case("acosh") => {
            args.first().map(|a| match eval_expression_with_catalog(a, bindings, storage, catalog) {
        ...
        n if n.eq_ignore_ascii_case("asinh") => {
            args.first().map(|a| match eval_expression_with_catalog(a, bindings, storage, catalog) {
        ...
        n if n.eq_ignore_ascii_case("atanh") => {
            args.first().map(|a| match eval_expression_with_catalog(a, bindings, storage, catalog) {
                PropertyValue::Double(f) if f > -1.0 && f < 1.0 => PropertyValue::Double(f.atanh()),
                PropertyValue::Int(n) if n > -1 && n < 1 => PropertyValue::Double((n as f64).atanh()),
                _ => PropertyValue::Null,
            }).unwrap_or(PropertyValue::Null)
        }
        // ─── Graph functions ───────────────────────────────────────
        ...
        n if n.eq_ignore_ascii_case("allshortestpaths") || n.eq_ignore_ascii_case("all_shortest_paths") => {
            if args.len() == 2 {
                let start = eval_expression_with_catalog(&args[0], bindings, storage, catalog);
                let end = eval_expression_with_catalog(&args[1], bindings, storage, catalog);
                match (&start, &end) {
                    (PropertyValue::Vertex(sv), PropertyValue::Vertex(ev)) => {
                        if let Some(s) = storage {
                            ...
                            PropertyValue::List(paths.into_iter().map(PropertyValue::Path).collect())
                        } else { PropertyValue::Null }
                    }
                    _ => PropertyValue::Null,
                }
        ...
        n if n.eq_ignore_ascii_case("percentilecont") || n.eq_ignore_ascii_case("percentile_cont") => {
            if args.len() == 2 {
                let list = match eval_expression_with_catalog(&args[0], bindings, storage, catalog) {
                    PropertyValue::List(l) => l, _ => return PropertyValue::Null,
                };
                ...
                let mut values: Vec<f64> = list.iter().filter_map(|v| match v {
                    PropertyValue::Int(n) => Some(*n as f64),
                    PropertyValue::Double(f) => Some(*f),
                    _ => None,
                }).collect();
                if values.is_empty() { return PropertyValue::Null; }
                values.sort_by(|a, b| a.partial_cmp(b).unwrap_or(std::cmp::Ordering::Equal));
                let idx = p * (values.len() - 1) as f64;
                let lower = idx.floor() as usize;
                let upper = idx.ceil() as usize;
                let frac = idx - lower as f64;
                if lower >= values.len() { return PropertyValue::Double(values[values.len() - 1]); }
                if upper >= values.len() { return PropertyValue::Double(values[lower]); }
                let result = values[lower] * (1.0 - frac) + values[upper] * frac;
                PropertyValue::Double(result)
            } else { PropertyValue::Null }
        }
        n if n.eq_ignore_ascii_case("percentiledisc") || n.eq_ignore_ascii_case("percentile_disc") => {
            if args.len() == 2 {
                let list = match eval_expression_with_catalog(&args[0], bindings, storage, catalog) {
                    PropertyValue::List(l) => l, _ => return PropertyValue::Null,
                };
        ...
        n if n.eq_ignore_ascii_case("stdev") || n.eq_ignore_ascii_case("stddev") => {
            args.first().map(|a| match eval_expression_with_catalog(a, bindings, storage, catalog) {
                PropertyValue::List(l) => {
                    let values: Vec<f64> = l.iter().filter_map(|v| match v {
                        PropertyValue::Int(n) => Some(*n as f64),
                        PropertyValue::Double(f) => Some(*f),
                        _ => None,
                    }).collect();
                    if values.len() < 2 { return PropertyValue::Null; }
        ...
        n if n.eq_ignore_ascii_case("stdevp") || n.eq_ignore_ascii_case("stddevp") => {
            args.first().map(|a| match eval_expression_with_catalog(a, bindings, storage, catalog) {
                PropertyValue::List(l) => {
                    let values: Vec<f64> = l.iter().filter_map(|v| match v {
                        PropertyValue::Int(n) => Some(*n as f64),
        ...
        n if n.eq_ignore_ascii_case("tobooleanornull") => {
            args.first().map(|a| match eval_expression_with_catalog(a, bindings, storage, catalog) {
                PropertyValue::Bool(b) => PropertyValue::Bool(b),
        ...
        n if n.eq_ignore_ascii_case("toboolean") => {
            args.first().map(|a| match eval_expression_with_catalog(a, bindings, storage, catalog) {
                PropertyValue::Bool(b) => PropertyValue::Bool(b),
                PropertyValue::String(s) => match s.to_lowercase().as_str() {
                    "true" => PropertyValue::Bool(true),
        ...
        n if n.eq_ignore_ascii_case("isEmpty") || n.eq_ignore_ascii_case("isempty") => {
            args.first().map(|a| match eval_expression_with_catalog(a, bindings, storage, catalog) {
                PropertyValue::List(l) => PropertyValue::Bool(l.is_empty()),
                PropertyValue::Map(m) => PropertyValue::Bool(m.is_empty()),
                PropertyValue::String(s) => PropertyValue::Bool(s.is_empty()),
        ...
        n if n.eq_ignore_ascii_case("randomstring") || n.eq_ignore_ascii_case("random_string") => {
            let len = args.first().map(|a| match eval_expression_with_catalog(a, bindings, storage, catalog) {
                PropertyValue::Int(n) => n.max(1).min(256) as usize,
                _ => 10usize,
            }).unwrap_or(10);
        ...
        n if n.eq_ignore_ascii_case("encodeJson") || n.eq_ignore_ascii_case("to_json") => {
            args.first().map(|a| {
                let val = eval_expression_with_catalog(a, bindings, storage, catalog);
        ...
        n if n.eq_ignore_ascii_case("decodeJson") || n.eq_ignore_ascii_case("from_json") => {
            args.first().map(|a| match eval_expression_with_catalog(a, bindings, storage, catalog) {
                PropertyValue::String(s) => {
        ...
        n if n.eq_ignore_ascii_case("sleep") || n.eq_ignore_ascii_case("apoc_util_sleep") => {
            let ms = args.first().map(|a| match eval_expression_with_catalog(a, bindings, storage, catalog) {
                PropertyValue::Int(n) => n.max(0).min(60000) as u64,
                _ => 0u64,
            }).unwrap_or(0);
            std::thread::sleep(std::time::Duration::from_millis(ms));
            PropertyValue::Bool(true)
        }
        n if n.eq_ignore_ascii_case("apoc_util_md5") || n.eq_ignore_ascii_case("md5_hash") => {
            args.first().map(|a| match eval_expression_with_catalog(a, bindings, storage, catalog) {
                PropertyValue::String(s) => {
                    PropertyValue::String(md5_hash(s.as_bytes()))
                }
                _ => PropertyValue::Null,
            }).unwrap_or(PropertyValue::Null)
        }
        n if n.eq_ignore_ascii_case("apoc_util_sha1") || n.eq_ignore_ascii_case("sha1_hash") => {
            args.first().map(|a| match eval_expression_with_catalog(a, bindings, storage, catalog) {
                PropertyValue::String(s) => {
                    PropertyValue::String(sha1_hash(s.as_bytes()))
                }
                _ => PropertyValue::Null,
            }).unwrap_or(PropertyValue::Null)
        }
        n if n.eq_ignore_ascii_case("apoc_util_sha256") || n.eq_ignore_ascii_case("sha256_hash") => {
            args.first().map(|a| match eval_expression_with_catalog(a, bindings, storage, catalog) {
                PropertyValue::String(s) => {
        ...
        n if n.eq_ignore_ascii_case("apoc_util_fromjson") || n.eq_ignore_ascii_case("from_json") => {
            if args.len() == 1 {
                match eval_expression_with_catalog(&args[0], bindings, storage, catalog) {
                    PropertyValue::String(s) => {
                        match serde_json::from_str::<serde_json::Value>(&s) {
        ...
        n if n.eq_ignore_ascii_case("elementId") => {
            args.first().map(|a| match eval_expression_with_catalog(a, bindings, storage, catalog) {
                PropertyValue::Vertex(vr) => PropertyValue::String(format!("v:{}", vr.gid.as_uint())),
                PropertyValue::Edge(er) => PropertyValue::String(format!("e:{}", er.gid.as_uint())),
                _ => PropertyValue::Null,
        ...
        n if n.eq_ignore_ascii_case("apoc_coll_slice") || n.eq_ignore_ascii_case("slice") => {
            if args.len() >= 2 {
                ...
                let end = args.get(2).map(|a| match eval_expression_with_catalog(a, bindings, storage, catalog) {
                    PropertyValue::Int(n) => n as usize,
                    _ => list.len(),
                }).unwrap_or(list.len());
        ...
        n if n.eq_ignore_ascii_case("count") => {
            args.first().map(|a| match eval_expression_with_catalog(a, bindings, storage, catalog) {
                PropertyValue::List(l) => PropertyValue::Int(l.len() as i64),
                PropertyValue::String(s) => PropertyValue::Int(s.len() as i64),
                PropertyValue::Map(m) => PropertyValue::Int(m.len() as i64),
        ...
        n if n.eq_ignore_ascii_case("uniformsample") || n.eq_ignore_ascii_case("uniform_sample") => {
            if args.len() == 2 {
                let list = match eval_expression_with_catalog(&args[0], bindings, storage, catalog) {
                    PropertyValue::List(l) => l, _ => return PropertyValue::Null,
                };
                let sample_size = match eval_expression_with_catalog(&args[1], bindings, storage, catalog) {
                    PropertyValue::Int(n) => n.max(0) as usize, _ => return PropertyValue::Null,
                };
                ...
                let mut rng = rand::thread_rng();
                use rand::seq::SliceRandom;
                let sample: Vec<PropertyValue> = list.choose_multiple(&mut rng, sample_size.min(list.len())).cloned().collect();
        ...
        n if n.eq_ignore_ascii_case("tobytestring") || n.eq_ignore_ascii_case("to_byte_string") => {
            if args.len() == 1 {
                ...
                match val {
                    PropertyValue::String(s) => {
                        let bytes: Vec<PropertyValue> = s.bytes().map(|b| PropertyValue::Int(b as i64)).collect();
                        PropertyValue::List(bytes)
                    }
                    PropertyValue::List(items) => {
                        let bytes: Result<Vec<u8>, _> = items.iter().map(|v| match v {
                            PropertyValue::Int(n) if *n >= 0 && *n <= 255 => Ok(*n as u8),
                            _ => Err(()),
                        }).collect();
                        match bytes {
                            Ok(b) => PropertyValue::String(String::from_utf8_lossy(&b).into_owned()),
                            Err(_) => PropertyValue::Null,
                        }
                    }
                    _ => PropertyValue::Null,
        ...
        n if n.eq_ignore_ascii_case("frombytestring") || n.eq_ignore_ascii_case("from_byte_string") => {
            if args.len() == 1 {
                match eval_expression_with_catalog(&args[0], bindings, storage, catalog) {
                    PropertyValue::String(s) => {
                        let bytes: Vec<PropertyValue> = s.bytes().map(|b| PropertyValue::Int(b as i64)).collect();
        ...
        n if n.eq_ignore_ascii_case("propertysize") || n.eq_ignore_ascii_case("property_size") => {
            if args.len() == 2 {
                ...
                let prop_name = match eval_expression_with_catalog(&args[1], bindings, storage, catalog) {
                    PropertyValue::String(s) => s, _ => return PropertyValue::Null,
                };
                ...
                match store {
                    Some(ps) => {
                        ...
                        let size = match val {
                            ...
                            PropertyValue::Map(m) => m.iter().map(|(k, v)| k.len() + 8).sum::<usize>() as i64,
                            _ => 8,
                        };
        ...
        n if n.eq_ignore_ascii_case("assert") => {
            if args.len() >= 1 {
                let cond = match eval_expression_with_catalog(&args[0], bindings, storage, catalog) {
                    PropertyValue::Bool(b) => b, _ => return PropertyValue::Null,
                };
                if !cond {
        ...
        n if n.eq_ignore_ascii_case("counter") => {
            if args.len() >= 2 {
                ...
                let initial = match eval_expression_with_catalog(&args[1], bindings, storage, catalog) {
                    PropertyValue::Int(n) => n, _ => return PropertyValue::Null,
                };
        ...
        n if n.eq_ignore_ascii_case("gethopscounter") || n.eq_ignore_ascii_case("get_hops_counter") => {
        ...
        n if n.eq_ignore_ascii_case("username") => {
            crate::active_auth_store()
        ...
        n if n.eq_ignore_ascii_case("withinbbox") || n.eq_ignore_ascii_case("within_bbox") => {
            if args.len() == 3 {
                ...
                let upper_right = eval_expression_with_catalog(&args[2], bindings, storage, catalog);
                match (&point, &lower_left, &upper_right) {
                    (PropertyValue::Point2D(p), PropertyValue::Point2D(ll), PropertyValue::Point2D(ur)) => {
                        if p.crs != ll.crs || p.crs != ur.crs {
                            return PropertyValue::Null;
                        }
                        PropertyValue::Bool(p.x >= ll.x && p.x <= ur.x && p.y >= ll.y && p.y <= ur.y)
                    }
                    (PropertyValue::Point3D(p), PropertyValue::Point3D(ll), PropertyValue::Point3D(ur)) => {
                        if p.crs != ll.crs || p.crs != ur.crs {
                            return PropertyValue::Null;
                        }
                        PropertyValue::Bool(p.x >= ll.x && p.x <= ur.x && p.y >= ll.y && p.y <= ur.y && p.z >= ll.z && p.z <= ur.z)
                    }
                    _ => PropertyValue::Null,
                }
            } else { PropertyValue::Null }
...
fn property_value_to_json(val: &PropertyValue) -> serde_json::Value {
    match val {
        ...
        PropertyValue::Double(f) => serde_json::Value::Number(serde_json::Number::from_f64(*f).unwrap_or(0.into())),
        PropertyValue::String(s) => serde_json::Value::String(s.clone()),
        PropertyValue::List(l) => serde_json::Value::Array(l.iter().map(property_value_to_json).collect()),
        PropertyValue::Map(m) => {
...
fn json_to_property_value(val: serde_json::Value) -> PropertyValue {
    match val {
        ...
        serde_json::Value::Array(a) => PropertyValue::List(a.into_iter().map(json_to_property_value).collect()),
...
fn md5_hash(data: &[u8]) -> String {
    ...
    for chunk in msg.chunks(64) {
        ...
        for i in 0..64 {
            ...
            b = b.wrapping_add(a.wrapping_add(f).wrapping_add(k).wrapping_add(w[g]).rotate_left(
                match i {
                    0..=15 => 7,
                    16..=31 => 12,
                    32..=47 => 17,
    ...
    h.iter().map(|x| format!("{:08x}", x.swap_bytes())).collect()
}
...
fn sha256_hash(data: &[u8]) -> String {
    ...
    for chunk in msg.chunks(64) {
        ...
        for i in 0..64 {
            ...
            let ch = (e & f) ^ ((!e) & g);
            let temp1 = h0.wrapping_add(s1).wrapping_add(ch).wrapping_add(k[i]).wrapping_add(w[i]);
    ...
    while i < chars.len() {
        ...
        match c {
            ...
            'B' => {
                primary.push('P');
                secondary.push('P');
                if i + 1 < chars.len() && chars[i + 1] == 'B' { i += 2; } else { i += 1; }
            }
            'C' => {
            ...
            'S' => {
                if i + 2 < chars.len() && chars[i + 1] == 'I' && (chars[i + 2] == 'O' || chars[i + 2] == 'A') {
                    primary.push('X');
                    secondary.push('X');
                    i += 3;
...
fn bfs_shortest_path(storage: &Storage, start: Gid, end: Gid) -> Option<mgcore::property_value::PathValue> {
    if start == end {
        return Some(mgcore::property_value::PathValue::new(mgcore::property_value::VertexRef::new(start, vec![], mgcore::property_store::PropertyStore::default())));
    }
    let mut visited = std::collections::HashSet::new();
    let mut queue = std::collections::VecDeque::new();
    let mut parent: std::collections::HashMap<Gid, (Gid, Gid, EdgeTypeId)> = std::collections::HashMap::new();
    ...
    while let Some(current) = queue.pop_front() {
        for (edge_gid, other, etype) in storage.vertex_out_edges(current, None) {
            if !visited.contains(&other) {
                ...
                if other == end {
                    // Reconstruct path
                    let mut path = mgcore::property_value::PathValue::new(mgcore::property_value::VertexRef::new(start, vec![], mgcore::property_store::PropertyStore::default()));
                    let mut node = end;
                    ...
                    for (_, eid, et, to_gid) in segments {
                        path.add_edge(mgcore::property_value::EdgeRefValue::new(eid, *et, path.end().gid, to_gid, mgcore::property_store::PropertyStore::default()));
                        path.add_vertex(mgcore::property_value::VertexRef::new(to_gid, vec![], mgcore::property_store::PropertyStore::default()));
                    }
                    return Some(path);
                }
...
fn all_shortest_paths(storage: &Storage, start: Gid, end: Gid) -> Vec<mgcore::property_value::PathValue> {
    if start == end {
        return vec![mgcore::property_value::PathValue::new(mgcore::property_value::VertexRef::new(start, vec![], mgcore::property_store::PropertyStore::default()))];
    }
    let mut visited = std::collections::HashSet::new();
    let mut frontier = vec![start];
    let mut parents: std::collections::HashMap<Gid, Vec<(Gid, Gid, EdgeTypeId)>> = std::collections::HashMap::new();
    visited.insert(start);
    let mut found = false;
        ...
        for &current in &frontier {
            for (edge_gid, other, etype) in storage.vertex_out_edges(current, None) {
                ...
                if frontier.contains(&other) || next_frontier.contains(&other) {
                    parents.entry(other).or_default().push((current, edge_gid, etype));
                }
                if other == end {
                    found = true;
                }
    ...
    fn backtrack(
        ...
        if current == start {
            ...
            for &(from, eid, et, to) in path_edges.iter().rev() {
                path.add_edge(mgcore::property_value::EdgeRefValue::new(eid, et, from, to, mgcore::property_store::PropertyStore::default()));
                path.add_vertex(mgcore::property_value::VertexRef::new(to, vec![], mgcore::property_store::PropertyStore::default()));
            }
            results.push(path);
            return;
        }
...
mod tests {
    ...
    fn test_eval_literals() {
        assert_eq!(eval_expression(&Expression::Int(42), &HashMap::new()), PropertyValue::Int(42));
        assert_eq!(eval_expression(&Expression::Bool(true), &HashMap::new()), PropertyValue::Bool(true));
        assert_eq!(eval_expression(&Expression::Null, &HashMap::new()), PropertyValue::Null);
    }
    ...
    fn test_eval_arithmetic() {
        ...
        assert_eq!(eval_expression(&expr, &HashMap::new()), PropertyValue::Int(3));
    }
    ...
    fn test_eval_comparison() {
        ...
        assert_eq!(eval_expression(&expr, &HashMap::new()), PropertyValue::Bool(true));
    }
    ...
    fn test_eval_logical() {
        ...
        assert_eq!(eval_expression(&expr, &HashMap::new()), PropertyValue::Bool(false));
    }
    ...
    fn test_eval_range() {
        ...
        assert_eq!(result, PropertyValue::List(vec![
            PropertyValue::Int(1), PropertyValue::Int(2), PropertyValue::Int(3), PropertyValue::Int(4),
        ]));
    }
    ...
    fn test_eval_coalesce() {
        ...
        assert_eq!(eval_expression(&expr, &HashMap::new()), PropertyValue::Int(42));
    }
    ...
    fn test_eval_id() {
        ...
        let mut bindings = HashMap::new();
        bindings.insert("v".into(), PropertyValue::Vertex(mgcore::property_value::VertexRef::new(
            mgcore::types::Gid::from(5u64), Vec::new(), mgcore::property_store::PropertyStore::default(),
        )));
    ...
    fn test_eval_startswith() {
        let expr = Expression::Function {
            ...
            arguments: vec![Expression::String("hello world".into()), Expression::String("hello".into())],
        };
        assert_eq!(eval_expression(&expr, &HashMap::new()), PropertyValue::Bool(true));
        ...
        let expr2 = Expression::Function {
            distinct: false,
            name: "startswith".into(),
            arguments: vec![Expression::String("hello world".into()), Expression::String("world".into())],
        };
        assert_eq!(eval_expression(&expr2, &HashMap::new()), PropertyValue::Bool(false));
    }
    ...
    fn test_eval_contains_string() {
        let expr = Expression::Function {
            distinct: false,
            name: "contains".into(),
            arguments: vec![Expression::String("hello world".into()), Expression::String("lo wo".into())],
        };
        assert_eq!(eval_expression(&expr, &HashMap::new()), PropertyValue::Bool(true));
    }
    ...
    fn test_eval_min_max() {
        let expr = Expression::Function {
            distinct: false,
            name: "min".into(),
            arguments: vec![Expression::List(vec![Expression::Int(5), Expression::Int(2), Expression::Int(8)])],
        };
        assert_eq!(eval_expression(&expr, &HashMap::new()), PropertyValue::Int(2));
        ...
        assert_eq!(eval_expression(&expr2, &HashMap::new()), PropertyValue::Int(8));
    }
    ...
    fn test_eval_sum_avg() {
        let expr = Expression::Function {
            distinct: false,
            name: "sum".into(),
            arguments: vec![Expression::List(vec![Expression::Int(1), Expression::Int(2), Expression::Int(3)])],
        };
        assert_eq!(eval_expression(&expr, &HashMap::new()), PropertyValue::Int(6));

        let expr2 = Expression::Function {
            distinct: false,
            name: "avg".into(),
            arguments: vec![Expression::List(vec![Expression::Int(2), Expression::Int(4), Expression::Int(6)])],
        };
        assert_eq!(eval_expression(&expr2, &HashMap::new()), PropertyValue::Double(4.0));
    }
    ...
    fn test_eval_reduce() {
        let expr = Expression::Function {
            ...
            arguments: vec![
                ...
                Expression::List(vec![Expression::Int(1), Expression::Int(2), Expression::Int(3)]),
                Expression::Add(Box::new(Expression::Identifier("_acc".into())), Box::new(Expression::Identifier("x".into()))),
            ],
        };
        assert_eq!(eval_expression(&expr, &HashMap::new()), PropertyValue::Int(6));
    }
    ...
    fn test_eval_extract() {
        let expr = Expression::Function {
            ...
            arguments: vec![
                Expression::Identifier("x".into()),
                Expression::List(vec![Expression::Int(1), Expression::Int(2), Expression::Int(3)]),
                Expression::Mul(Box::new(Expression::Identifier("x".into())), Box::new(Expression::Int(2))),
            ],
        };
        let result = eval_expression(&expr, &HashMap::new());
        assert_eq!(result, PropertyValue::List(vec![
    ...
    fn test_eval_duration_from_map() {
        ...
        let result = eval_expression(&expr, &HashMap::new());
        assert_eq!(result, PropertyValue::Duration(Duration::new(0, 5, 3 * 3_600_000_000)));
    }
    ...
    fn test_eval_degree_with_storage() {
        ...
        storage.create_edge(&tx, e0, v0, v1, EdgeTypeId::from(0u32)).unwrap();
        storage.create_edge(&tx, e1, v0, v2, EdgeTypeId::from(0u32)).unwrap();
        storage.create_edge(&tx, e2, v1, v0, EdgeTypeId::from(0u32)).unwrap();
        storage.commit_transaction(&tx);

        let mut bindings = HashMap::new();
        bindings.insert("v".into(), PropertyValue::Vertex(VertexRef::new(v0, vec![], mgcore::property_store::PropertyStore::default())));
    ...
    fn test_eval_percentile_cont() {
        let expr = Expression::Function {
            ...
            arguments: vec![
                Expression::List(vec![Expression::Int(1), Expression::Int(2), Expression::Int(3), Expression::Int(4)]),
    ...
    fn test_eval_stdev() {
        let expr = Expression::Function {
            distinct: false,
            name: "stDev".into(),
            arguments: vec![Expression::List(vec![Expression::Int(2), Expression::Int(4), Expression::Int(4), Expression::Int(4), Expression::Int(5), Expression::Int(5), Expression::Int(7), Expression::Int(9)])],
        };
        let result = eval_expression(&expr, &HashMap::new());
        if let PropertyValue::Double(v) = result {
    ...
    fn test_eval_to_float_or_null() {
        ...
        assert_eq!(eval_expression(&expr, &HashMap::new()), PropertyValue::Double(3.14));
        ...
        assert_eq!(eval_expression(&expr_bad, &HashMap::new()), PropertyValue::Null);
    }
