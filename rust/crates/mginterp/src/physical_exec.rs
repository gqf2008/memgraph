...
use mgparser::ast::{Expression, OrderByItem, ReturnItem};
use mgplanner::{PhysicalOp, PhysicalPlan};
use mgstorage::storage::Storage;
use mgstorage::transaction::Transaction;

use crate::{eval_expression_with_storage, set_active_transaction, ExecError, QueryResult, ResultRow};
...
fn execute_plan_rows(
    ...
    match op {
        ...
        PhysicalOp::IndexSeek { alias, label, property, value } => {
        ...
        | PhysicalOp::Aggregate {
            ...
            for row in child_rows {
                ...
                let key_vals: Vec<PropertyValue> = group_by
                    .iter()
                    .map(|expr| eval_expression_with_storage(expr, &row, Some(storage)))
                    .collect();
                let key_str = format!("{:?}", key_vals);
                groups.entry(key_str).or_insert_with(|| (key_vals, Vec::new())).1.push(row);
            }

            let mut rows = Vec::new();
            for (_key, (_key_vals, group_rows)) in groups {
                let mut result_row = ResultRow::new();
                for (expr, alias) in aggregates {
                    let val = crate::eval_aggregate(storage, expr, &group_rows);
                    let col = alias.clone().unwrap_or_else(|| format!("agg_{:?}", expr));
...
mod tests {
        ...
        let rows = execute_plan_rows(
            ...
            &PhysicalOp::TopN {
                ...
                child: Box::new(PhysicalPlan {
                    op: PhysicalOp::SeqScan { alias: Some("n".to_string()), label: None },
    ...
    fn test_topn_ascending() {
        ...
        for i in 0..5 {
            let gid = mgcore::types::Gid::from(i as u64);
            let _ = storage.create_vertex(&tx, gid);
            let _ = storage.vertex_set_property(&tx, gid, PropertyId::from(0), PropertyValue::Int(10 - i));
        }
