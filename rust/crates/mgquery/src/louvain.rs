...
pub fn louvain(storage: &Storage, resolution: f64, max_iterations: usize) -> HashMap<Gid, usize> {
    ...
    let mut unique: Vec<usize> = community.values().copied().collect::<HashSet<_>>().into_iter().collect();
    unique.sort();
    let remap: HashMap<usize, usize> = unique.into_iter().enumerate().map(|(i, c)| (c, i)).collect();
...
pub fn modularity(storage: &Storage, community: &HashMap<Gid, usize>, resolution: f64) -> f64 {
    ...
    for &i in &all {
        for &j in &all {
            ...
            let delta = if community[&i] == community[&j] { 1.0 } else { 0.0 };
...
fn edge_exists(storage: &Storage, from: Gid, to: Gid) -> bool {
    storage.vertex_out_edges(from, None).into_iter().any(|(_, t, _)| t == to)
}
...
mod tests {
    ...
    fn build_graph(storage: &Storage, edge_pairs: &[(u64, u64)], start_edge: u64) -> u64 {
        ...
        for &(from_u, to_u) in edge_pairs {
            ...
            if created.insert(from) { let _ = storage.create_vertex(&tx, from); }
            if created.insert(to) { let _ = storage.create_vertex(&tx, to); }
            let _ = storage.create_edge(&tx, Gid::from(next_edge), from, to, EdgeTypeId::from(0u32));
            next_edge += 1;
        }
