    ...
    while let Some(State { cost, node }) = heap.pop() {
        ...
        for (_, other, _) in storage.vertex_out_edges(node, None) {
            ...
            if next_cost < *dist.get(&other).unwrap_or(&f64::INFINITY) {
                ...
                heap.push(State { cost: next_cost, node: other });
            }
        }
...
fn get_edge_weight(
    ...
    for (edge_gid, e_from, e_to, _etype, props) in storage.all_edges() {
...
mod tests {
    ...
    fn test_shortest_path_weighted_basic() {
        ...
        storage
            .create_edge(&tx, Gid::from(100u64), Gid::from(1u64), Gid::from(2u64), EdgeTypeId::from(0u32))
            .unwrap();
        storage
            .create_edge(&tx, Gid::from(101u64), Gid::from(2u64), Gid::from(3u64), EdgeTypeId::from(0u32))
            .unwrap();
        storage
            .create_edge(&tx, Gid::from(102u64), Gid::from(3u64), Gid::from(4u64), EdgeTypeId::from(0u32))
            .unwrap();
        storage.commit_transaction(&tx);
