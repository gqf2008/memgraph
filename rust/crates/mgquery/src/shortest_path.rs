...
pub fn floyd_warshall(storage: &Storage) -> HashMap<(Gid, Gid), f64> {
    let vertices: Vec<Gid> = storage.all_vertices().into_iter().map(|(g, _, _)| g).collect();
...
mod tests {
    ...
    fn test_dijkstra_simple() {
        ...
        assert_eq!(path, vec![Gid::from(1u64), Gid::from(2u64), Gid::from(3u64), Gid::from(4u64)]);
        assert_eq!(dist, 3.0);
    }
