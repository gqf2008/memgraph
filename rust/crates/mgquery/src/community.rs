...
pub fn label_propagation(storage: &Storage, max_iter: usize) -> HashMap<Gid, u64> {
    ...
    let mut labels: HashMap<Gid, u64> = gids
        .iter()
        .enumerate()
        .map(|(i, g)| (*g, i as u64))
        .collect();
...
pub fn louvain(storage: &Storage) -> HashMap<Gid, u64> {
    ...
    let mut communities: HashMap<Gid, u64> = gids
        .iter()
        .enumerate()
        .map(|(i, g)| (*g, i as u64))
        .collect();
        ...
        for gid in &gids {
            ...
            for (comm, k_i_in) in neighbor_comms {
                ...
                let gain = if comm == current_comm {
                    ...
                    (k_i_in - sum_in) / m - (k_i * (sum_tot + k_i)) / (m * m)
                        + (k_i * k_i) / (m * m)
                };
                if gain > best_gain {
                    best_gain = gain;
...
pub fn eigenvector_centrality(
    ...
    for _ in 0..max_iter {
        ...
        for gid in &all {
            let mut sum = 0.0;
            for (_, other, _) in storage.vertex_out_edges(*gid, None) {
                let deg = storage.vertex_out_degree(other).max(1)
                    + storage.vertex_in_degree(other).max(1);
...
mod tests {
    ...
    fn test_label_propagation_basic() {
        ...
        let a = storage
            ...
            .unwrap();
        let b = storage
            .create_vertex(&tx, mgcore::types::Gid::from(2u64))
            .unwrap();
        let c = storage
            .create_vertex(&tx, mgcore::types::Gid::from(3u64))
            .unwrap();
        storage
            .create_edge(
                &tx,
                mgcore::types::Gid::from(10u64),
                a,
                b,
    ...
    fn test_louvain_basic() {
        ...
        let c = storage
            ...
            .unwrap();
        let d = storage
            .create_vertex(&tx, mgcore::types::Gid::from(4u64))
            .unwrap();
        storage
            .create_edge(
                &tx,
    ...
    for s in &all {
        ...
        while let Some(v) = queue.pop_front() {
            ...
            for (_, other, _) in storage.vertex_out_edges(v, None) {
                if let std::collections::hash_map::Entry::Vacant(e) = visited.entry(other) {
                    e.insert(d + 1);
