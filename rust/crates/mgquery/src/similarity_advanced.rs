...
pub fn jaccard_similarity(storage: &Storage, a: Gid, b: Gid) -> f64 {
    let neighbors_a: HashSet<Gid> = storage.vertex_out_edges(a, None)
        .into_iter().map(|(_, other, _)| other)
        .chain(storage.vertex_in_edges(a, None).into_iter().map(|(_, other, _)| other))
        .collect();
    let neighbors_b: HashSet<Gid> = storage.vertex_out_edges(b, None)
        .into_iter().map(|(_, other, _)| other)
        .chain(storage.vertex_in_edges(b, None).into_iter().map(|(_, other, _)| other))
        .collect();
...
pub fn adamic_adar(storage: &Storage, a: Gid, b: Gid) -> f64 {
    ...
    for &common in neighbors_a.intersection(&neighbors_b) {
        let deg = storage.vertex_out_edges(common, None).len() + storage.vertex_in_edges(common, None).len();
        if deg > 1 {
...
pub fn cosine_similarity(storage: &Storage, a: Gid, b: Gid) -> f64 {
    ...
    if len_a == 0.0 || len_b == 0.0 { 0.0 } else { intersection / (len_a.sqrt() * len_b.sqrt()) }
}
...
pub fn resource_allocation(storage: &Storage, a: Gid, b: Gid) -> f64 {
    ...
    neighbors_a.intersection(&neighbors_b).map(|&common| {
        let deg = storage.vertex_out_edges(common, None).len() + storage.vertex_in_edges(common, None).len();
