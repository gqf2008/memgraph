...
pub fn bfs(storage: &Storage, start: Gid) -> Vec<Gid> {
    ...
    while let Some(current) = queue.pop_front() {
        ...
        let degree_in = storage.vertex_in_degree(current);
        let degree_out = storage.vertex_out_degree(current);
        for (edge_gid, other, _etype) in storage.vertex_out_edges(current, None) {
            if !visited.contains(&other) { queue.push_back(other); }
        }
        for (edge_gid, other, _etype) in storage.vertex_in_edges(current, None) {
            if !visited.contains(&other) { queue.push_back(other); }
        }
    }
    visited
...
pub fn pagerank(storage: &Storage, damping: f64, max_iter: usize, epsilon: f64) -> HashMap<Gid, f64> {
    ...
    let out_deg: HashMap<Gid, usize> = gids.iter()
        .map(|g| (*g, storage.vertex_out_degree(*g).max(1)))
        .collect();

    let in_neighbors: HashMap<Gid, Vec<Gid>> = gids.iter()
        .map(|g| (*g, storage.vertex_in_edges(*g, None).into_iter().map(|(_, other, _)| other).collect()))
        .collect();
...
pub fn triangle_count(storage: &Storage) -> usize {
    ...
    for (gid, _labels, _props) in &all_v {
        let neighbors: HashSet<Gid> = storage.vertex_out_edges(*gid, None)
            .into_iter().map(|(_, other, _)| other)
            .chain(storage.vertex_in_edges(*gid, None).into_iter().map(|(_, other, _)| other))
            .collect();
        for n in &neighbors {
            if *n <= *gid { continue; }
            let n2: HashSet<Gid> = storage.vertex_out_edges(*n, None)
                .into_iter().map(|(_, other, _)| other)
                .chain(storage.vertex_in_edges(*n, None).into_iter().map(|(_, other, _)| other))
                .collect();
...
pub fn shortest_path(storage: &Storage, start: Gid, target: Gid) -> Option<Vec<Gid>> {
    if start == target { return Some(vec![start]); }
    ...
    while let Some(current) = queue.pop_front() {
        let neighbors: Vec<Gid> = storage.vertex_out_edges(current, None)
            .into_iter().map(|(_, other, _)| other)
            .chain(storage.vertex_in_edges(current, None).into_iter().map(|(_, other, _)| other))
            .collect();
...
pub fn degree_centrality(storage: &Storage) -> Vec<(Gid, usize)> {
    let mut result: Vec<(Gid, usize)> = storage.all_vertices()
        .into_iter()
        .map(|(gid, _, _)| {
    ...
    for (start, _, _) in &all {
        ...
        while let Some(current) = stack.pop() {
            ...
            for (_, other, _) in storage.vertex_out_edges(current, None) {
                if !visited.contains(&other) { stack.push(other); }
            }
        ...
        for i in 0..neighbor_vec.len() {
            for j in (i + 1)..neighbor_vec.len() {
                ...
                for (_, target, _) in storage.vertex_out_edges(a, None) {
                    if target == b { connected = true; break; }
                }
                if !connected {
...
pub fn betweenness_centrality(storage: &Storage) -> HashMap<Gid, f64> {
    ...
    for s in &all {
        ...
        while let Some(v) = queue.pop_front() {
            ...
            for (_, other, _) in storage.vertex_out_edges(v, None) { neighbors.push(other); }
            for (_, other, _) in storage.vertex_in_edges(v, None) { neighbors.push(other); }
            for w in neighbors {
        ...
        while let Some(v) = queue.pop_front() {
            ...
            for w in neighbors {
                if !visited.contains_key(&w) {
                    visited.insert(w, d + 1);
                    queue.push_back(w);
                }
            }
        }
        let sum_dist: usize = all.iter().filter(|g| *g != s).map(|g| visited.get(g).copied().unwrap_or(0)).sum();
...
pub fn scc(storage: &Storage) -> HashMap<Gid, usize> {
    ...
    fn strongconnect(
        v: Gid, storage: &Storage, index: &mut usize, stack: &mut Vec<Gid>,
        on_stack: &mut HashSet<Gid>, indices: &mut HashMap<Gid, usize>,
        lowlinks: &mut HashMap<Gid, usize>, sccs: &mut Vec<Vec<Gid>>,
    ) {
    ...
    for v in &all {
        if !indices.contains_key(v) {
            strongconnect(*v, storage, &mut index, &mut stack, &mut on_stack, &mut indices, &mut lowlinks, &mut sccs);
        }
    }
...
pub fn has_cycle(storage: &Storage) -> bool {
    ...
    fn dfs(v: Gid, storage: &Storage, visited: &mut HashSet<Gid>, rec_stack: &mut HashSet<Gid>) -> bool {
        visited.insert(v);
        rec_stack.insert(v);
        for (_, w, _) in storage.vertex_out_edges(v, None) {
            if !visited.contains(&w) {
                if dfs(w, storage, visited, rec_stack) { return true; }
    ...
    for v in &all {
        if !visited.contains(v) {
            if dfs(*v, storage, &mut visited, &mut rec_stack) { return true; }
        }
    }
    false
}
...
pub fn topological_sort(storage: &Storage) -> Option<Vec<Gid>> {
    let all_v = storage.all_vertices();
    let gids: Vec<Gid> = all_v.iter().map(|(g, _, _)| *g).collect();
    if gids.is_empty() { return Some(Vec::new()); }
    ...
    let mut queue: VecDeque<Gid> = in_degree.iter()
        .filter(|(_, &d)| d == 0)
        .map(|(&g, _)| g)
        .collect();
    ...
    while let Some(v) = queue.pop_front() {
        ...
        for (_, other, _) in storage.vertex_out_edges(v, None) {
            ...
            *d -= 1;
            if *d == 0 { queue.push_back(other); }
        }
    }

    if sorted.len() == gids.len() { Some(sorted) } else { None }
}
...
pub fn bridges(storage: &Storage) -> Vec<(Gid, Gid)> {
    ...
    fn dfs(
        u: Gid, storage: &Storage, visited: &mut HashSet<Gid>,
        disc: &mut HashMap<Gid, usize>, low: &mut HashMap<Gid, usize>,
        parent: &mut HashMap<Gid, Option<Gid>>, result: &mut Vec<(Gid, Gid)>, time: &mut usize,
    ) {
        visited.insert(u);
        *time += 1;
        disc.insert(u, *time);
        ...
        let mut neighbors = Vec::new();
        for (_, other, _) in storage.vertex_out_edges(u, None) { neighbors.push(other); }
        for (_, other, _) in storage.vertex_in_edges(u, None) { neighbors.push(other); }
    ...
    for v in &all {
        if !visited.contains(v) {
            ...
            dfs(*v, storage, &mut visited, &mut disc, &mut low, &mut parent, &mut result, &mut time);
        }
    }
    result
}
        ...
        while let Some(u) = queue.pop_front() {
            ...
            for v in neighbors {
                if let Some(&color_v) = colors.get(&v) {
                    if color_v == color_u { return false; }
                } else {
...
pub fn average_path_length(storage: &Storage) -> f64 {
    ...
    if count == 0 { 0.0 } else { total_dist as f64 / count as f64 }
}
...
pub fn eigenvector_centrality(storage: &Storage, max_iter: usize, epsilon: f64) -> HashMap<Gid, f64> {
    ...
    let neighbors: HashMap<Gid, Vec<Gid>> = all.iter()
        .map(|g| {
            let mut nbrs = Vec::new();
            for (_, other, _) in storage.vertex_out_edges(*g, None) { nbrs.push(other); }
            for (_, other, _) in storage.vertex_in_edges(*g, None) { nbrs.push(other); }
            (*g, nbrs)
        })
        .collect();
...
pub fn radius(storage: &Storage) -> usize {
    ...
    if ecc.is_empty() { 0 } else { *ecc.values().min().unwrap_or(&0) }
}
...
pub fn graph_density(storage: &Storage) -> f64 {
    ...
    if max_edges == 0.0 { 0.0 } else { m / max_edges }
}
...
pub fn global_clustering_coefficient(storage: &Storage) -> f64 {
    ...
    for (gid, _, _) in &all_v {
        let mut neighbors = Vec::new();
        for (_, other, _) in storage.vertex_out_edges(*gid, None) { neighbors.push(other); }
        for (_, other, _) in storage.vertex_in_edges(*gid, None) { neighbors.push(other); }
        let k = neighbors.len();
        if k < 2 { continue; }
        ...
        for i in 0..neighbors.len() {
            for j in (i + 1)..neighbors.len() {
                ...
                for (_, other, _) in storage.vertex_out_edges(a, None) {
                    if other == b { connected = true; break; }
                }
                if !connected {
                ...
                if connected { closed_triplets += 1; }
            }
        }
    }
    if all_triplets == 0 { 0.0 } else { closed_triplets as f64 / all_triplets as f64 }
}
...
pub fn coreness(storage: &Storage) -> HashMap<Gid, usize> {
    let all_v: Vec<Gid> = storage.all_vertices().into_iter().map(|(g, _, _)| g).collect();
    if all_v.is_empty() { return HashMap::new(); }

    // Compute initial degrees (undirected: in + out, counting unique neighbors)
    let mut degrees: HashMap<Gid, HashSet<Gid>> = HashMap::new();
    for &gid in &all_v {
        let mut neighbors = HashSet::new();
        for (_, other, _) in storage.vertex_out_edges(gid, None) { neighbors.insert(other); }
        for (_, other, _) in storage.vertex_in_edges(gid, None) { neighbors.insert(other); }
    ...
    while !remaining.is_empty() {
        ...
        while changed {
            ...
            for gid in to_remove {
                if !remaining.contains(&gid) { continue; }
...
pub fn rich_club_coefficient(storage: &Storage, max_k: usize) -> HashMap<usize, f64> {
    ...
    let degrees: HashMap<Gid, usize> = all_v.iter().map(|&g| {
        let mut neighbors = HashSet::new();
        for (_, other, _) in storage.vertex_out_edges(g, None) { neighbors.insert(other); }
        for (_, other, _) in storage.vertex_in_edges(g, None) { neighbors.insert(other); }
        (g, neighbors.len())
    }).collect();
    ...
    for k in 0..=max_k.min(max_deg) {
        let rich_nodes: Vec<Gid> = all_v.iter()
        ...
        let phi = if max_possible == 0 { 0.0 } else { actual_edges as f64 / max_possible as f64 };
        result.insert(k, phi);
    }
...
pub fn small_world_coefficient(storage: &Storage) -> f64 {
    ...
    if apl == 0.0 { return 0.0; }

    // Random graph approximations (Erdos-Renyi with same n and m)
    let p = if n > 1 { m as f64 / (n * (n - 1)) as f64 } else { 0.0 };
    ...
    if cc_random == 0.0 || apl_random == 0.0 { return 0.0; }
...
pub fn periphery(storage: &Storage) -> Vec<Gid> {
    let ecc = eccentricity(storage);
    if ecc.is_empty() { return Vec::new(); }
    let max_ecc = *ecc.values().max().unwrap_or(&0);
    ecc.into_iter().filter(|(_, e)| *e == max_ecc).map(|(g, _)| g).collect()
}
...
pub fn center(storage: &Storage) -> Vec<Gid> {
    ...
    ecc.into_iter().filter(|(_, e)| *e == min_ecc).map(|(g, _)| g).collect()
}
...
pub fn modularity(storage: &Storage, partition: &HashMap<Gid, usize>) -> f64 {
    ...
    let degrees: HashMap<Gid, usize> = all_v.iter().map(|&g| {
        let mut deg = 0usize;
        for (_, _, _) in storage.vertex_out_edges(g, None) { deg += 1; }
        for (_, _, _) in storage.vertex_in_edges(g, None) { deg += 1; }
        (g, deg)
    }).collect();
...
pub fn conductance(storage: &Storage, partition: &HashMap<Gid, usize>) -> HashMap<usize, f64> {
    ...
    for (&cid, members) in &communities {
        ...
        let phi = if min_vol == 0 { 0.0 } else { cut_edges as f64 / min_vol as f64 };
...
mod tests {
    ...
    fn test_shortest_path() {
        ...
        assert_eq!(path, Some(vec![
            Gid::from(1u64), Gid::from(2u64), Gid::from(3u64), Gid::from(4u64)
        ]));
    }
    ...
    fn test_degree_assortativity() {
        ...
        assert!(r < 0.0, "path graph should have negative assortativity, got {}", r);
        ...
        assert!(r2.abs() < 0.5, "ring graph should have near-zero assortativity, got {}", r2);
    }
    ...
    fn test_rich_club_coefficient() {
        ...
        assert!((rc[&2] - 1.0).abs() < 1e-6, "clique should have phi=1.0, got {}", rc[&2]);
    }
    ...
    fn test_small_world_coefficient() {
        ...
        assert!(sigma > 0.0, "small-world coefficient should be positive, got {}", sigma);
    }
    ...
    fn test_modularity() {
        ...
        assert!(q > 0.3, "two disconnected triangles should have positive modularity, got {}", q);
    }
