...
impl Recovery {
    ...
    pub fn load_from_path(
        ...
        let snap = match crate::version::detect_format(&snap_data) {
            ...
            Ok(crate::version::FormatKind::Current) => SnapshotReader::read(snapshot_path.as_ref())
                .map_err(|e| format!("snapshot read error: {}", e))?,
    ...
    fn restore_snapshot(
        ...
        if let Some(cat) = catalog {
            cat.load_mappings(
                &snap.name_mapper.labels,
                &snap.name_mapper.properties,
                &snap.name_mapper.edge_types,
            );
        }
...
pub fn replay_record(storage: &Storage, record: &DeltaRecord) -> Result<(), StorageError> {
    ...
    let result = match record {
        ...
        DeltaRecord::VertexAddLabel { gid, label } => storage.vertex_add_label(&tx, *gid, *label),
...
mod tests {
    ...
    fn test_full_persistence_roundtrip() {
        ...
        let edge = recovered.get_edge(Gid::from(100u64), &verify_tx).unwrap();
    ...
    fn test_recovery_load_from_path_current_format() {
        ...
        storage
            .vertex_add_label(&tx, Gid::from(7u64), LabelId::from(3u32))
            .unwrap();
        storage.commit_transaction(&tx);
