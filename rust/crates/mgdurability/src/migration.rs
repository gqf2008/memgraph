...
//!
//! These are stub implementations that perform the version detection and
//! delegate to the appropriate legacy reader, then re-serialize with the
//! current Rust writer.
...
pub fn convert_v14_to_current(input_path: impl AsRef<Path>, output_path: impl AsRef<Path>) -> Result<(), ConversionError> {
    ...
    match format {
        ...
        crate::version::FormatKind::LegacyCpp(v) if v < 14 => {
            Err(ConversionError::Unsupported(format!("version {} is too old", v)))
        }
        crate::version::FormatKind::LegacyCpp(v) if v > 14 => {
        ...
        _ => Err(ConversionError::Unsupported(format!("cannot convert format {:?}", format))),
    }
}
...
pub fn convert_v20_to_current(input_path: impl AsRef<Path>, output_path: impl AsRef<Path>) -> Result<(), ConversionError> {
    let data = std::fs::read(input_path.as_ref())?;
    let format = detect_format(&data).map_err(|e| ConversionError::Detect(e.to_string()))?;
    ...
    match format {
        ...
        crate::version::FormatKind::LegacyCpp(v) if v < 20 => {
            Err(ConversionError::Unsupported(format!("v{} snapshot too old for v20 converter", v)))
        }
        _ => Err(ConversionError::Unsupported(format!("cannot convert format {:?}", format))),
    }
}
...
pub fn convert_legacy_wal_to_current(input_path: impl AsRef<Path>, output_path: impl AsRef<Path>) -> Result<(), ConversionError> {
    let data = std::fs::read(input_path.as_ref())?;
    let format = detect_format(&data).map_err(|e| ConversionError::Detect(e.to_string()))?;
    ...
    match format {
        ...
        _ => Err(ConversionError::Unsupported(format!("cannot convert WAL format {:?}", format))),
    }
}
...
mod tests {
    ...
    fn test_convert_current_snapshot_noop() {
        ...
        let data = crate::snapshot::SnapshotData {
            name_mapper: crate::snapshot::NameMapperSnapshot { labels: vec![], properties: vec![], edge_types: vec![] },
    ...
    fn test_convert_legacy_wal_to_current() {
        ...
        assert!(matches!(reader.records()[0], crate::delta_record::DeltaRecord::VertexCreate { .. }));
