        ...
        match section_marker {
            marker::SECTION_VERTEX => {
                ...
                for _ in 0..count {
                    ...
                    vertices.push(VertexSnapshotEntry {
                        gid,
                        labels,
                        properties,
                    });
            ...
            marker::SECTION_MAPPER => {
                ...
                for _ in 0..prop_count {
                    ...
                    name_mapper
                        .properties
                        .push((name, PropertyId::from(id as u32)));
                }
                ...
                for _ in 0..edge_type_count {
                    ...
                    name_mapper
                        .edge_types
                        .push((name, EdgeTypeId::from(id as u32)));
                }
            }
...
mod tests {
    ...
    fn test_read_cpp_snapshot_unsupported_version() {
        let data = vec![0u8; 12];
        assert!(matches!(
            read_cpp_snapshot(&data, 5),
            Err(CppFormatError::UnsupportedVersion(5))
        ));
        assert!(matches!(
            read_cpp_snapshot(&data, 99),
            Err(CppFormatError::UnsupportedVersion(99))
        ));
    }
    ...
    fn test_read_cpp_snapshot_vertex_section() {
        ...
        assert!(matches!(
            result.vertices[0].properties[0].1,
    ...
    fn test_read_cpp_wal_truncated() {
        ...
        assert!(matches!(
            read_cpp_wal(&data, 20),
            Err(CppFormatError::Corrupt(_))
        ));
    }
    ...
    fn test_read_cpp_snapshot_property_value_map() {
        ...
        assert_eq!(
            result.vertices[0].properties[0].1,
            PropertyValue::Map(vec![("key".into(), PropertyValue::String("value".into())),])
        );
    }
}
