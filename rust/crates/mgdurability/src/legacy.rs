...
impl LegacyDeltaType {
    ...
    pub fn from_tag(tag: u8, version: u64) -> Self {
        ...
        match mapped {
            ...
            legacy_delta::V20_LABEL_PROPERTIES_INDEX_CREATE => LegacyDeltaType::LabelPropertyIndexCreate,
            legacy_delta::V20_LABEL_PROPERTIES_INDEX_DROP => LegacyDeltaType::LabelPropertyIndexDrop,
            legacy_delta::V20_EXISTENCE_CONSTRAINT_CREATE => LegacyDeltaType::ExistenceConstraintCreate,
...
impl<'a> CppReader<'a> {
    fn new(data: &'a [u8]) -> Self {
        Self { data, pos: 0, enum_mappings: HashMap::new() }
    }
        ...
        match marker {
            ...
            marker::TYPE_TEMPORAL_DATA => {
                ...
                match type_tag {
                    ...
                    1 => Ok(PropertyValue::LocalTime(LocalTime::from_microseconds(microseconds))),
                    2 => Ok(PropertyValue::LocalDateTime(LocalDateTime::from_microseconds(microseconds))),
                    3 => Ok(PropertyValue::Duration(Duration::new(0, 0, microseconds))),
                    other => Err(CppFormatError::Corrupt(format!("unknown temporal type: {}", other))),
                }
            }
            marker::TYPE_ZONED_TEMPORAL_DATA => {
                ...
                match tz_marker {
                    marker::TYPE_STRING => {
                        ...
                        Ok(PropertyValue::ZonedDateTime(ZonedDateTime::new(microseconds, 0, timezone)))
                    }
                    marker::TYPE_INT => {
                        let offset_minutes = self.read_u64_le()? as i16;
                        Ok(PropertyValue::ZonedDateTime(ZonedDateTime::new(microseconds, offset_minutes, String::new())))
                    }
            ...
            marker::TYPE_POINT_2D => {
                ...
                let crs = match srid {
                    ...
                    4979 => Crs::WGS843D,
                    other => return Err(CppFormatError::Corrupt(format!("unknown CRS srid: {}", other))),
                };
...
impl SectionOffsets {
    ...
    fn read(reader: &mut CppReader<'_>, version: u64) -> Result<Self, CppFormatError> {
        ...
        offsets.edges = reader.read_tagged_u64()?;
        offsets.vertices = reader.read_tagged_u64()?;
        offsets.indices = reader.read_tagged_u64()?;
        if version >= 17 {
...
impl LegacySnapshotReader {
    ...
    pub fn read(data: &[u8], version: u64) -> Result<SnapshotData, CppFormatError> {
        ...
        let mut read_vertices = |offset: u64| -> Result<Vec<VertexSnapshotEntry>, CppFormatError> {
            ...
            for _ in 0..count {
                ...
                verts.push(VertexSnapshotEntry { gid, labels, properties });
            }
            Ok(verts)
        };

        // Helper to read edges at a given offset.
        let mut read_edges = |offset: u64| -> Result<Vec<EdgeSnapshotEntry>, CppFormatError> {
            if offset == 0 || (12 + offset as usize) >= data.len() {
                return Ok(Vec::new());
            }
            let mut r = CppReader::new(&data[12 + offset as usize..]);
        ...
        let mut read_mapper = |offset: u64| -> Result<NameMapperSnapshot, CppFormatError> {
            if offset == 0 || (12 + offset as usize) >= data.len() {
            ...
            Ok(NameMapperSnapshot { labels, properties, edge_types })
        };
            ...
            while reader.remaining() > 0 {
                ...
                match section_marker {
                    ...
                    marker::SECTION_MAPPER => {
                        ...
                        mapper = NameMapperSnapshot { labels, properties, edge_types };
                    }
...
mod tests {
    ...
    fn test_legacy_wal_reader_v20() {
        ...
        assert!(matches!(records[0], DeltaRecord::VertexCreate { gid, .. } if gid == Gid::from(10u64)));
        assert!(matches!(records[1], DeltaRecord::VertexSetProperty { .. }));
        assert!(matches!(records[2], DeltaRecord::TransactionEnd { timestamp: 1, commit_timestamp: 101 }));
    }
    ...
    fn test_legacy_wal_reader_v14_tag_mapping() {
        ...
        data[payload_start + 1 + 8 + 8 + 1 + 8 + 8 + 1 + 1 + 8 + 5] = legacy_delta::V14_TRANSACTION_END;
    ...
    fn test_legacy_delta_type_enum() {
        assert_eq!(LegacyDeltaType::from_tag(legacy_delta::V20_VERTEX_CREATE, 20), LegacyDeltaType::VertexCreate);
        assert_eq!(LegacyDeltaType::from_tag(legacy_delta::V20_EDGE_DELETE, 20), LegacyDeltaType::EdgeDelete);
        assert_eq!(LegacyDeltaType::from_tag(legacy_delta::V20_TRANSACTION_END, 20), LegacyDeltaType::TransactionEnd);
        assert_eq!(LegacyDeltaType::from_tag(legacy_delta::V20_UNIQUE_CONSTRAINT_CREATE, 20), LegacyDeltaType::UniqueConstraintCreate);
        assert_eq!(LegacyDeltaType::from_tag(0xff, 20), LegacyDeltaType::Unknown(0xff));

        // v14 tag mapping
        assert_eq!(LegacyDeltaType::from_tag(legacy_delta::V14_VERTEX_CREATE, 14), LegacyDeltaType::VertexCreate);
        assert_eq!(LegacyDeltaType::from_tag(legacy_delta::V14_EDGE_SET_PROPERTY, 14), LegacyDeltaType::EdgeSetProperty);
    }
    ...
    fn test_read_legacy_delta_record_roundtrip() {
        ...
        assert!(matches!(record, DeltaRecord::VertexCreate { gid, timestamp } if gid == Gid::from(42u64) && timestamp == 100));
    }
    ...
    fn test_legacy_snapshot_temporal_property_values() {
        ...
        assert_eq!(props[1].1, PropertyValue::LocalTime(LocalTime::from_microseconds(1)));
        assert_eq!(props[2].1, PropertyValue::LocalDateTime(LocalDateTime::from_microseconds(1_000_000)));
        assert_eq!(props[3].1, PropertyValue::Duration(Duration::new(0, 0, 3_600_000_000)));
    }
    ...
    fn test_legacy_snapshot_zoned_temporal_property_value() {
        ...
        let snap = LegacySnapshotReader::read(&buf, 20).unwrap();
        assert_eq!(snap.vertices.len(), 1);
        assert!(matches!(&snap.vertices[0].properties[0].1, PropertyValue::ZonedDateTime(zdt) if zdt.utc_microseconds == 1_000_000 && zdt.timezone == "Europe/Paris"));
    }
    ...
    fn test_legacy_snapshot_point2d_property_value() {
        ...
        let snap = LegacySnapshotReader::read(&buf, 20).unwrap();
        assert_eq!(snap.vertices.len(), 1);
        assert!(matches!(&snap.vertices[0].properties[0].1, PropertyValue::Point2D(p) if p.crs == Crs::WGS84 && (p.x - 45.8150).abs() < 0.0001));
    }
    ...
    fn test_legacy_snapshot_point3d_property_value() {
        ...
        let snap = LegacySnapshotReader::read(&buf, 20).unwrap();
        assert_eq!(snap.vertices.len(), 1);
        assert!(matches!(&snap.vertices[0].properties[0].1, PropertyValue::Point3D(p) if p.crs == Crs::Cartesian3D && p.z == 3.0));
    }
