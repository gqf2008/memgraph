...
use mgdurability::cpp_format::{
    read_cpp_snapshot, read_cpp_wal, write_cpp_snapshot, write_cpp_wal,
};
use mgdurability::snapshot::{
    EdgeSnapshotEntry, NameMapperSnapshot, SnapshotData, VertexSnapshotEntry,
};
use mgdurability::version::FormatKind;
use mgslk::{SlkLoad, SlkSave};
...
fn all_property_value_variants() -> Vec<PropertyValue> {
    vec![
        ...
        PropertyValue::Map(vec![(
            "nested".into(),
            PropertyValue::Map(vec![("inner".into(), PropertyValue::Int(42))]),
        )]),
...
fn test_property_value_all_variants_roundtrip() {
    for pv in all_property_value_variants() {
        ...
        assert_eq!(
            pv,
            loaded,
            "roundtrip failed for variant with tag {}",
            pv.type_tag()
        );
    }
}
    ...
    if depth == 0 {
        ...
        match rng.gen_range(0..10) {
            ...
            5 => {
                ...
                PropertyValue::List(
                    (0..len)
                        .map(|_| generate_random_property_value(rng, depth - 1))
                        .collect(),
                )
            }
            6 => {
                ...
                PropertyValue::Map(
                    (0..len)
                        .map(|i| {
                            (
                                format!("k{}", i),
                                generate_random_property_value(rng, depth - 1),
                            )
                        })
                        .collect(),
                )
...
fn test_snapshot_native_roundtrip() {
    use mgdurability::snapshot::{SnapshotReader, SnapshotWriter};
    ...
    let data = SnapshotData {
        name_mapper: NameMapperSnapshot {
            ...
            edge_types: vec![("KNOWS".into(), EdgeTypeId::from(1u32))],
        },
        vertices: vec![
            VertexSnapshotEntry {
                gid: Gid::from(1u64),
            ...
            VertexSnapshotEntry {
                ...
                properties: vec![(
                    PropertyId::from(0u32),
                    PropertyValue::String("Memgraph".into()),
                )],
            },
        ],
    ...
    assert_eq!(
        loaded.vertices[0].properties[0].1,
        PropertyValue::String("Alice".into())
    );
...
fn test_cpp_snapshot_writer_roundtrip() {
    let data = SnapshotData {
        ...
        edges: vec![EdgeSnapshotEntry {
            ...
            to_vertex: Gid::from(2u64),
            edge_type: EdgeTypeId::from(1u32),
            properties: vec![(PropertyId::from(0u32), PropertyValue::Bool(true))],
        }],
    };
    ...
    assert_eq!(
        loaded.vertices[1].properties[0].1,
        PropertyValue::String("Memgraph".into())
    );
...
fn test_cpp_property_value_writer_roundtrip() {
    ...
    for (i, expected) in variants.iter().enumerate() {
        ...
        let loaded = read_cpp_snapshot(&buf, 20).unwrap();
        assert_eq!(
            loaded.vertices[0].properties[0].1, *expected,
            "roundtrip failed for variant at index {}: {:?}",
            i, expected
        );
    }
}
