...
fn find_legacy_cpp_output() -> Option<PathBuf> {
    ...
    for _ in 0..5 {
        for rel in [
            "cpp_slk_output.bin",
            "rust/tests/cpp_slk_output.bin",
            "tests/cpp_slk_output.bin",
        ] {
            let p = cwd.join(rel);
            if p.exists() {
                return Some(p);
...
fn test_legacy_cpp_generated_slk() {
    ...
    assert!(
        extra.is_err(),
        "expected no more data after payload, but got extra bytes"
    );
}
...
fn test_cpp_rust_bidirectional_10k() {
    ...
    assert!(result.is_ok(), "C++ write failed: {:?}", result.err());
    ...
    for n in 0..COUNT {
        ...
        let actual: Vec<Option<(i64, String)>> = Vec::slk_load(&mut reader)
            .unwrap_or_else(|e| panic!("Rust deserialize failed at iteration {}: {}", n, e));
        assert_eq!(
            expected, actual,
