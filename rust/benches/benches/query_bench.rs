...
fn bench_delta_chain_walk(c: &mut Criterion) {
    c.bench_function("delta_chain_walk_100_updates", |b| {
        ...
        for i in 0..100u64 {
            let tx = storage.begin_transaction(IsolationLevel::SnapshotIsolation);
            storage.vertex_set_property(&tx, gid, prop, PropertyValue::Int(i as i64)
            ).unwrap();
            storage.commit_transaction(&tx);
        }
...
fn bench_label_scan_1k(c: &mut Criterion) {
    c.bench_function("label_scan_1000", |b| {
        ...
        for i in 0..1000u64 {
            storage.create_vertex(&tx, Gid::from(i)).unwrap();
            if i % 2 == 0 {
                storage.vertex_add_label(&tx, Gid::from(i), label_person).unwrap();
            } else {
                storage.vertex_add_label(&tx, Gid::from(i), label_company).unwrap();
            }
        }
        storage.commit_transaction(&tx);
...
fn bench_e2e_simple_match(c: &mut Criterion) {
    ...
    c.bench_function("e2e_simple_match_1000", |b| {
        b.iter(|| {
            black_box(mginterp::execute(&storage, "MATCH (n) RETURN n"
            ).unwrap());
        })
    });
}
...
fn bench_e2e_match_with_where(c: &mut Criterion) {
    ...
    for i in 0..1000u64 {
        storage.create_vertex(&tx, Gid::from(i)).unwrap();
        storage.vertex_set_property(
            &tx, Gid::from(i), prop_age, PropertyValue::Int((i % 100) as i64)
        ).unwrap();
    }
    storage.commit_transaction(&tx);
    ...
    c.bench_function("e2e_match_where_1000", |b| {
        b.iter(|| {
            black_box(mginterp::execute(
                &storage, "MATCH (n) WHERE n.age > 50 RETURN n"
            ).unwrap());
        })
    });
}
...
fn bench_e2e_aggregate(c: &mut Criterion) {
    ...
    c.bench_function("e2e_aggregate_count_1000", |b| {
        b.iter(|| {
            black_box(mginterp::execute(
                &storage, "MATCH (n:Person) RETURN COUNT(n) AS cnt, AVG(n.age) AS avg"
            ).unwrap());
        })
    });
}
...
fn bench_e2e_create(c: &mut Criterion) {
    c.bench_function("e2e_create", |b| {
        b.iter(|| {
            let storage = Storage::new();
            black_box(mginterp::execute(
                &storage, "CREATE (n:Person {name: 'Alice', age: 30})"
            ).unwrap());
        })
    });
}
...
fn bench_e2e_shortest_path_2hop(c: &mut Criterion) {
    ...
    for i in 0..99u64 {
        storage.create_edge(
            &tx, Gid::from(1000 + i), Gid::from(i), Gid::from(i + 1), edge_type
        ).unwrap();
    }
    storage.commit_transaction(&tx);
    ...
    c.bench_function("e2e_path_2hop_100", |b| {
        b.iter(|| {
            black_box(mginterp::execute(
                &storage, "MATCH (a)-[:KNOWS*1..3]->(b) RETURN a, b"
            ).unwrap());
        })
    });
}
...
fn bench_e2e_order_by_limit(c: &mut Criterion) {
    ...
    for i in 0..1000u64 {
        storage.create_vertex(&tx, Gid::from(i)).unwrap();
        storage.vertex_set_property(
            &tx, Gid::from(i), prop_score, PropertyValue::Int((i * 7 % 1000) as i64)
        ).unwrap();
    }
    storage.commit_transaction(&tx);
    ...
    c.bench_function("e2e_order_by_limit_1000", |b| {
        b.iter(|| {
            black_box(mginterp::execute(
                &storage, "MATCH (n) RETURN n ORDER BY n.score DESC LIMIT 10"
            ).unwrap());
        })
    });
}
