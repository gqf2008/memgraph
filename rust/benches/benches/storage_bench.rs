...
fn bench_index_vs_scan(c: &mut Criterion) {
    ...
    for n_vertices in [100, 1000, 10000].iter() {
        ...
        for i in 0..*n_vertices {
            ...
            storage.vertex_set_property(&tx, gid, prop, PropertyValue::Int(i as i64)).unwrap();
            storage.vertex_set_property(&tx, gid, prop_score, PropertyValue::Int((i % 100) as i64)).unwrap();
        }
        storage.commit_transaction(&tx);
        ...
        group.bench_function(format!("label_property_scan_{}", n_vertices), |b| {
            b.iter(|| {
                black_box(storage.vertices_by_label_property(label, prop_score, &PropertyValue::Int(42)).len());
            })
        });
